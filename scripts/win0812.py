...
'害虫ブロッカー':45000,'接触冷感UVパーカー':30000,'ムダ毛シェーバー':20000,'完全遮光・形状記憶':15750,
'偏光・調光サングラス':16500,'5WAY腰掛けファン':16250,'完全遮光・接触冷感UVハット':12000,
'2WAYシートボックス':15000,'姿勢サポートチェア':10000,'携帯電動シェーバー':4000,
'接触冷感UVアームカバー':8000,'卓上冷感クーラー':7500,'バランスケアスリッパ':5000,'快適マジックインソール':8000}
def blk(n,days):
...
print("\n■ 宣言済みトリガーの判定（2026-08-11宣言）")
for n,c3,c7,new,cur in [('5WAY腰掛けファン',1.64,2.00,12000,16250),('卓上冷感クーラー',1.52,1.70,5500,7500)]:
    r={}
    for w,days in W.items():
        s_,k_,a_=blk(n,days); r[w]=s_/a_ if a_ else 0
    fire = r[3]<c3 or r[7]<c7
    print(f"  {n[:20]:20} 3日MER {r[3]:.2f}（発動 <{c3}）／7日MER {r[7]:.2f}（発動 <{c7}）"
          f" → {'🚨発動 ' + f'{cur:,}→{new:,}' if fire else '未発動・据え置き'}")
