# -*- coding: utf-8 -*-
"""2026-08-16 の判定。すべて Shopify実測 × Meta実測（8/14まで）。
...
n = 'W固定スマホ車載ホルダー'
W1, W0 = rng(2, 8), rng(9, 15)
s1, s0 = net(n, W1), net(n, W0)
a1 = ad(n, W1); a0_6 = ad(n, rng(9, 14))          # 8/15 は取得不可
...
print(f'  {"W0  8/09-14(6日)":<16}{net(n, rng(9,14)):>11,}{a0_6:>11,.0f}{net(n,rng(9,14))/a0_6:>7.2f}')
print(f'  {"W0  8/09-15(7日)":<16}{s0:>11,}{"取得不可":>11}{"—":>7}')
print(f'\n  8/15の広告費が取れないので、いくらだったかを振って感度を見る:')
print(f'  {"8/15の広告費(仮)":>18}{"Δ広告費":>11}{"Δ売上":>12}{"増分MER":>9}   判定')
for x in (25000, 30000, 32500, 35000, 40000):
    a0 = a0_6 + x; da, ds = a0 - a1, s0 - s1
    im = ds / da if da else float('nan')
    v = '🚀増額(≥2.88)' if im >= 2.88 else ('⏸据置(1.43-2.88)' if im >= be else '🔻巻き戻し(<1.43)')
    print(f'  {x:>18,}{da:>11,.0f}{ds:>12,}{im:>9.2f}   {v}')
print(f'\n  → 8/15の広告費がどの値でも判定は変わらない（Δ広告費が大きくΔ売上が小さいため）。')

# ── ② 3窓判定（3日/5日/7日・すべて8/14終端） ─────────────────────────────
print('\n■ ② 3窓判定（3日 8/12-14 / 5日 8/10-14 / 7日 8/08-14・すべて前日終端）')
print('   3窓すべてMER<分岐→停止 ／ 3窓すべて余裕<0.30→−25% ／ 3窓すべてMER≥目標→+25% ／ 割れたら据え置き')
WINS = [('3日', rng(12, 14)), ('5日', rng(10, 14)), ('7日', rng(8, 14))]
print(f'  {"商品":<26}{"分岐":>6}{"目標":>6}' + ''.join(f'{w:>16}' for w, _ in WINS) + '   判定')
res = []
for n in sorted(COST, key=lambda x: -net(x, rng(8, 14))):
    if ad(n, rng(8, 14)) < 20000: continue
