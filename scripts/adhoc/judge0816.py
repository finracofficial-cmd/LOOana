...
print(f'  ※ 8/16朝の感度分析（8/15の広告費を25,000〜40,000で振った）では1.78〜2.00。実測でも同じ帯。')

# ── ② 3窓判定（3日/5日/7日・すべて8/14終端） ─────────────────────────────
print('\n■ ② 3窓判定（3日 8/13-15 / 5日 8/11-15 / 7日 8/09-15・すべて前日終端）')
...
res = []
for n in sorted(COST, key=lambda x: -net(x, rng(9, 15))):
    if ad(n, rng(9, 15)) < 20000: continue
