# -*- coding: utf-8 -*-
"""2026-08-17 定例レポート本体（昨日=8/16 日）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R=pickle.load(open('/tmp/rep0816w.pkl','rb')); FEE=0.03452
D1,D3,D7,D30,CUM=R['D1'],R['D3'],R['D7'],R['D30'],R['CUM']
# --- 予算スナップショット（8/17・Meta直読。完全遮光日傘 15,750→10,000、ヘアドライタオル 新規8,000）
rows=list(csv.reader(open('data/budget-snapshots.csv'))); hdr=rows[0]; body=[r for r in rows[1:] if r]
assert any(r[0]=='2026-08-17' for r in body), '8/17スナップショット未記録'
BUD=collections.defaultdict(int)
for r in body:
    if r[0]=='2026-08-17': BUD[r[1]]+=int(r[3])
BSNAP=collections.defaultdict(lambda: collections.defaultdict(int))
for r in body: BSNAP[r[1]][r[0]]+=int(r[3])
# --- Meta 7日ファネル実測（8/10-16, キャンペーン別）
MF={}   # 7日ファネルはチャット本文で提示（Excelは4期間の断面＋商品タブ）
# 7日(8/10-16)の注文数 = ShopifyQL実測（FROM sales SHOW orders GROUP BY product_title）
ORD={'ナノガラス脱毛パッド':288,'W固定スマホ車載ホルダー':137,'形状記憶日傘':66,'接触冷感UVパーカー':65,
'害虫ブロッカー':63,'ムダ毛シェーバー':52,'偏光・調光サングラス':45,'2WAYシートボックス':35,'4WAY':35,
'完全遮光・接触冷感UVハット':35,'完全遮光・形状記憶':30,'快適マジックインソール':29,'接触冷感UVアームカバー':23,
'姿勢サポートチェア':23,'優先配送':22,'5WAY腰掛けファン':17,'バランスケアスリッパ':14,'携帯電動シェーバー':10,
'卓上冷感クーラー':4,'ヘアドライタオル':2,'瞬間冷感ポンチョ':2,'健康サンダル':2,'ナノバブルシャワーヘッド':2,
'3WAYサーキュレーター':2,'ジェットウォッシャー':1,'壁掛けディスペンサー':1,'姿勢サポートベルト':1}
...
sh('シンプル判定',['LOOTY 2026-08-17 定例（昨日=8/16 日）',
...
# ---------- 2 ネクストアクション ----------
NA=[['🔴 本日実行','接触冷感UVパーカー','日予算 23,000 → 17,250（−25%）。宣言済みの日次トリガー「MER<分岐が3日連続」が発動（8/14 0.66 / 8/15 1.01 / 8/16 1.06 vs 分岐1.56）。3窓すべて分岐割れ（0.89/1.15/1.48）、頑健性も3端一致（削減1円あたり +0.431/+0.337/+0.242）。稼働中で唯一の7日赤字（−6,858円）','週+約12,000円','本日'],
    ['🔴 本日実行','5WAY腰掛けファン','停止を推奨。8/16は9,001円消化で注文ゼロ、8/17も16:17時点で3,289円消化・注文ゼロ。2日で12,290円を使って売上ゼロ。3窓すべて分岐1.59割れ（0.76/1.02/1.39）、頑健性3端一致（+0.475/+0.387/+0.300）。8/16の−25%から3クリーン日ロック中だが、測る対象（注文）がゼロなので待っても何も測れない','週+約8,500円','本日'],
    ['👀 様子見','形状記憶日傘','宣言済みの日傘統合判定＋2本合算の3窓判定。単独では今日も3窓すべて余裕<0.30（+0.03/−0.04/+0.08）で−25%の条件を満たしている','週+約5,000円','8/18'],
    ['✅ 判定済','完全遮光・形状記憶（日傘・長）','8/17にユーザーが 15,750 → 10,000（−36.5%）を実施。Meta直読で daily_budget=10000 を確認。3窓判定も🔻−25%（+0.96/+1.46/+1.64、余裕 −0.40/+0.10/+0.28）で方向は一致。宣言済みの−25%より深い削減なので、この予算変更が実験を消費する → 次の判定は3クリーン日後の8/20','週+約9,000円','完了'],
    ['👀 様子見','高吸水・速乾ヘアドライタオル','中間判定（3日目）。宣言済み基準は ①link_CTR<2.2%で即停止 ②CPM≥3,100円で即停止 ③RPM/CPM<0.7で停止 ④クリック100未満なら延期。2日実測は CTR 2.87%（合格）・クリック66（100未満＝延期ライン）・CPM 3,923円（②に抵触）','—','8/18'],
    ['👀 様子見','快適マジックインソール','本判定。7日CPA<3,303円で合格。ただし3窓が割れており（1.95/2.49/2.49 vs 目標2.08）増額条件は満たしていない','—','8/19'],
    ['👀 様子見','5WAY腰掛けファン（据え置いた場合）','8/16の−25%の効果判定。3日MERが分岐1.59を割ったら停止','—','8/19'],
    ['👀 様子見','関連商品セクション','W固定+2WAY合算CVRが4.004%を割ったら即オフ、横ばい以上かつ点数/注文≥1.199で維持','週+1〜4万円','8/22'],
    ['🟡 推奨','CJへの問い合わせ1通','3週間以上未着手。新商品5品の見積 ／ 既存12品のACアダプタ同梱確認（PSE） ／ 主力6品の原価交渉。下書きは data/lp/cj-inquiry-2026-08-15.txt。パーカー(−10.1%)・4WAY(−16.0%)・携帯電動シェーバー(−3.8%)はこの交渉だけで利益率10%を超える','上位3品−10%で月274,584円','本日'],
    ['🟡 推奨','優先配送のアタッチ率','7日 22件/971注文 = 2.27%。15%まで上げると週+約47,000円（原価0なので全額が利益）。ただし商品ページ590円 vs 実課金536円の不一致が未解消。アップセルアプリの設定を先に直す','週+約47,000円','本日'],
    ['🟡 推奨','偽カウントダウンタイマーの削除','snippets/price.liquid。#minutes/#seconds が存在せず何も描画していない死んだコード。コレクションページで16個が同時に空回りしている','ページが軽くなる','本日'],
    ['📌 記録','W固定スマホ車載ホルダー','8/16の週次ランプ判定は据え置きで確定済み（消費済み）。ただし窓を1週ずらすと W-1(8/03-09) 3.96 → W0(8/10-16) 2.45、Δ広告費+125,629円に対しΔ売上+125,370円で増分MER 1.00＝分岐1.43割れ。3窓判定は⏸据え置き（2.37/2.47/2.45・余裕+0.95〜+1.05）で商品自体は健全。次の1段は増やさない','—','8/19に再判定'],
    ['👀 様子見','非常ブレーキ','3日移動平均利益 133,598円。発動ライン130,000円までの余裕は3,598円しかない。割ったら カタログ23,000→10,000 ／ 赤字商品を一律−25% を即実行','—','毎日'],
    ['👀 様子見','安全弁','全店MERが実効分岐1.42を2日連続で下回ったら赤字商品を一律−25%／単日1.20割れで即停止。8/15 2.11 → 8/16 1.87（いずれも上回っている）','—','毎日']]
sh('ネクストアクション',['今日やること・判定カレンダー・持ち越しタスク（実施が確認できるまで毎日残す）'],
   ['区分','商品/対象','内容','効果','期限'],NA,[12,26,58,20,10])
# ---------- 3 全体サマリー ----------
SUMR=[]
for lbl,N,K,ACC in [('前日 8/16(日)',R['N1'],R['K1'],R['ACCT1']),('3日 8/14-16',R['N3'],R['K3'],R['ACCT3']),
        ('7日 8/10-16',R['N7'],R['K7'],R['ACCT7']),('30日 7/18-8/16',R['N30'],R['K30'],R['ACCT30']),
        ('★当月累積 8/01-16',R['NC'],R['KC'],R['ACCTC'])]:
...
sh('全体サマリー',['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '当月累積 8/01-16: Shopify実測。返品・注文数は下の商品別シートを参照',
...
ws.cell(r,1,'■ 昨日(8/16 日) × 期間比較').font=Font(name='Arial',bold=True,size=11); r+=1
...
for lbl,d1,m3,m7,m30 in per:
    ...
    r+=1
ws.cell(r+1,1,'対同曜日（非祝日の日曜平均 1,343,866円）= 53.2%。ただし同曜日平均は日予算が50〜59万円だった時期を含むため、水準比較には使えない（下の「日曜MER比較」を見る）。3日移動平均利益 133,598円 > 非常ブレーキ130,000円 → 未発動だが余裕は3,598円しかない').font=Font(name='Arial',size=10,color='CC0000')
# ---------- 4 商品別 ----------
P=[]
for n in sorted(R['N7'],key=lambda x:-R['N7'][x]):
...
# ---------- 9 曜日指数 ----------
sh('曜日指数',['直近30日(7/18-8/16)・祝日 7/20・8/11 を除外。売上=gross−値引',
...
wb.save('data/reports/report-2026-08-17.xlsx')
