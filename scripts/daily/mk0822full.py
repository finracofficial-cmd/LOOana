...
R=pickle.load(open('/tmp/rep0821w.pkl','rb')); FEE=0.03452
...
NA=[['🔴 本日実行','形状記憶日傘 ＋ 完全遮光日傘','宣言済みの「日傘2本を1つの予算とみなした3窓判定」に該当。両方とも3窓すべて余裕<0.30（形状記憶 +0.20/+0.09/+0.14 ／ 完全遮光 −0.38/−0.09/+0.16）、頑健性も3端一致。形状記憶 35,000→26,250 ／ 完全遮光 10,000→7,500','週+約24,000円','本日'],
    ...
    ['👀 様子見','接触冷感UVパーカー','3窓すべて分岐1.56割れ（1.10/1.06/1.36）・7日 −20,186円で**稼働中で唯一の赤字**。頑健性3端一致（+0.302〜+0.477）。ただし8/17に23,000→17,250（−25%）を実施したばかりで3クリーン日ロック中。判定日は8/21','週+約13,000円','8/21'],
...
wb.save('data/reports/report-2026-08-22.xlsx')
