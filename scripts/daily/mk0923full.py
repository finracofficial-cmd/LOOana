# -*- coding: utf-8 -*-
"""2026-09-23 定例レポート本体（昨日=9/22 火・国民の休日）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0922w.pkl', 'rb'))
...
ORD = {'ナノガラス脱毛パッド':258,'伸縮ガラスクリーナー':149,'W固定スマホ車載ホルダー':123,
...
ORD30 = {'ナノガラス脱毛パッド':1123,'W固定スマホ車載ホルダー':505,'快適マジックインソール':397,
...
MF_RAW = {'ナノガラス脱毛パッド':(2406547,1236364,32856),'W固定スマホ車載ホルダー':(978904,373307,12466),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-23（昨日 = 9/22 火・国民の休日）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-22・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-23 定例（昨日=9/22 火・国民の休日）',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/22(火祝)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/20-22', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/16-22', R['N7'], R['K7'], R['ACCT7']), ('30日 8/24-9/22', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-22', R['NC'], R['KC'], R['ACCTC'])]:
...
ws.cell(r, 1, '■ 昨日(9/22 火祝) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','9/22(火祝)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/22(火・国民の休日)は実売609,765円・利益+204,499円（33.5%）で4日連続の20万円台。連休4日の合計利益は+906,616円と最高の連休だった。朝8時の「前週火曜比−36%」は終日+16.6%に反転し朝の小標本ノイズと確定。CVRは3.02%で4日連続3%台。増額3本はクリーン1日目全員合格ペース、サンダルは未達圏続き。歯ブラシは9/22失速で9/26の7日判定が未達圏濃厚＝秋衛生CRの準備を推奨。本日9/24から平日＝答え合わせ初日、明朝9/25は判定5本').font = Font(name='Arial', size=10, color='CC0000')
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/24-9/22)」。他シートの7日窓と混ぜて読まないこと',
...
sh('予算見直しチェック', ['前日(9/22)スナップショット → 本日(9/23)',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/24-9/22)。売上=gross−値引。次回定期更新9/28(月)',
