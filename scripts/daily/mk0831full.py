...
# --- 7日(8/24-30) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD = {'ナノガラス脱毛パッド':244,'W固定スマホ車載ホルダー':139,'快適マジックインソール':84,'ムダ毛シェーバー':51,
'むくみ取りかっさ':42,'2WAYシートボックス':38,'姿勢サポートチェア':36,'バランスケアスリッパ':33,
'完全遮光・接触冷感UVハット':18,'偏光・調光サングラス':17,'ナノバブルシャワーヘッド':12,'3D足臭リセットブラシ':11,
'4WAY':7,'形状記憶日傘':7,'優先配送':6,'ビジュアル耳かき':5,'携帯電動シェーバー':4,
'湯上がりガーゼワンピース':1,'瞬間冷感ポンチョ':1,'接触冷感UVアームカバー':1,'接触冷感UVパーカー':1,
'ジェットウォッシャー':1,'壁掛けディスペンサー':1,'ヘアドライタオル':1,'1秒折り畳みチェア':1,'完全遮光・形状記憶':1}
STORE_ORD7, STORE_SESS7 = 748, 22346
assert sum(ORD.values()) == 763, sum(ORD.values())   # 全店748より多いのは複数商品を含む注文が両方で数えられるため

# --- 30日(8/01-8/30) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1224,'W固定スマホ車載ホルダー':509,'形状記憶日傘':274,'接触冷感UVパーカー':231,
'4WAY':220,'ムダ毛シェーバー':213,'害虫ブロッカー':205,'快適マジックインソール':178,'偏光・調光サングラス':172,
'2WAYシートボックス':144,'完全遮光・形状記憶':134,'完全遮光・接触冷感UVハット':119,'優先配送':104,
'姿勢サポートチェア':103,'5WAY腰掛けファン':85,'接触冷感UVアームカバー':76,'バランスケアスリッパ':74,
'卓上冷感クーラー':50,'携帯電動シェーバー':47,'むくみ取りかっさ':42,'瞬間冷感ポンチョ':27,
'3WAYサーキュレーター':21,'3D足臭リセットブラシ':21,'健康サンダル':19,'ナノバブルシャワーヘッド':15,
'UV歯ブラシ除菌器':6,'壁掛けディスペンサー':5,'ビジュアル耳かき':5,'ジェットウォッシャー':4,
'湯上がりガーゼワンピース':4,'ヘアドライタオル':3,'瞬間冷却ハンディファン':1,'スマートノーズEMS':1,
'姿勢サポートベルト':1,'指圧マット':1,'1秒折り畳みチェア':1,'リカバリーサンダル':1}

# --- 30日(8/01-8/30) Metaファネル ---
# ★★ data_query がハーネス側でブロックされているため、通る campaign_and_resource_get の
#    health_check（last_30_days = 8/01-8/30 の実測・UTC基準の窓）から取った。したがって:
#      ・窓は「7日」ではなく「30日」。過去レポートの CVR順位シートと数値を並べて比較しないこと
#      ・クリックは link_clicks ではなく **全クリック**（CTR/CVR/CPC は全クリック基準で低め/高めに出る）
#    全キャンペーンで health_check の spend = CSV(8/01-8/30) が **差0円** で一致することを検算済み。
MF_RAW = {'ナノガラス脱毛パッド':(2391621,1307396,38936),'W固定スマホ車載ホルダー':(864888,462083,15064),
'カタログ全部（テスト）':(654336,170118,7088),'ムダ毛シェーバー':(565766,465091,6910),
'偏光・調光サングラス':(473626,217613,6909),'快適マジックインソール':(342789,201143,5854),
'2WAYシートボックス':(333628,114246,3967),'姿勢サポートチェア':(293963,184271,4196),
'完全遮光・接触冷感UVハット':(284128,66389,3628),'バランスケアスリッパ':(183248,118969,2479),
'3D足臭リセットブラシ':(85838,24927,422),'むくみ取りかっさ':(74495,20141,798),
'ナノバブルシャワーヘッド':(52366,22888,914),'ビジュアル耳かき':(20554,7070,141)}
...
sh('収益', [f'LOOTY 収益レポート 2026-08-31（昨日 = 8/30 日）',
  ...
  f'★このペースなら8月着地は 利益 {cp/len(CUM)*_dim:,.0f}円（残{_dim-len(CUM)}日）',
...
sh('シンプル判定', ['LOOTY 2026-08-31 定例（昨日=8/30 日）',
...
NA = [
 ['🎉 変化点','非常ブレーキの上を5日連続で維持',
  f'3日移動平均利益 {p3ma:,.0f}円 ≧ 発動ライン130,000円。8/26に上抜けして以降 156,354 → 141,188 → 151,714 → **141,718** で5日連続ライン上。'
  '前日(8/30 日)は 実売594,407・利益+164,494円・利益率27.7%・MER 2.25。'
  '**前週日曜(8/23 507,832円)比 +17.0%**',
  '—','毎日'],
 ['🔴 判定日','4WAY・接触冷感UVアームカバー 停止判定 → ✅合格',
  '宣言済み基準「全店週次利益(8/24-30) ≥ 696,901円」に対し 実測 **1,244,014円（基準の179%）**。'
  '両方とも8/23が最終消化で停止済み。**この判定は完了。以後リストから外す**',
  '—','本日 完了'],
 ['🔴 判定日','ビジュアル耳かき 3日判定（8/28-30）→ 🔻 8,000 → 5,000円',
  '宣言済み基準「3日累計倍率 <0.70→停止 ／ 0.70〜1.00→8,000→5,000 ／ ≥1.00→据え置き」。'
  '実測: 実売28,884円 / 6点 / 広告20,554円 → 実CPA 3,426円 vs 分岐CPA 3,325円 = **倍率0.97**。損益 −604円。'
  '★0.97は1.00のすぐ下で、6点しかない。**停止ではなく減額**が宣言どおりの処置。'
  '累計消化20,554円で「30,000円超かつ倍率1.00未満なら即停止」にはまだ当たらない',
  '週+約1,000円','本日 実行'],
 ['⏸ 前倒ししない','快適マジックインソール（新実験のP2が今日から）',
  '8/30は 実売27,860円 / 7点 / 広告28,842円 で **MER 0.97**（分岐1.21割れ）。3日MERも1.84まで低下。'
  'ただし **今日から P2（8/31-9/2・予算30,000）が始まる**。ここで動かすと2回目の実験も壊れる。'
  '**判定日 9/3・合格ライン 1日23,280円**。それまで触らない',
  '—','9/3に執行'],
 ['⚠️ 監視','快適マジックインソールの8/30の弱さ',
  'セッション396（全店−6.4%に対し−12.0%）／カート追加率 6.00%→4.80%／カート→購入 51.9%→31.6%。'
  '**どちらも単独では有意差なし（カート→購入は z=−1.41）**。全店のカート追加率は6.25%で完全に横ばい。'
  '8/31・9/1で2日続けて4%台なら質の変化として確定させ、CR側の手当てを出す',
  '—','9/1まで監視'],
 ['🚀 増額候補','バランスケアスリッパ（3窓とも目標超え）',
  '3日2.72 / 5日2.67 / 7日2.60 で目標2.59を全窓で超過。分岐1.36に対し余裕+1.24〜1.36。'
  '★ただし **8/30に 10,000→12,000 へ増額したばかり**（3クリーン日ロック中）。'
  '**9/3に「1日あたり利益 ≥ 増額前の7日平均」で判定**してから次を考える',
  '—','9/3に執行'],
 ['⚠️ 未確定','本日の予算スナップショットが未取得',
  'このレポートの「現日予算」列と週消化率は **8/30 18時のMeta直読値（合計255,000円）** を使っている。'
  '8/31朝の読み直しがまだ。予算を変更していたら消化率がずれるので、変更があれば教えてください',
  '—','本日'],
 ['🔴 判定日','ナノバブルシャワーヘッド 本判定','倍率 ≥1.3 継続 ／ 1.0〜1.3 は−50% ／ <1.0 停止。'
  '7日(8/24-30) MER 2.14・分岐1.49・余裕+0.66。★増額は日10,000円が上限','—','9/1'],
 ['🔴 判定日','決済ミックス 月次再計測','直近100注文の paymentGatewayNames × 金額でブレンド率を更新（現行3.452%）','—','9/1'],
 ['🔴 判定日','カタログ 3日判定（8/31-9/2）','CPA基準の倍率 ≥1.00 で据え置き。8/27-29は倍率1.12だった','—','9/2'],
 ['🔴 判定日','優先配送 790円',
  '宣言済み基準「14日で件数 ≥ 28件」。7日実測 **6件**（アタッチ率 6/748 = 0.80%）。不合格の見込み','—','9/4'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 8/30(日)', R['N1'], R['K1'], R['ACCT1']), ('3日 8/28-30', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/24-30', R['N7'], R['K7'], R['ACCT7']), ('30日 8/01-8/30', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 8/01-30', R['NC'], R['KC'], R['ACCTC'])]:
...
sh('全体サマリー', ['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '縦照合3本すべて通過: Σ商品gross=全店607,150 ／ Σ値引=12,743 ／ Σキャンペーン広告費=264,342円（8/30）',
...
ws.cell(r, 1, '■ 昨日(8/30 日) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','8/30(日)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
ws.cell(r+1, 1, '⚠️ 同曜日平均比は当面読まないこと。直近30日の日曜（8/02・8/09・8/16）が夏物崩壊前・日予算590,000時代で汚染されている。'
  f'実態は前週日曜(8/23 507,832円)比 **+17.0%**。3日移動平均利益 {p3ma:,.0f}円 は非常ブレーキ130,000円を'
  '5日連続で上回っている').font = Font(name='Arial', size=10, color='CC0000')
...
sh('商品別', ['商品別（7日 8/24-30 = 判定単位）。カタログ行の広告費は「7日合計」（前日ではない）',
  '分岐CPA(注文) = 1個あたり粗利 × 点数/注文。まとめ買い商品（点数/注文>1.2）は必ず注文ベースで比較する',
  'Σ商品広告費 + カタログ = Metaアカウント7日消化 1,780,812円（差0円・w0830.pyでassert済み）',
  '★3D足臭は8/29に停止済み。7日窓には残消化52,914円が入っている（8/30の消化は0円）',
...
sh('週次診断', ['全店の週次分解。原価は日次のバリアント構成が取れないナノガラス/ディスペンサーのみ30日実測ミックスの加重平均単価【近似】',
  ...
  '★最新週(8/24-30)は日予算253,000〜255,000で運転。日予算590,000だった7月とは水準が違うので、利益額の絶対比較はしない'],
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/01-8/30)」。他シートの7日窓と混ぜて読まないこと',
  ...
  '✅ 検算: 14キャンペーンすべてで health_check の消化 = CSV(8/01-8/30) が差0円で一致（コード内assert）',
...
JUDGE = [
 ['8/30','4WAY・接触冷感UVアームカバー 停止判定','全店週次利益(8/24-30) ≥ 696,901円',
  '実測 **1,244,014円**（基準の179%）','✅ 合格・完了',
  '両方とも8/23が最終消化で停止済み。判定は完了したのでカレンダーから外す'],
 ['8/31','ビジュアル耳かき 3日判定（8/28-30）',
  '3日累計倍率 <0.70→停止 ／ 0.70〜1.00→8,000→5,000 ／ ≥1.00→据え置き',
  '実売28,884円 / 6点 / 広告20,554円 → 実CPA 3,426円 vs 分岐CPA 3,325円 = **倍率0.97**（損益 −604円）',
  '🔻 8,000 → 5,000円',
  '0.70〜1.00の帯なので宣言どおり減額。停止ではない。'
  '3窓判定は「停止/−50%」を出すが、**3窓とも同じ3日を見ているだけ**（8/28開始のため）。'
  '累計消化20,554円で「30,000円超かつ倍率1.00未満なら即停止」には未到達。'
  '5,000円で3日回して、9/3に同じ基準で再判定する'],
 ['9/1','ナノバブルシャワーヘッド 本判定','倍率 ≥1.3 継続 ／ 1.0〜1.3 は−50% ／ <1.0 停止',
  '7日(8/24-30) MER 2.14・分岐1.49・余裕+0.66','進行中',
 ...
 ['9/2','カタログ 3日判定（8/31-9/2）','CPA基準: 倍率 ≥1.00 で据え置き（share基準は監視指標）',
  '前回(8/27-29)は 帰属19点・実CPA 2,859円 vs 分岐3,207円 = 倍率1.12','進行中',
  '8/30の消化は17,731円。注文UTM(utm_term=6966272711408)で3日ぶんを数え直して判定する'],
 ['9/3','快適マジックインソール 増額判定（30,000の実験）','P2(8/31-9/2)の1日あたり利益 ≥ 23,280円',
  'P1 = 8/28-29（予算27,000・2日）= 1日23,280円。**P2は本日から開始**','進行中',
  '★8/30は変更当日なので両窓から除外。8/30単日はMER 0.97と弱いが、これはP1にもP2にも入らない。'
  '「グリーン」全7サイズ在庫0の影響を判定時に必ず併記する'],
 ['9/3','バランスケアスリッパ 増額判定','1日あたり利益 ≥ 増額前(8/23-29)の7日平均',
  '8/30に 10,000→12,000 へ増額。3窓とも目標2.59超（3日2.72/5日2.67/7日2.60）','進行中',
  '3窓判定は「+25%」を出すが、増額直後のロック中なので重ねない'],
 ['9/4','優先配送 790円','14日で件数 ≥ 28件','7日実測 **6件**（アタッチ率 6/748 = 0.80%）','🚨不合格の見込み',
  '前週6件から横ばい。8/22に536→790へ値上げ済み'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は1件: **ビジュアル耳かき 8,000 → 5,000円**（3日倍率0.97＝0.70〜1.00の帯）',
  '★4WAY・アームカバーの停止判定は 1,244,014円 ≥ 696,901円 で合格・完了',
  '★快適マジックインソールとバランスケアスリッパは3窓判定がシグナルを出しているが、'
  'どちらも実験のロック中（9/3判定）なので触らない'],
...
# ---------- 11 曜日指数 ----------
sh('曜日指数', ['直近30日(8/01-8/30)・祝日 8/11(山の日) を除外。売上=gross−値引',
...
wb.save('data/reports/report-2026-08-31.xlsx')
