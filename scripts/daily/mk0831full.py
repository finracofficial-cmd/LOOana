...
SNAP = '2026-08-31'   # ★8/31朝に data_query で直読（8/30b から変更なし・255,000円/日）
...
# ★2026-08-31: `data_query` のブロックが解除されたため、health_check(30日・UTC窓・全クリック) から
#   **日別集計と同じ Asia/Tokyo の 8/01-8/30 窓・アウトバウンドクリック**の実測へ差し替えた。
#   これで他シート（7日窓）とクリックの定義が揃い、過去レポートの「CVR順位」とも比較できる。
#   (cost, impressions, outbound_clicks)
MF_RAW = {'ナノガラス脱毛パッド':(2391621,1307396,33423),'形状記憶日傘':(898297,273266,10533),
'W固定スマホ車載ホルダー':(864888,462083,13186),'害虫ブロッカー':(685874,315961,6344),
'カタログ全部（テスト）':(654336,170118,6327),'4WAY 取り付けOK 小型瞬間冷却ハンディファン':(652983,155942,5129),
'ムダ毛シェーバー':(565766,465091,5743),'接触冷感UVパーカー':(510916,161987,5470),
'偏光・調光サングラス':(473626,217613,5483),'完全遮光・形状記憶・晴雨兼用・UV日傘':(409558,105397,4801),
'快適マジックインソール':(342787,201142,5133),'2WAYシートボックス':(333628,114246,3314),
'姿勢サポートチェア':(293963,184271,3574),'完全遮光・接触冷感UVハット':(284128,66389,3126),
'5WAY腰掛けファン':(240525,84906,3651),'接触冷感UVアームカバー':(225861,59115,1765),
'バランスケアスリッパ':(183248,118969,2124),'卓上冷感クーラー':(173635,68088,2876),
'携帯電動シェーバー':(105430,51022,882),'瞬間冷感ポンチョ':(104188,22483,1018),
'3D足臭リセットブラシ':(85838,24927,338),'3WAYサーキュレーター扇風機':(84346,21609,880),
'むくみ取りかっさ':(74495,20141,644),'健康サンダル':(69499,22016,641),
'ナノバブルシャワーヘッド':(52366,22888,730),'UV歯ブラシ除菌器':(35675,12218,259),
'ビジュアル耳かき':(20554,7070,118),'高吸水・速乾ヘアドライタオル':(10669,2706,75),
'ディスペンサー':(10210,3481,120),'カタログ全部（テスト） 夏以外':(6080,1903,50),
...
NA = [
 ['🎉 変化点','非常ブレーキの上を5日連続で維持',
  f'3日移動平均利益 {p3ma:,.0f}円 ≧ 発動ライン130,000円。8/18-25の9日連続割れ → 8/26に136,219で上抜け → '
  '8/27 156,339 → 8/28 141,196 → 8/29 152,731 → 8/30 142,731 で **5日連続ライン上**。'
  '前日(8/30 日)は 実売594,407・利益+164,143円・利益率27.6%。前週日曜(8/23 507,832円)比 **+17.0%**、'
  '前日(8/29 583,139円)比 +1.9%。土日で2日続けて戻している',
  '—','毎日'],
 ...
 ['🔴 判定日','カタログ 3日判定（8/27-29）→ ✅据え置き',
  ...
  '—','完了（次回 9/2）'],
 ['✅ 決着','4WAY・接触冷感UVアームカバー 停止判定 → 合格（確定）',
  '宣言済み基準「全店週次利益(8/24-30) ≥ 696,901円」に対し、7日確定で **1,030,500円（+47.9%）**。'
  '両方とも8/23が最終消化で停止済み。**夏物2つを止めても全店の週次利益は落ちていない**ことが7日で確定した',
  '—','完了'],
 ['🔴 今日やること','ビジュアル耳かき 日予算 8,000 → 5,000',
  '宣言済みの3日判定を執行。8/28-30の3日累計は 6個・広告20,554円・実CPA 3,426円 vs 分岐CPA 3,325円 = **倍率0.97**'
  '（0.70〜1.00帯 → 減額）。3日利益 −1,600円。'
  '★決め手は日別の形: 売上は3日とも 9,960円(2個) で完全に固定なのに、広告費だけ 3,328 → 7,347 → 9,879円 と3倍。'
  '配信を広げても買う人が増えていない。★停止ではなく減額なのは、まだ累計6個で「購入50件まで確定判断を保留」の手前だから。'
  '次は9/3に同じ形で見る（倍率<0.70 or 累計消化30,000円超で倍率<1.00 なら停止）',
  '出血 約1,900円/日を止める','本日 5分'],
 ['🔵 予算見直し','8/30 昼にユーザーが6セット変更（合計 253,000→255,000円）',
  ...
  '週+1,143円見込み','完了'],
 ['🚫 判定を差し替えた','快適マジックインソールの8/31判定は執行不能（宣言済み）',
  'P2窓(8/28-30)の最終日に予算が27,000→30,000へ変わったため、宣言済みの判定は成立しない。'
  '「同一キャンペーンの再変更は前の実験を消費する」ルールに従い実験を終了し、'
  '**新しい実験（P1=8/28-29の27,000 vs P2=8/31-9/2の30,000・判定日9/3・合格ライン 1日23,356円）**を宣言済み。'
  '⚠️ 予算は判定日の翌日以降に動かすと、この取りこぼしが起きない',
  '—','9/3に執行'],
 ['⚠️ トリガー点灯','完全遮光・接触冷感UVハット が3日連続で分岐割れ（ただし本日は動かさない）',
  '8/28 1.28 ／ 8/29 1.41 ／ 8/30 1.26（分岐1.42）で日次トリガー「3日連続 MER<分岐 → −25%」が点灯。'
  'ただし **8/30に 8,000→6,000（−25%）を実行済み**で、トリガーが見ている3日のうち2日は減額前の8,000円時代。'
  '今日さらに削ると1つのシグナルに2回反応することになる。7日はまだ MER 1.89・余裕+0.47・利益+18,680円の黒字。'
  '**9/2（変更から3クリーン日）に 6,000円での3日MERが分岐1.42を超えているかで判定。割れていれば夏物なので停止**',
  '—','9/2に判定'],
 ['⚠️ 学習中','むくみ取りかっさ +30% は学習リセットに注意',
  ...
  '—','9/2まで監視'],
 ['⏸ 増額しない','バランスケアスリッパ（3窓判定は「+25%」だがロック中）',
  '3窓すべてで目標超（7日MER 2.60・3日2.72・前日3.07 vs 目標2.59）＋週消化率95%で、機械的には +25%（12,000→15,000）。'
  'ただし **8/30に 10,000→12,000（+20%）へ上げたばかり**で、変更後3クリーン日ロック（9/2まで）に当たる。'
  '重ねて上げると、どちらの段の効果か分離できなくなる。**9/2に12,000での実績を見てから次の段を決める**',
  '—','9/2に判定'],
 ['⚠️ 在庫','快適マジックインソール「グリーン」全7サイズが在庫0',
  '他3色は各1万個以上ある。売れ筋色が買えない状態なので、非表示にするか色の選択肢から落とすかを決める。'
  '9/3の増額判定に直接効く。★8/30は 実売27,860 / 広告28,839 = **−5,718円** と全商品で唯一の前日赤字だった',
  '—','本日'],
 ['✅ 実行済み','3D足臭リセットブラシ 停止',
 ...
 ['🔴 判定日','8月の着地（本日が最終日）',
  '8/01-30の30日で 実売21,688,393 / 原価5,997,005 / 広告10,850,694 → **利益4,840,694円（22.3%）**、1日あたり161,356円。'
  '残り1日を直近3日平均（142,619円）で見ると **8月着地は約4,983,000円**。'
  '★7月（日予算590,000円時代）と利益額を単純比較しないこと。8月は日予算を 590,000→255,000（−57%）に絞った月',
  '—','9/1に確定'],
...
sh('全体サマリー', ['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '縦照合3本すべて通過: Σ商品gross=全店607,150 ／ Σ値引=12,743 ／ Σキャンペーン広告費=264,544円（8/30）',
 '★8/31に data_query のブロックが解除された。8/30は日別×キャンペーン別の実測を直接書き込み、'
 'アカウントレベル1本と Σキャンペーン が **差0円** で一致することを確認済み（導出でも推計でもない）',
 '🚨 あわせて2件の訂正を入れた。①8/29の広告費は health_check の差分法で 241,446円としていたが、確定値は 238,492円'
 '（+2,954円/+1.24%の過大）。差分法は「30日窓の全日が確定済み」を前提にするが直近数日はドリフトするため、'
 'そのズレが最終日に全部乗る構造だった。②8/01-28 も日次書き込み時の切り捨てで −5,083円（−0.047%）過少だったので実測へ置換。'
 '結果、当月(8/01-30)の広告費は全30日でMetaの実測と差0円になった',
...
JUDGE = [
 ...
 ['8/30','4WAY・接触冷感UVアームカバー 停止判定','全店週次利益(8/24-30) ≥ 696,901円',
  '8/24-30 の7日確定で **1,030,500円**（基準比 +47.9%）','✅合格（確定）',
  '両方とも8/23が最終消化で停止済み。7日目まで揃えて確定。この2商品を止めても全店の週次利益は落ちていない'],
 ...
 ['8/31','ビジュアル耳かき 早期撤退ライン','3日累計(8/28-30)倍率 <0.70→停止 ／ 0.70〜1.00→8,000→5,000 ／ ≥1.00→据え置き',
  '8/28-30の3日で 6個・広告20,554円・実CPA 3,426円 vs 分岐CPA 3,325円 = **倍率0.97**（3日利益 −1,600円）',
  '🔻 8,000 → 5,000 へ減額（本日執行）',
  '★0.70〜1.00帯にちょうど入ったので宣言どおり減額。★決め手は日別の形: 売上は3日とも 9,960円(2個)で完全に固定なのに、'
  '広告費だけ 3,328 → 7,347 → 9,879円 と3倍に伸びている。予算を足しても売れる数が動かない＝配信を広げても'
  '買う人が増えていない。停止ではなく減額なのは、まだ3日6個で購入50件ルールの手前だから。次は9/3に同じ形で見る'],
 ['8/31→中止','快適マジックインソール 増額判定（27,000の実験）','P2(8/28-30)の1日あたり利益 ≥ 28,310円',
  'P1(8/24-26・予算22,000) = 1日31,704円。P2は8/28-29の2日で1日23,356円',
  '🚫 執行不能 → 実験を差し替え',
 ...
 ['9/3','快適マジックインソール 増額判定（30,000の実験・新規）','P2の1日あたり利益 ≥ P1の1日あたり利益',
  'P1 = 8/28-29（予算27,000・2日）実売117,609 / 31個 / 広告49,911 / 利益+46,711 = **1日23,356円**。'
  'P2 = 8/31-9/2（予算30,000・3日）。8/30は変更当日なので両窓から除外',
  '進行中（基準を本日宣言）',
  '★合格ライン = **1日23,356円以上**。P1が2日しかないので、判定時に「P1が短い」ことを併記する。'
  '★「グリーン」全7サイズ在庫0の影響を判定時に必ず併記する。'
  '⚠️ 変更当日の8/30は 実売27,860 / 広告28,839 で **−5,718円**（全商品で唯一の前日赤字）。'
  '増額直後2〜3日はペーシングが追いつかず落ちて見えるのが正常なので、この1日で判断しない'],
 ['9/2','完全遮光・接触冷感UVハット（日次トリガーが点灯）','MER<分岐 が3日連続 → その日に −25%',
  '8/28 1.28 ／ 8/29 1.41 ／ 8/30 1.26（分岐1.42）＝**3日連続で分岐割れ**。3日利益 −1,359円',
  '⏸ 本日は追加減額しない（8/30に実行済みの−25%がこのシグナルへの対応そのもの）',
  '★8/30に 8,000→6,000（−25%）を実行済み。トリガーが見ている3日のうち2日(8/28-29)は減額前の8,000円時代なので、'
  '今日さらに削ると1つのシグナルに2回反応することになる。★7日はまだMER 1.89・余裕+0.47・利益+18,680円の黒字。'
  '**9/2（変更から3クリーン日）に、6,000円での3日MERが分岐1.42を超えているかで判定する。'
  '割れていれば夏物なので停止**（残シーズン1週）'],
...
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は1件: ビジュアル耳かき 8,000 → 5,000（3日倍率0.97＝宣言済みの0.70〜1.00帯）',
  '★4WAY・アームカバーの停止判定は7日確定で ✅合格（1,030,500円 ≥ 基準696,901円）',
  '★バランスケアスリッパは3窓判定が「+25%」を出しているが、**8/30に10,000→12,000へ上げたばかり**。'
  '変更後3クリーン日ロック（9/2まで）に当たるので重ねて増額しない',
  '★快適マジックインソールも同じ理由で据え置き。9/3の判定を待つ'],
...
sh('曜日指数', ['直近30日(8/01-8/30)・祝日 8/11(山の日) を除外。売上=gross−値引。★毎週月曜に更新（本日更新）',
  ...
  '★代わりに「前週同曜日比」で読む。8/30(日)は前週日曜(8/23 507,832円)比 +17.0%',
