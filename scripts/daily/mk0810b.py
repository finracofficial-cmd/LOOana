...
cur=sum(v['2026-08-09'][0]-v['2026-08-09'][1] for v in S.values() if '2026-08-09' in v)
print(f"  直近4木曜平均 {pa:,.0f} → 8/06 {cur:,.0f} ({cur/pa-1:+.1%})")
print()
j=days.index('2026-08-09'); ma3=sum(prof[x] for x in days[j-2:j+1])/3
print(f"=== 非常ブレーキ（3日移動平均利益 < 130,000 で発動） ===\n  8/04-06 の3日MA = {ma3:,.0f} → {'🚨 発動' if ma3<130000 else '✅ 未発動'}")
print(f"=== 利益額の目標（最低100,000 / 目標180,000） ===\n  8/06 利益 {prof['2026-08-09']:,.0f}（手数料控除後 {prof['2026-08-09']-cur*0.03452:,.0f}）")
