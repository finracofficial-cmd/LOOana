# -*- coding: utf-8 -*-
"""2026-08-08 定例（昨日=8/08 土実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(34,13),'d3':(115,46),'d7':(250,100),'d30':(838,357)},  # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(0,0),'d7':(1,0),'d30':(53,0)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':197},'湯上がりガーゼワンピース':{'d30':65},
      '完全遮光・接触冷感UVハット':{'d30':39},'3WAYサーキュレーター':{'d7':18,'d30':173},
      '姿勢サポートチェア':{'d7':22,'d30':22}}
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-10']
assert D1==['2026-08-08'] and D3[0]=='2026-08-06' and D7[0]=='2026-08-02', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-10', (len(D30),D30[0])
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0809.pkl','wb'))
