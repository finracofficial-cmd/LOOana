# -*- coding: utf-8 -*-
"""2026-08-12 定例レポート本体。8/06まであったシート構成を復元（8/07-8/10は4シートに欠落していた）。"""
...
if not any(r[0]=='2026-08-12' for r in body):
    for r in [x for x in body if x[0]=='2026-08-11']: body.append(['2026-08-12',r[1],r[2],r[3]])
...
for r in body:
    if r[0]=='2026-08-12': BUD[r[1]]+=int(r[3])
...
MF={}   # 7日ファネルは翌日以降にまとめて取得（8/12は日次のMeta実測のみ）
...
sh('シンプル判定',['LOOTY 2026-08-12 定例（昨日=8/10 月）',
...
sh('曜日指数',['直近30日(7/12-8/10)・祝日7/20除外。売上=gross−値引',
  ...
  '★2026-08-11(山の日)に祝日の上振れは起きなかった（平常火曜比73.0%・海の日比57.0%）。祝日を一律に週末扱いしない'],
 ['曜日','指数(全体=100)'],[[k,round(v,1)] for k,v in sorted(R['IDX'].items(),key=lambda x:-x[1])],[10,16])
wb.save('data/reports/report-2026-08-12.xlsx')
