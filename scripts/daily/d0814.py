# -*- coding: utf-8 -*-
"""2026-08-14 定例（昨日=8/13 木 実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(21,11),'d3':(87,45),'d7':(226,111),'d30':(852,375),'cum':(438,191)},
     '壁掛けディスペンサー':{'d1':(0,2),'d3':(0,2),'d7':(1,2),'d30':(39,10),'cum':(3,2)}}
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':200},'湯上がりガーゼワンピース':{'d30':38},
      '3WAYサーキュレーター':{'d30':138,'cum':27},'姿勢サポートチェア':{'d30':42,'cum':42},
      '完全遮光・接触冷感UVハット':{'d30':65}}
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-15']
assert D1==['2026-08-13'] and D3[0]=='2026-08-11' and D7[0]=='2026-08-07', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-15', (len(D30),D30[0])
...
assert len(CUM)==13 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-13'
...
assert sum(v[0] for n in S for d in CUM for v in [S[n][d]] if d in S[n])==10296944+926316+704468+676628
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0814.pkl','wb'))
