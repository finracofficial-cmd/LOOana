# -*- coding: utf-8 -*-
"""2026-08-08 定例（昨日=8/09 日実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(25,22),'d3':(104,56),'d7':(235,101),'d30':(833,368)},  # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(1,0),'d3':(1,0),'d7':(2,0),'d30':(50,0)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':198},'湯上がりガーゼワンピース':{'d30':58},
      '3WAYサーキュレーター':{'d30':167},'姿勢サポートチェア':{'d30':25},
      '完全遮光・接触冷感UVハット':{'d30':47}}
# 壁掛けディスペンサーの30日は gross の9.1%がバリアント未分解 → 原価は【概算】
DISP_D30_COST_APPROX=147293   # 430,720×35.77%（バリアント未分解分あり=概算）
HOL={'2026-07-20'}
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-11']
assert D1==['2026-08-09'] and D3[0]=='2026-08-07' and D7[0]=='2026-08-03', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-11', (len(D30),D30[0])
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0810.pkl','wb'))
