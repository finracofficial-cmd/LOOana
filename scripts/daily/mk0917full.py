# -*- coding: utf-8 -*-
"""2026-09-17 定例レポート本体（昨日=9/16 水）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0916w.pkl', 'rb'))
...
ORD30 = {'ナノガラス脱毛パッド':1129,'W固定スマホ車載ホルダー':485,'快適マジックインソール':371,
...
MF_RAW = {'ナノガラス脱毛パッド':(2400994,1230784,31089),'W固定スマホ車載ホルダー':(991812,402843,12185),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-17（昨日 = 9/16 水）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-16・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-17 定例（昨日=9/16 水）',
...
NA = [
 ['✅ 本日判定','もちふわ肉球サンダル **Phase1合格 → 8,000で継続確定**',
  ...
  '—','クローズ'],
 ['⚠️ 注意','9/16(水)は利益+96,497円（21.2%）と9月では弱い日。3日移動平均130,828円はブレーキ線130,000円の+828円',
 ...
 ['🟡 提案','非常ブレーキ閾値の再校正（130,000→100,000円）',
  '承認待ち継続。**本日3日移動平均130,828円で旧基準に+828円まで接近**＝現行基準では体制縮小後の平常運転でも誤発動しかねない',
  '—','ユーザー判断'],
]
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/16(水)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/14-16', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/10-16', R['N7'], R['K7'], R['ACCT7']), ('30日 8/18-9/16', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-16', R['NC'], R['KC'], R['ACCTC'])]:
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/16(水)は実売454,983円・利益+96,497円（21.2%）と9月の弱い日。負け枠（スリッパ・カタログ・電動かっさ・チェア）が同日に重なった一方、勝ち枠（インソール・ガラス・ナノ・サンダル）は通常運転。本日の判定はサンダルPhase1が圧勝合格。明日はガラス/フック/カタログ/電動かっさの4本判定で、カタログと電動かっさが不合格圏＝負け枠の整理が進む見込み').font = Font(name='Arial', size=10, color='CC0000')
...
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  ...
  '★9/14-16の3日利益 +392,483円（平均130,828円/日）＝ブレーキ線130,000円に+828円まで接近'],
...
sh('予算見直しチェック', ['前日(9/16)スナップショット → 本日(9/17)',
...
'2026-08-28':85,'2026-08-29':115,'2026-08-30':114,'2026-08-31':84,'2026-09-01':88,'2026-09-02':92,'2026-09-03':103,'2026-09-04':74,'2026-09-05':106,'2026-09-06':122,'2026-09-07':96,'2026-09-08':95,'2026-09-09':98,'2026-09-10':116,'2026-09-11':96,'2026-09-12':117,'2026-09-13':140,'2026-09-14':102,'2026-09-15':103,'2026-09-16':99}
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/18-9/16)。売上=gross−値引。次回定期更新は 9/21(月)',
