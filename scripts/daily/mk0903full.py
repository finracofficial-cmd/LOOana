# -*- coding: utf-8 -*-
"""2026-09-04 定例レポート本体（データ終端=9/02 水／9/03の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0902w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
...
ORD = {'ナノガラス脱毛パッド':263,'W固定スマホ車載ホルダー':121,'快適マジックインソール':83,'ムダ毛シェーバー':48,
'バランスケアスリッパ':38,'むくみ取りかっさ':37,'姿勢サポートチェア':28,'2WAYシートボックス':27,
'ナノバブルシャワーヘッド':19,'完全遮光・接触冷感UVハット':14,'偏光・調光サングラス':10,'優先配送':6,
'ビジュアル耳かき':5,'3D足臭リセットブラシ':5,'携帯電動シェーバー':4,'4WAY':3,'形状記憶日傘':2,
'ネックマッサージャー':1,'瞬間冷感ポンチョ':1,'接触冷感UVパーカー':1,'ジェットウォッシャー':1,
'壁掛けディスペンサー':1,'温感EMSフェイシャルワンド':1,'1秒折り畳みチェア':1,'完全遮光・形状記憶':1}
STORE_ORD7, STORE_SESS7 = 708, 21980
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 721, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1183,'W固定スマホ車載ホルダー':533,'快適マジックインソール':213,'形状記憶日傘':208,
'ムダ毛シェーバー':194,'接触冷感UVパーカー':165,'2WAYシートボックス':151,'偏光・調光サングラス':146,
'害虫ブロッカー':144,'4WAY':140,'完全遮光・接触冷感UVハット':118,'姿勢サポートチェア':105,
'完全遮光・形状記憶':87,'バランスケアスリッパ':87,'優先配送':76,'接触冷感UVアームカバー':58,
'5WAY腰掛けファン':55,'むくみ取りかっさ':55,'携帯電動シェーバー':35,'卓上冷感クーラー':26,
'ナノバブルシャワーヘッド':24,'3D足臭リセットブラシ':21,'瞬間冷感ポンチョ':11,'3WAYサーキュレーター':7,
'健康サンダル':6,'ビジュアル耳かき':5,'壁掛けディスペンサー':4,'ヘアドライタオル':3,'湯上がりガーゼワンピース':3,
'ジェットウォッシャー':2,'瞬間冷却ハンディファン':1,'UV歯ブラシ除菌器':1,'姿勢サポートベルト':1,'指圧マット':1,
'温感EMSフェイシャルワンド':1,'1秒折り畳みチェア':1,'リカバリーサンダル':1,'ネックマッサージャー':1}
...
# ★2026-09-04 取得の health_check（窓 = 8/04-9/02 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2387017,1289789,38027),'W固定スマホ車載ホルダー':(940058,490450,16149),
'カタログ全部（テスト）':(616402,156004,6367),'ムダ毛シェーバー':(566408,460181,6638),
'快適マジックインソール':(437423,261945,7489),'偏光・調光サングラス':(415243,182906,5910),
'2WAYシートボックス':(359273,122995,4207),'姿勢サポートチェア':(328942,206444,4658),
'完全遮光・接触冷感UVハット':(280179,63866,3542),'バランスケアスリッパ':(202850,139860,2838),
'むくみ取りかっさ':(111340,30391,1191),'ナノバブルシャワーヘッド':(74112,34205,1248),
'温感EMSフェイシャルワンド':(4551,886,30)}
...
sh('シンプル判定', ['LOOTY 2026-09-04 定例（データ終端=9/02 水）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/02終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['⚠️ お詫び','**9/3朝のレポートを出せませんでした**',
  '9/3の朝に9/02のShopify実測までは取れていたのですが、広告費の窓（health_checkの30日集計はUTC基準で、'
  'JST 09:00を回るまで前日が入らない）を待っている間にセッションが止まり、そのまま届けられませんでした。'
  '★このレポートは **データ終端 9/02（水）** で、9/3が期日だった判定2件を1日遅れで執行しています。'
  '9/03(木)のShopify実績は下の速報行に、広告費は09:00 JST以降に確定させます',
  '—','記録'],
 ['🔴 判定日','快適マジックインソール 増額判定（27,000→30,000）→ **❌ 不合格**',
  '宣言済み基準「P2(8/31-9/2)の1日あたり利益 ≥ 23,280円」。'
  'P1(8/28-29・27,000) 実売117,609 / 31点 / 広告50,062 → 利益+46,560 ＝ **1日23,280円**。'
  'P2(8/31-9/2・30,000) 実売170,941 / 45点 / 広告94,634 → 利益+45,842 ＝ **1日15,281円（基準の66%）**。'
  '★増分で見ると 広告 +6,514円/日 に対し 売上 −1,825円/日 で、**増額分の追加売上は検出されず**（V7）。'
  '→ **30,000 → 27,000 に戻してください**',
  '+約8,000円/日','本日'],
 ['🟡 補足','ただしインソールは日を追って回復している（次の再テストの材料）',
  'P2の日別利益は 8/31 +9,383 → 9/01 +17,017 → 9/02 **+19,442** と単調に改善。最終日は基準23,280の83%まで来ている。'
  '★またP1の基準値は2日しかなく、8/28 +4,511 と 8/29 +42,049 の平均＝ばらつきが大きい。'
  '27,000へ戻したあと **9/10に「7日利益が27,000時代の7日平均を上回るか」で再テスト**する。'
  'なお9/02の消化は38,340円で日予算30,000の128%。Metaの単日+75%ペーシングの範囲内だが、'
  '予算を触っていたら教えてください',
  '—','9/10に再判定'],
 ['🔴 判定日','バランスケアスリッパ 増額判定（10,000→12,000）→ **✅ 合格**',
  '宣言済み基準「増額後の1日あたり利益 ≥ 増額前(8/23-29)の7日平均」。'
  '増額前 1日+5,727円 → 増額後(8/31-9/2) 1日**+8,997円（+57%）**。3窓とも余裕+1.06〜1.23で健全。'
  '→ **12,000円で据え置き**。次の増額は判断材料が溜まってから',
  '—','据え置き'],
 ['🚨 非常ブレーキ','**2日連続で発動、しかも深くなっています（110,507 → 81,737円）**',
  '3日移動平均利益 **81,737円** ＝ 床130,000円の**63%**。'
  '8/31 +74,208 ／ 9/01 +92,968 ／ 9/02 **+78,035**（利益率17.8%）。'
  '★9/02は同じ水曜の8/26と比べて **実売 −19.1%（542,334→438,759）なのに広告は +9.3%（231,515→253,111）**。'
  'MERが 2.34 → **1.73** まで落ちた',
  '—','本日'],
 ['📊 原因分解','9/02の悪化はほぼ2商品で説明できる',
  '同曜日（水）で前週比: ①**ムダ毛シェーバー −61,424円**（89,344→27,920）なのに広告は +3,008円。'
  'ただし3窓は 3日+0.54 / 5日+1.02 / 7日+1.17 と健全なので**単日の谷**と読む。'
  '②**快適マジックインソール**は広告を **+19,256円** 積んで売上は **−18,507円**（89,152→70,645）。'
  'これは上の増額判定の不合格そのもので、**27,000へ戻せば止まる**。'
  '③3D足臭リセットブラシは停止済みで広告 −9,676円（効いている）',
  '—','記録'],
 ['🚨 執行','カタログ全部（テスト）**14,000 → 10,000円**（非常ブレーキ①・2日連続で未実行）',
  '9/02の消化は **18,756円** で、昨日提案した減額は入っていません。非常ブレーキが2日続いているので'
  '**今日こそ実行してください**。インソールの27,000戻しと合わせて **約12,000円/日**の圧縮になります。'
  '★9/3が期日だったカタログ3日判定は、この変更で消費されます（S1b③）',
  '+4,000円/日','本日'],
 ['⚠️ 訂正','**偏光・調光サングラスは「季節終了」ではありませんでした**',
  '9/2のレポートで「Metaが入札しなくなった＝季節終了。正式に停止を」と書きましたが、**早計でした**。'
  '消化は 8/31 151 → 9/01 124 → 9/02 **3,760円** と戻り、販売も 9/02 2点・9/03 2点。'
  '3窓は 3日 MER2.96 余裕+1.67 ／ 5日 +0.24 ／ 7日 +0.54 で**分岐割れなし**。'
  '★2日ぶんの消化で構造を断定したのが誤りです（自分で「単日・2日で決めない」と決めているのに破りました）。'
  '**停止せず、日予算5,000のまま様子見**にしてください',
  '—','訂正'],
 ['🆕 新商品','**温感EMSフェイシャルワンド**（9/02にキャンペーン新設・ユーザー実施）',
  '9/02 広告4,551円 / 1点 → 9/03 **4点・23,920円**（ピンク3・シルバー1）と立ち上がり良好。'
  '売価5,980円 / 原価 シルバー1,866・ピンク2,079 → 分岐MER 約1.45。'
  '★3窓判定は「🚨停止」と出ていますが、**これは誤検知**です（窓が3つとも同じ1日しか含んでいない）。'
  '**新商品は購入50件に達するまで判定しない**のが宣言済みルール。現在5件。**触らないでください**',
  '—','監視'],
 ['⏸ 触らない','W固定スマホ車載ホルダー（最も要注意）',
  '余裕が 7日+0.86 → 5日+0.78 → 3日**+0.40** と単調に縮んでいる。3窓とも分岐は割っていないので今日は動かさない。'
  '**1窓でも分岐割れしたら即減額対象**に上げる',
  '—','監視'],
 ['📈 速報','9/03(木)のShopifyは戻しています',
  '実売 **518,956円** / 注文103 / 販売数118 / 客単価5,038円 / セッション3,340 / カート追加率5.90%。'
  '9/02の438,759から +18.3%。ただし前週の木曜(8/27 653,692円)比では **−20.6%**。'
  '★広告費は09:00 JSTに窓が回ってから確定させます。4WAYに返品1件(4,233円)',
  '—','速報'],
 ['⚠️ 未確定','予算スナップショットが 8/30 18時から更新できていません（5日連続）',
  'このレポートの「現日予算」列と週消化率は 8/30 18時のMeta直読値（合計255,000円）が土台です。'
  'その後に ①ビジュアル耳かき停止 ②温感EMS新設 が起きているので実態と合っていません。'
  '★予算を変更していたら教えてください',
  '—','本日'],
 ['🔴 判定日','優先配送 790円',
  '宣言済み基準「14日で件数 ≥ 28件」。7日実測 **6件**（アタッチ率 6/708 = 0.85%）。**不合格の見込み**','—','9/4'],
 ['🟡 推奨','秋冬商材の仕込み',
  '夏の撤退組（サングラス以外）が抜けた穴は埋まっていません。温感EMSの立ち上がりは良い兆候。'
  'CJで あったかインソール 1,000円以下 ／ 外反母趾サポーター 800円以下 を確認してテスト枠に入れる','—','優先'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
  'data/lp/upsell-pairs-2026-08-26.txt の5組をアプリに手入力する。判定は実施+14日で異商品ミックス率3.0%以上','週+約4万円','任意'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/02(水)', R['N1'], R['K1'], R['ACCT1']), ('3日 8/31-9/02', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/27-9/02', R['N7'], R['K7'], R['ACCT7']), ('30日 8/04-9/02', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-02', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/3','快適マジックインソール 増額判定（27,000→30,000）','P2(8/31-9/2)の1日あたり利益 ≥ 23,280円',
  'P2 実売170,941 / 45点 / 広告94,634 → 利益+45,842 ＝ **1日15,281円**（基準の66%）',
  '❌ 不合格 → 30,000へ戻す',
  '増分は 広告+6,514円/日 に対し 売上−1,825円/日 で追加売上を検出できず（V7）。'
  'ただし日別は 8/31 +9,383 → 9/01 +17,017 → 9/02 +19,442 と単調改善。27,000へ戻して9/10に再テスト'],
 ['9/3','バランスケアスリッパ 増額判定（10,000→12,000）','増額後の1日あたり利益 ≥ 増額前(8/23-29)の7日平均',
  '増額前 1日+5,727円 → 増額後(8/31-9/2) 1日**+8,997円（+57%）**','✅ 合格 → 12,000で据え置き',
  '3窓とも余裕+1.06〜1.23。次の増額は判断材料が溜まってから'],
 ['9/4','**非常ブレーキ**','3日移動平均利益 < 130,000円 で発動',
  '8/31 +74,208 / 9/01 +92,968 / 9/02 +78,035 → **平均 81,737円**（床の63%）',
  '🚨 2日連続・深化 → 2本執行',
  '①カタログ 14,000→10,000（昨日から未実行）②インソール 30,000→27,000（上の判定）。合わせて約12,000円/日'],
 ['9/3','カタログ 3日判定（8/31-9/2）','CPA基準: 倍率 ≥1.00 で据え置き',
  '9/02の消化 18,756円。減額は未実行のまま','❌ 消費（実施しない）',
  '非常ブレーキ①で予算を14,000→10,000へ動かすため、S1b③により前の実験は消費される'],
 ['9/4','**偏光・調光サングラス（9/2の判断を訂正）**','「2日連続でほぼゼロ消化＝季節終了」と判断していた',
  '消化 8/31 151 → 9/01 124 → 9/02 **3,760円**で復活。販売も9/02 2点・9/03 2点。3窓とも分岐割れなし',
  '⚠️ 訂正 → 停止しない',
  '★2日ぶんの消化で構造を断定したのが誤り。自分で決めた「単日・2日で決めない」を破っていた。日予算5,000のまま様子見'],
 ['—','温感EMSフェイシャルワンド（3窓判定の誤検知）','新商品は購入50件に達するまで判定しない',
  '9/02にキャンペーン新設。累計5件（9/02 1点 → 9/03 4点）','⏸ 判定しない',
  '★判定表に「🚨停止/−50%」と出るが、窓3つが同じ1日しか含んでいないための誤検知。触らないこと'],
 ['9/4','優先配送 790円','14日で件数 ≥ 28件','7日実測 **6件**（アタッチ率 0.85%）','🚨不合格の見込み',
  '8/22に536→790へ値上げ済み'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均',
  '—','予約','P2の日別が単調改善していたため、戻したうえで7日窓で測り直す'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は2件: ①**カタログ 14,000→10,000**（非常ブレーキ①・昨日から未実行）'
  '②**快適マジックインソール 30,000→27,000**（増額判定が不合格）。合わせて約12,000円/日',
  '★3日移動平均利益 81,737円 ＝ 床130,000円の63%。2日連続で発動し、しかも深くなっている',
  '★訂正2件: サングラスは季節終了ではなかった（停止しない）／温感EMSの🚨は誤検知（新商品なので判定しない）'],
...
wb.save('data/reports/report-2026-09-04.xlsx')
