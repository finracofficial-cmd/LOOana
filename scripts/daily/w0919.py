# -*- coding: utf-8 -*-
"""[9/19終端に固定した窓ビルダー] 2026-09-21 定例（データ終端=9/19 土）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0919=220558  # 9/19 広告費（health_check 8/21-9/19 − CSV 8/21-9/18 の実測差）
# 窓の検証: 停止済み **7本すべて** で HC − CSV = 0円ちょうど
#   （ムダ毛 448,114 ／ 電動温熱カッサ 20,149 ／ 4-in-1 80,605 ／ ナノバブル 163,744 ／
#     むくみ取りかっさ 180,745 ／ 温感EMS 64,858 ／ 3D足臭 81,780）
#   ★3D足臭のHCが 85,838 → 81,780 に落ちた（8/20の4,058円が窓から抜けた）＝窓スライドの独立証拠
#   8/20始まりは7本が負値、8/22始まりは合計438,203円（日予算228,000の192%）で非現実的 → 8/21-9/19 で確定
...
HOL={'2026-07-20','2026-08-11'}  # 海の日・山の日  # 海の日・山の日
...
END='2026-09-19'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-21']
assert D1==['2026-09-19'] and D3[0]=='2026-09-17' and D7[0]=='2026-09-13', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-21', (len(D30),D30[0])
...
assert len(CUM)==19 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-19'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-19']==647460 and sum(S[n]['2026-09-19'][1] for n in S if '2026-09-19' in S[n])==27768
assert abs(ACCT(D1)-AD0919)<1, ('9/19 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0919w.pkl','wb'))
