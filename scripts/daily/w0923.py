# -*- coding: utf-8 -*-
"""[9/23終端の窓ビルダー] 2026-09-24 定例（昨日=9/23 水・秋分の日 実績）。実測のみ・毎回ゼロから積み直す。原価は9/20反映のCJ最新表（再調達原価ベース）。"""
import csv, collections, datetime, json, pickle
AD0923=267788  # キャンペーン合算（アカウント照合 差0円）※翌朝再確認。予算256,000の105%
...
END='2026-09-23'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=ALLD[-30:]
assert D1==['2026-09-23'] and D3[0]=='2026-09-21' and D7[0]=='2026-09-17', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-25', (len(D30),D30[0])
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（23日）
assert len(CUM)==23 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-23'
...
# 9/23の縦照合（全店1クエリ実測 gross 636,040円 / 値引 17,117円 / 注文128件 / 点数148）
assert GROSSD['2026-09-23']==636040 and sum(S[n]['2026-09-23'][1] for n in S if '2026-09-23' in S[n])==17117
assert abs(ACCT(D1)-AD0923)<1, ('9/23 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0923w.pkl','wb'))
