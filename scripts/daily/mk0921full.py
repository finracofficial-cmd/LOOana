# -*- coding: utf-8 -*-
"""2026-09-21 定例レポート本体（昨日=9/20 日）。原価は9/20反映のCJ最新表。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0920w.pkl', 'rb'))
...
ORD30 = {'ナノガラス脱毛パッド':1128,'W固定スマホ車載ホルダー':498,'快適マジックインソール':393,
...
MF_RAW = {'ナノガラス脱毛パッド':(2406043,1233058,32115),'W固定スマホ車載ホルダー':(981069,379855,12371),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-21（昨日 = 9/20 日・連休2日目）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-20・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-21 定例（昨日=9/20 日）',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/20(日)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/18-20', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/14-20', R['N7'], R['K7'], R['ACCT7']), ('30日 8/22-9/20', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-20', R['NC'], R['KC'], R['ACCTC'])]:
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/20(日)は実売651,240円・利益+212,721円（32.7%）と2日連続の21万円台。本日の判定はサンダル14,000が不合格（3日平均1,551円）で11,000へ戻し提案、スリッパ9,000は基準の2.1倍で合格確定。CVRは9/19 2.98%→9/20 3.13%と連休中に前倒しで3%台回復＝外部要因説の傍証（確定は9/24-25）。原価は9/20反映のCJ最新表で全期間再評価（ガラス988円でMER4.57の優等生に）。明朝9/22は復活2本のPhase1: シェーバー合格圏・歯ブラシ不合格圏').font = Font(name='Arial', size=10, color='CC0000')
...
sh('予算見直しチェック', ['前日(9/20)スナップショット → 本日(9/21)',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/22-9/20)。売上=gross−値引。本日9/21(月)更新済み・次回9/28(月)',
