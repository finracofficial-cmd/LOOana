# -*- coding: utf-8 -*-
"""[9/05終端に固定した窓ビルダー] 2026-09-07 定例（データ終端=9/05 土）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0905=218144  # 9/05 広告費（health_check 8/07-9/05 − CSV 8/07-9/04 の実測差）
...
END='2026-09-05'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-07']
assert D1==['2026-09-05'] and D3[0]=='2026-09-03' and D7[0]=='2026-08-30', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-07', (len(D30),D30[0])
...
assert len(CUM)==5 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-05'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-05']==543620 and sum(S[n]['2026-09-05'][1] for n in S if '2026-09-05' in S[n])==9155
assert abs(ACCT(D1)-AD0905)<1, ('9/05 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0905w.pkl','wb'))
