# -*- coding: utf-8 -*-
"""[9/5終端に固定した窓ビルダー] 2026-09-06 定例（昨日=9/5 土 実績）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0905=218217  # data_query 実測（9/6 17:00 確定値で上書き済み・fix0905ad.py）
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（5日）
assert len(CUM)==5 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-05'
...
# 9/5の縦照合（全店1クエリ実測 gross 543,620円 / 値引 9,155円 / 注文106件 / 点数117）
assert GROSSD['2026-09-05']==543620 and sum(S[n]['2026-09-05'][1] for n in S if '2026-09-05' in S[n])==9155
assert abs(ACCT(D1)-AD0905)<1, ('9/5 広告費 アカウント照合', ACCT(D1))
