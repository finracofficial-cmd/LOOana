# -*- coding: utf-8 -*-
"""[8/27終端に固定した窓ビルダー] 2026-08-28 定例（昨日=8/27 木 実績）。実測のみ・毎回ゼロから積み直す。"""
...
PRICEBYDATE={'優先配送':[('0000-00-00',536),('2026-08-22',790)],                       # 商品ページは590円だが実課金は536円のまま
...
# 2026-08-28: 8/27まで揃った（Σキャンペーン＝アカウントレベルで差0円）。窓は8/27終端。
END='2026-08-28'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-30']
assert D1==['2026-08-28'] and D3[0]=='2026-08-26' and D7[0]=='2026-08-22', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-30', (len(D30),D30[0])
...
assert len(CUM)==28 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-28'
...
assert GROSSD['2026-08-28']==431890 and sum(S[n]['2026-08-28'][1] for n in S if '2026-08-28' in S[n])==8359
assert abs(ACCT(D1)-245288)<1, ('8/27 広告費 アカウント照合', ACCT(D1))
