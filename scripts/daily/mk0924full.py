# -*- coding: utf-8 -*-
"""2026-09-24 定例レポート本体（昨日=9/23 水・秋分の日）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0923w.pkl', 'rb'))
...
ORD = {'ナノガラス脱毛パッド':262,'伸縮ガラスクリーナー':159,'W固定スマホ車載ホルダー':134,
...
ORD30 = {'ナノガラス脱毛パッド':1131,'W固定スマホ車載ホルダー':504,'快適マジックインソール':400,
...
MF_RAW = {'ナノガラス脱毛パッド':(2409207,1238602,33035),'W固定スマホ車載ホルダー':(981687,372576,12611),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-24（昨日 = 9/23 水・秋分の日＝連休最終日）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-23・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-24 定例（昨日=9/23 水・秋分の日）',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/23(水祝)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/21-23', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/17-23', R['N7'], R['K7'], R['ACCT7']), ('30日 8/25-9/23', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-23', R['NC'], R['KC'], R['ACCTC'])]:
...
ws.cell(r, 1, '■ 昨日(9/23 水祝) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','9/23(水祝)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/23(水・秋分)は実売618,923円・利益+191,612円（31.0%）。連休5日の合計利益は+1,097,733円と百万円超えで、体制再編（停止4本・復活2本・増額3本）が完全に機能した連休だった。CVRは3.17%で5日連続3%台。明朝9/25の判定5本はガラス・フックが合格圏、インソールが僅かに未達圏、サンダルは8,000へ戻し濃厚。歯ブラシは9/23に5個・CPA2,094円と月間最高日を出し7日判定が競り合いに復帰。本日9/24(木)が平日初日＝答え合わせ本番').font = Font(name='Arial', size=10, color='CC0000')
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/25-9/23)」。他シートの7日窓と混ぜて読まないこと',
...
for r in body:
    if r[0] == '2026-09-23': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
...
sh('予算見直しチェック', ['前日(9/23)スナップショット → 本日(9/24)',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/25-9/23)。売上=gross−値引。次回定期更新9/28(月)',
...
wb.save('data/reports/report-2026-09-24.xlsx')
