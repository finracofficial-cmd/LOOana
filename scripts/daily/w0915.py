# -*- coding: utf-8 -*-
"""[9/15終端に固定した窓ビルダー] 2026-09-17 定例（データ終端=9/15 火）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0915=235587  # 9/15 広告費（health_check 8/17-9/15 − CSV 8/17-9/14 の実測差）
# 窓の検証: 停止済み5本すべてで HC − CSV = 0円ちょうど
#   （3D足臭 85,838 ／ むくみ取りかっさ 180,745 ／ 温感EMS 64,858 ／ ナノバブル 163,744 ／ 4-in-1 80,605）
#   8/16始まりは7本が負値、8/18始まりは合計405,486円（日予算252,000の161%）で非現実的 → 8/17-9/15 で確定
...
END='2026-09-15'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-17']
assert D1==['2026-09-15'] and D3[0]=='2026-09-13' and D7[0]=='2026-09-09', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-17', (len(D30),D30[0])
...
assert len(CUM)==15 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-15'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-15']==542580 and sum(S[n]['2026-09-15'][1] for n in S if '2026-09-15' in S[n])==20897
assert abs(ACCT(D1)-AD0915)<1, ('9/15 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0915w.pkl','wb'))
