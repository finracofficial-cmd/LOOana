# -*- coding: utf-8 -*-
"""2026-09-13 定例レポート本体（データ終端=9/11 金／9/12の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0911w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-13'   # ★Meta 直読。ナノバブルシャワーヘッドが PAUSED（−14,000）
...
'高見えレザーヘッドレストフック':13,'2WAYシートボックス':11,'温感EMSフェイシャルワンド':10,'優先配送':5,
'携帯電動シェーバー':4,'偏光・調光サングラス':4,'壁掛けディスペンサー':3,'完全遮光・形状記憶':2,
'形状記憶日傘':2,'ジェットウォッシャー':2,'湯上がりガーゼワンピース':1,'3D足臭リセットブラシ':1,
'UV歯ブラシ除菌器':1,'5WAY腰掛けファン':1,'スマートノーズEMS美顔器':1,'ビジュアル耳かき':1,
'卓上冷感クーラー':1,'リカバリーサンダル':1}
STORE_ORD7, STORE_SESS7 = 729, 21824
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 741, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1129,'W固定スマホ車載ホルダー':509,'快適マジックインソール':319,'ムダ毛シェーバー':184,
'バランスケアスリッパ':121,'2WAYシートボックス':115,'姿勢サポートチェア':101,'形状記憶日傘':98,
'偏光・調光サングラス':83,'むくみ取りかっさ':78,'完全遮光・接触冷感UVハット':71,'伸縮ガラスクリーナー':52,
'ナノバブルシャワーヘッド':49,'4WAY':42,'優先配送':42,'接触冷感UVパーカー':37,'害虫ブロッカー':31,
'完全遮光・形状記憶':29,'接触冷感UVアームカバー':28,'3D足臭リセットブラシ':22,'4-in-1マルチクリーナー':20,
'携帯電動シェーバー':18,'温感EMSフェイシャルワンド':16,'高見えレザーヘッドレストフック':13,
'5WAY腰掛けファン':7,'壁掛けディスペンサー':6,'ビジュアル耳かき':6,'ジェットウォッシャー':4,
'ヘアドライタオル':4,'健康サンダル':3,'湯上がりガーゼワンピース':2,'癒しの指圧マット':2,'瞬間冷感ポンチョ':2,
'スマートノーズEMS美顔器':1,'リカバリーサンダル':1,'1秒折り畳みチェア':1,'卓上冷感クーラー':1,
'ネックマッサージャー':1,'姿勢サポートベルト':1,'UV歯ブラシ除菌器':1}
...
# ★2026-09-13 取得の health_check（窓 = 8/13-9/11 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2396777,1256646,37778),'W固定スマホ車載ホルダー':(1002306,451807,16218),
'快適マジックインソール':(690371,423811,11615),'カタログ全部（テスト）':(562020,141394,5477),
'ムダ毛シェーバー':(538298,432155,6223),'姿勢サポートチェア':(319625,188056,4558),
'バランスケアスリッパ':(287931,197186,4449),'ナノバブルシャワーヘッド':(165207,78813,2522),
'4-in-1マルチクリーナー':(64594,18480,572),'伸縮ガラスクリーナー':(58514,18907,989),
'高見えレザーヘッドレストフック':(24728,5892,295)}
...
sh('シンプル判定', ['LOOTY 2026-09-13 定例（データ終端=9/11 金）',
...
NA = [
 ['🔴 判定日','伸縮ガラスクリーナー → **✅ 両方合格。16,000 → 20,000円（+25%）**',
  '昨日1本化すると予告した2つの判定が、どちらも通りました。'
  '**判定①（50点到達時）**: 7日(9/05-11) 実売254,123 / 67点 / 広告58,514 → 利益 **+120,294円**。'
  'MER **4.34** ≥ 目標MER 2.83（原価率29.6%）✅ ／ 16,000での消化率（9/09-11の3日）**97%** ≥95% ✅。'
  '**判定②（8,000→16,000の効果）**: 8,000時代(9/07-08) 1日 +15,270円 → '
  '16,000時代(9/09-11) 1日 **+29,918円 ＝ 基準の196%** ✅。'
  '★**倍増して、1日あたり利益もほぼ倍**になりました。増額が素直に効いています',
  '+約7,000円/日','本日'],
 ['📊 その中身','9/11は消化を減らして利益が最大になりました',
  '9/07 広告2,086円→3点（MER5.72）／ 9/08 9,703円→13点（4.98）／ 9/09 14,470円→13点（3.52）／'
  '9/10 17,558円→17点（3.72）／ **9/11 14,697円→21点（MER5.28・利益+39,286円）**。'
  '★9/11は消化が前日より2,861円少ないのに、点数は17→21点。'
  '**まだ余力がある**ので、20,000へ上げる判断を支持します',
  '—','記録'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（**12日連続で未実行**）',
  'Meta直読で日予算は今も18,000円。9/11の消化は13,013円。30日で **562,020円**。'
  '★伸縮ガラスクリーナーを20,000へ上げる原資（+4,000円/日）は、ここから出せば'
  '**全体の日予算を増やさずに済みます**（カタログ −8,000 / 伸縮 +4,000 で差し引き −4,000円/日）',
  '+8,000円/日','本日'],
 ['⚠️ 確認','ナノバブルシャワーヘッドが **停止**されていました',
  'Meta直読で PAUSED。9/11に12,746円で稼働したのが最後です。'
  '★8,000→14,000へ増額した直後の停止で、私からは減額も停止も出していません。'
  '直前の3窓は **+0.08 / +0.30 / +0.44** と黒字圏ではありましたが、'
  '3日窓が+0.08まで細っていたので**判断としては理解できます**。'
  '日予算14,000円が空きました',
  '+14,000円/日','記録'],
 ['🎉 好調','高見えレザーヘッドレストフックが 3窓とも MER3.37・余裕+1.80',
  '9/09 3点 → 9/10 6点 → 9/11 **14点** → 9/12 5点。累計28点。'
  '★**伸縮ガラスクリーナーに次ぐ立ち上がり**です。'
  '9/11の消化は9,821円＝日予算8,000円の**123%**で、すでに予算が天井。'
  '**購入50件まで判定しません**が、50点は9/15前後の見込みです',
  '—','50点で判定'],
 ['📈 好転','3日移動平均利益が5日連続で床の上、しかも改善（140,071 → **148,097円**）',
  '9/09 +152,825 ／ 9/10 +149,905 ／ 9/11 **+141,561（利益率28.0%）**。'
  '★**3日とも利益率が26〜28%**と安定しています。9/01-04の谷（1日平均76,969円）とは別の局面です。'
  '9月の当月累積（11日）は 実売5,500,163 / 利益 **1,340,089円（24.4%）**／1日あたり121,826円',
  '—','記録'],
 ['🚫 除外','快適マジックインソールが4日連続で「🚀増額候補」ですが、**9/22まで除外**のままです',
  '3窓とも目標超え（余裕 3日+1.21 / 5日+1.16 / 7日+1.21）で、数字は毎日強くなっています。'
  '★ですが8/30の 27,000→30,000 で追加売上を検出できなかった事実は変わりません。'
  '**昨日切った期日（9/22＝前回実験から3週間）を動かしません**。'
  '基準を都合よく前倒しすると、判定の意味がなくなります',
  '—','9/22まで除外'],
 ['✅ 確認','停止済み5本は今朝も⚠️のままで、判断は正しかった',
  '**むくみ取りかっさ** 3窓とも余裕<0.30（−0.29/+0.10/+0.20）⚠️｜'
  '**2WAYシートボックス** 3窓とも余裕<0.30（−0.28/+0.09/−0.16）⚠️｜'
  '温感EMS・UVハット・サングラスは7日窓の消化がほぼ消えてデータ不足へ。'
  '★9/11の残消化はゼロ。5本とも完全に止まっています',
  '—','完了'],
 ['⏸ 監視','ムダ毛シェーバーの3日窓が分岐割れ（MER0.72・余裕−0.95）',
  '9/12が **2点13,960円**（うち1点は返品）と落ちたのが効いています。5日 −0.01 ／ 7日 **+0.51**。'
  '★3窓ルールでは据え置き（7日が+0.51）。**単日・2日で決めない**原則どおり今日は動かしません。'
  'ただし3日窓がここまで割れたのは久しぶりなので、明日の数字を注意して見ます',
  '—','監視'],
 ['📈 速報','9/12(土)は実売 **568,890円**・販売数140点',
  '注文117 / 客単価4,862円 / セッション3,132 / **カート追加率 6.77%**（直近で最高）。'
  '前週の土曜(9/05 534,465円)比 **+6.4%**。'
  '★**伸縮ガラスクリーナーが32点127,360円**で過去最高。累計99点で、明日にも100点に届きます。'
  '返品3点14,940円（ムダ毛・害虫・サングラス各1）',
  '—','速報'],
 ['🟡 推奨','空いた日予算の使い道（ナノバブル14,000円ぶん）',
  'ナノバブル停止で14,000円が空きました。'
  '**伸縮ガラスクリーナー +4,000（本日の増額）／ 高見えレザーヘッドレストフックが50点を通れば +2,000**で、'
  'まだ8,000円残ります。**クリーナー系・車用品系の新商品テスト**に回すのが最有力です','—','優先'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/11(金)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/09-9/11', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/05-9/11', R['N7'], R['K7'], R['ACCT7']), ('30日 8/13-9/11', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-11', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/13','伸縮ガラスクリーナー **増額判定（2つを1本化）**',
  '①50点到達時: 7日MER ≥ 目標MER かつ 消化率 ≥95% ／ ②16,000時代の1日あたり利益 ≥ 8,000時代の +15,270円',
  '①7日 実売254,123 / 67点 / 広告58,514 → 利益+120,294円・MER **4.34** ≥ 目標2.83 ✅／消化率 **97%** ✅。'
  '②8,000時代 1日+15,270円 → 16,000時代 1日 **+29,918円（196%）** ✅',
  '✅ **両方合格 → 16,000を20,000へ（+25%）**',
  '★倍増して1日あたり利益もほぼ倍。9/11は消化を2,861円減らして点数17→21点と、まだ余力がある'],
 ['9/13','カタログ全部（テスト）','非常ブレーキ由来ではなく単体の効率問題',
  'Meta直読で日予算18,000のまま。9/11の消化13,013円。30日で562,020円',
  '🚨 18,000→10,000（**12日連続で未実行**）',
  '★伸縮の+4,000円の原資はここから出せる。カタログ−8,000/伸縮+4,000で差し引き−4,000円/日'],
 ['9/13','ナノバブルシャワーヘッド','ユーザーが停止（私からは出していない）',
  'Meta直読で PAUSED。9/11に12,746円で稼働したのが最後。直前の3窓は +0.08 / +0.30 / +0.44',
  '⏹ 停止を記録',
  '★8,000→14,000の増額直後の停止。3日窓が+0.08まで細っていたので判断としては理解できる。14,000円が空いた'],
 ['9/13','快適マジックインソール（4日連続で増額候補だが除外）','3窓とも目標超え（+1.16〜+1.21）',
  '数字は毎日強くなっているが、8/30の 27,000→30,000 で追加売上を検出できなかった事実は変わらない',
  '🚫 9/22まで除外（期日を動かさない）',
  '★基準を都合よく前倒しすると判定の意味がなくなる'],
 ['50点','高見えレザーヘッドレストフック','購入50件に達するまで判定しない',
  '累計28点（9/09 3 → 9/10 6 → 9/11 **14** → 9/12 5）。3窓とも MER3.37 vs 分岐1.57・**余裕+1.80**。'
  '9/11の消化9,821円＝日予算8,000の**123%**で既に天井','⏸ 50点は9/15前後の見込み',
  '伸縮ガラスクリーナーに次ぐ立ち上がり'],
 ['50点','4-in-1マルチクリーナー','20点判定の基準を持ち越し: 余裕が+0.30未満なら −25%',
  '累計20点。3窓は 3日+1.14 / 5日+0.55 / 7日+0.56 と改善が続く','⏸ あと30点','—'],
 ['9/13','停止済み5本の事後確認','むくみ取りかっさ／2WAY／温感EMS／UVハット／サングラス',
  'かっさ 3窓とも余裕<0.30（⚠️）／2WAY 3窓とも余裕<0.30（⚠️）／残り3本は7日消化がほぼ消えてデータ不足へ',
  '✅ 停止は正しかった','9/11の残消化はゼロ。5本とも完全に止まっている'],
 ['—','ムダ毛シェーバー（監視）','3窓すべて 余裕<0.30 で −25%',
  '3日 **−0.95**（MER0.72・9/12が2点で1点は返品）／ 5日 −0.01 ／ **7日 +0.51**','⏸ 据え置き（7日が黒字）',
  '★3日窓がここまで割れたのは久しぶり。単日・2日で決めないので今日は動かさず、明日の数字を見る'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の判定: **伸縮ガラスクリーナーは2つとも合格 → 16,000 を 20,000 へ（+25%）**',
  '　①7日MER **4.34** ≥ 目標2.83／消化率97% ②16,000時代の1日あたり利益 **+29,918円 ＝ 基準の196%**',
  '★もう1件の執行: **カタログ 18,000→10,000**（12日連続で未実行）。'
  '伸縮の+4,000円の原資はここから出せば、全体の日予算は増えません',
  '★ナノバブルシャワーヘッドが停止されていました（私からは出していません）。日予算14,000円が空きます',
  '★3日移動平均利益は5日連続で床の上、しかも改善して **148,097円**。利益率も3日連続で26〜28%'],
...
for r in body:
    if r[0] == '2026-09-12': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
