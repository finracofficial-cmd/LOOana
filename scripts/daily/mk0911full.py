...
# --- 7日(9/04-9/10) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD = {'ナノガラス脱毛パッド':270,'W固定スマホ車載ホルダー':102,'快適マジックインソール':87,
'バランスケアスリッパ':41,'ムダ毛シェーバー':38,'伸縮ガラスクリーナー':38,'姿勢サポートチェア':23,
'ナノバブルシャワーヘッド':21,'むくみ取りかっさ':20,'4-in-1マルチクリーナー':18,'2WAYシートボックス':13,
'温感EMSフェイシャルワンド':12,'高見えレザーヘッドレストフック':6,'優先配送':4,'携帯電動シェーバー':4,
'壁掛けディスペンサー':3,'偏光・調光サングラス':3,'完全遮光・形状記憶':2,'形状記憶日傘':2,'ジェットウォッシャー':2,
'湯上がりガーゼワンピース':1,'3D足臭リセットブラシ':1,'UV歯ブラシ除菌器':1,'5WAY腰掛けファン':1,
'スマートノーズEMS美顔器':1,'ビジュアル耳かき':1,'癒しの指圧マット':1,'完全遮光・接触冷感UVハット':1,'卓上冷感クーラー':1}
STORE_ORD7, STORE_SESS7 = 707, 22103
assert sum(ORD.values()) == 718, sum(ORD.values())   # 全店707より多いのは複数商品を含む注文が両方で数えられるため

# --- 30日(8/12-9/10) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1126,'W固定スマホ車載ホルダー':507,'快適マジックインソール':315,
'ムダ毛シェーバー':185,'2WAYシートボックス':125,'バランスケアスリッパ':121,'形状記憶日傘':105,
'姿勢サポートチェア':104,'偏光・調光サングラス':86,'むくみ取りかっさ':77,'完全遮光・接触冷感UVハット':74,
'ナノバブルシャワーヘッド':48,'接触冷感UVパーカー':47,'4WAY':45,'優先配送':44,'害虫ブロッカー':43,
'伸縮ガラスクリーナー':38,'完全遮光・形状記憶':32,'接触冷感UVアームカバー':30,'3D足臭リセットブラシ':22,
'携帯電動シェーバー':19,'4-in-1マルチクリーナー':18,'温感EMSフェイシャルワンド':16,'5WAY腰掛けファン':10,
'ビジュアル耳かき':6,'壁掛けディスペンサー':6,'高見えレザーヘッドレストフック':6,'ヘアドライタオル':4,
'ジェットウォッシャー':4,'卓上冷感クーラー':3,'健康サンダル':3,'癒しの指圧マット':2,'瞬間冷感ポンチョ':2,
'湯上がりガーゼワンピース':2,'ネックマッサージャー':1,'3WAYサーキュレーター':1,'UV歯ブラシ除菌器':1,
'スマートノーズEMS美顔器':1,'姿勢サポートベルト':1,'1秒折り畳みチェア':1}

# --- 30日(8/12-9/10) Metaファネル（data_query 実測・Asia/Tokyo・アウトバウンドクリック）---
MF_RAW = {'ナノガラス脱毛パッド':(2396011,1265695,32061),'W固定スマホ車載ホルダー':(1008446,466508,13478),
'快適マジックインソール':(669541,408350,9949),'カタログ全部（テスト）':(569330,143214,5014),
'ムダ毛シェーバー':(542891,436108,5039),'形状記憶日傘':(415782,106652,4125),
'2WAYシートボックス':(342064,109922,3020),'姿勢サポートチェア':(319493,192181,3950),
'バランスケアスリッパ':(279747,192640,3589),'偏光・調光サングラス':(249368,106251,2867),
'完全遮光・接触冷感UVハット':(221591,50120,2356),'むくみ取りかっさ':(180745,52807,1584),
'接触冷感UVパーカー':(165126,52778,1498),'ナノバブルシャワーヘッド':(152455,72553,1776),
'害虫ブロッカー':(132818,61908,1190),'4WAY 取り付けOK 小型瞬間冷却ハンディファン':(112907,28857,855),
'完全遮光・形状記憶・晴雨兼用・UV日傘':(92289,21160,900),'3D足臭リセットブラシ':(85838,24927,338),
'接触冷感UVアームカバー':(85706,19336,681),'温感EMSフェイシャルワンド':(64858,15636,456),
'4-in-1マルチクリーナー':(56492,15984,383),'5WAY腰掛けファン':(54222,16208,681),
'伸縮ガラスクリーナー':(43779,13774,528),'ビジュアル耳かき':(26464,9402,148),
'高見えレザーヘッドレストフック':(14898,3442,140),'高吸水・速乾ヘアドライタオル':(10669,2706,75),
'卓上冷感クーラー':(10608,5095,154),'カタログ全部（テスト） 夏以外':(6080,1903,50),
'1秒折り畳みチェア':(5704,1773,49),'携帯電動シェーバー':(4532,1433,55)}
...
COSTa['ナノガラス脱毛パッド'] = (908 * 757 + 318 * 740) / 1226          # 30日実測ミックスの加重平均【近似】
...
sh('収益', [f'LOOTY 収益レポート 2026-09-11（昨日 = 9/10 木）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-10・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
  '★9/10(木)は 実売 **578,136円（9月最高）**・利益 +149,109円（25.8%）・MER 2.04。前週木曜(9/3 実売518,956円・MER1.96)比 **実売+11.4%**',
  '★主役は**ナノガラス: 53個・実売207,756円・利益+73,004円・MER2.19・倍率1.77**＝9月最高日。消化94,988円（予算80,000円の119%）は'
  'Metaの週内ペーシングが強い日へ寄せた仕様内の上振れ。**「今日のパフォーマンスで調整」の答え: 減額の根拠なし**',
  f'✅ 注文116件・セッション3,514（9/6以来の水準）・CVR 3.30%。3日移動平均利益 {p3ma:,.0f}円',
  '✅ リバランス初日（9/10・非クリーン日の記録）: インソール32,000→**+41,480円**／ガラス16,000→**+28,321円**／W固定33,000→+17,639円と増額組が快走。'
  '一方 **ナノバブル14,000→−11,427円**（1個）と**ムダ毛−15,684円**（1個・据え置き組の単日事故）が足を引いた',
  '🔧 訂正: 昨日のレポートの「9/9は前週水曜(9/3)比+4.3%」は誤り（9/3は木曜）。正しくは**前週水曜(9/2 実売438,759円)比+23.4%**＝9/9はさらに良い日だった',
  '⚠️ 楽天スーパーセールは本日9/11 01:59に終了。9/12-14で反発（セール吸引の有無）を確認する',
...
sh('シンプル判定', ['LOOTY 2026-09-11 定例（昨日=9/10 木）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/10終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
# ---------- 2 ネクストアクション ----------
NA = [
 ['📋 本日の執行','**なし**（9/11朝に判定日の商品はない）',
  '9/10のリバランス（増額5本・停止2本・カタログ14,000）の初日は非クリーン日として記録のみ。'
  '判定は9/12朝（3件）→9/14朝（増額5本）→9/18朝（カタログ）の順で消化する',
  '—','—'],
 ['✅ 回答','ナノガラス「今日のパフォーマンスで調整」への終日データ',
  '9/10終日: **53個・実売207,756円・利益+73,004円・MER2.19・倍率1.77 ＝ 9月最高日**。'
  '3日倍率1.40／7日倍率1.61。消化94,988円（予算80,000円の119%）はMetaが強い日に寄せた仕様内の上振れ。'
  '**減額の根拠はどの窓にもない。現状維持を推奨**（増額したい場合も9/14朝の増額組判定を見てからが安全）',
  '—','ユーザー判断'],
 ['🚀 記録','リバランス初日（9/10・非クリーン日）の増額組',
  'インソール32,000: 26個・**+41,480円**・MER2.61 ／ ガラス16,000: 17個・**+28,321円**・MER3.73 ／ '
  'W固定33,000: 19個・+17,639円・MER2.05 ／ フック10,000(9/10中にさらに増額): 6個・+3,259円・MER2.03。'
  '4本とも初日から利益。正式判定は9/14朝（クリーン3日 9/11-13）',
  '—','9/14朝'],
 ['⚠️ 記録','ナノバブル 増額2日目で単日大負け',
  '9/10: 1個・実売6,980円・広告16,152円・**−11,427円**（MER0.43）。9/9の+21,714円から一転。'
  '11,000→14,000の増額が2日目で裏目に見えるが、単日では動かさない。9/14朝の判定（3日倍率≥1.3）で機械的に決める',
  '—','9/14朝'],
 ['⚠️ 記録','ムダ毛シェーバー 単日−15,684円（据え置き組の事故）',
  '9/10: 1個・広告19,866円（予算17,000円の117%消化）・MER0.35。'
  '7日では38個・**+37,650円**・MER2.19と健全なので単日では動かさない。明日も1個以下なら3日窓で点検',
  '—','監視'],
 ['👀 明日9/12朝','3件の判定',
  '①ヘッドレストフックPhase1（3日9/9-11・CTR≥1.5%かつCPA≤2,636円。現状2日で9個と好調） '
  '②4-in-1 Phase2本判定（7日9/5-11・分岐CPA5,206円。9/10も6個・+11,847円で合格圏） '
  '③チェア再点検（7日クリックCVR≥2.13%。9/10も6個・+14,628円と続伸中）＋暦週ペーシング確認（参考値）',
  '—','9/12朝'],
 ['🔧 訂正','前週比の曜日取り違え（9/10レポート）',
  '「9/9は前週水曜(9/3)比+4.3%」→ 9/3は木曜。正しくは前週水曜9/2(438,759円)比 **+23.4%**。'
  '今後は曜日をdatetimeで機械判定してから前週比を書く',
  '—','完了'],
 ['🔴 明朝','9/10広告費 283,011円の再取得・確認',
  '9/11 06:1x 取得の暫定値（adset合算=アカウント差0円）。9/9は 239,663→**240,454**（+791円）で確定済み・上書き実施',
  '—','明朝'],
 ...
 ['🟡 推奨','秋テスト第2波の準備（複数個買う商品を優先）',
  'ガラスクリーナー（4日で38注文・数量割引6回）とフック（2日で9個・2セット割引発生）が基準の有効性を実証中。'
  '次弾: ◎あったかインソール／着圧ソックス2-3足／衣類圧縮袋セット（9/15前後）',
  '当たり1本=週6万円〜','今週'],
 ['🟡 提案','非常ブレーキ閾値の再校正（3日移動平均 130,000 → 100,000円）',
  '承認待ちが継続中',
  '—','ユーザー判断'],
 ['📅 セール明け','楽天スーパーセール 9/11 01:59 終了（本日未明）→ 9/12-14 反発確認',
  '9/5〜9/10は利益率23.8〜33.5%で推移。セールによる悪化は最後まで観測されず。'
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/10(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/8-10', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/04-10', R['N7'], R['K7'], R['ACCT7']), ('30日 8/12-9/10', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-10', R['NC'], R['KC'], R['ACCTC'])]:
...
sh('全体サマリー', ['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '縦照合3本すべて通過: Σ商品gross=全店598,990 ／ Σ値引=20,854 ／ Σ広告費=アカウント283,011円（9/10・広告セット合算・差0円）',
 '✅ 9/9の広告費は再取得で確定済み（239,663円 → **240,454円**・+791円。9/11 06:1xに上書き。adset合算=アカウント差0円）',
 '⚠️ 9/10の広告費は 9/11 06:1x の取得【暫定】。確定まで+0.1%程度動く → 明朝に再取得して確認する',
...
ws.cell(r, 1, '■ 昨日(9/10 木) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','9/10(木)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/10(木)は実売578,136円で9月最高。ナノガラス53個・+73,004円が牽引し、リバランス増額組（インソール・ガラス・W固定）も初日から利益を積んだ。CVR3.30%は3日連続更新の後の一服だがセッション3,514で量が伸びた。負け組はナノバブル−11,427円とムダ毛−15,684円の単日2件').font = Font(name='Arial', size=10, color='CC0000')
...
sh('商品別', ['商品別（7日 9/04-9/10 = 判定単位）。カタログ行の広告費は「7日合計」（前日ではない）',
  ...
  '★伸縮ガラスクリーナーは9/7出稿（稼働4日）・高見えレザーヘッドレストフックは9/9昼出稿（稼働1.5日）。7日ベースで他商品と単純比較しないこと',
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/11','（本日の判定なし）','9/10リバランスの翌日。増額組はクリーン日1日目',
  '—','—','—'],
 ['9/12','高見えレザーヘッドレストフック Phase1（3日 9/9-11）','リンクCTR ≥1.5% かつ CPA(個) ≤2,636円',
  '2日累計9個。9/10: 6個・+3,259円・MER2.03・消化10,965円','待機（合格圏）','不合格なら停止。予算は9/10中に8,000→10,000へ増額済み'],
 ['9/12','4-in-1マルチクリーナー Phase2本判定（7日 9/5-11）','CPA(個)で判定。分岐5,206円超は停止',
  '6日累計20個。9/10は6個・+11,847円・MER4.24','待機（合格圏）',''],
 ['9/12','姿勢サポートチェア再点検','**7日**クリックCVR ≥2.13%（基準週3.55%比−40%ライン）',
  '9/10も6個・+14,628円・MER3.77と続伸','待機','窓は7日と明記済み'],
 ['9/12','Meta週内ペーシングの収束確認（参考値）','暦週(9/6-9/12)の合計消化÷(日予算×7) が90〜105%か',
  '9/10は283,011円（予算250,000円の113%）。リバランス直後のため参考値扱い','待機',''],
 ['9/12-14','セール明けの反発確認','同曜日比が平常圏へ戻るか',
  'セールは9/11 01:59終了。期間中の悪化は観測されず','待機',''],
 ['9/14','増額5本の判定（クリーン3日 9/11-13）','インソール32,000: 3日平均利益≥24,134円/日（割れたら27,000）／'
  'W固定33,000: ≥25,106円/日（割れたら30,000）／ナノバブル14,000: 3日倍率≥1.3（割れたら11,000）／'
  'ガラス16,000: 3日倍率≥1.3かつCPA≤2,837円（割れたら12,000）／フック10,000: Phase1通過後、3日倍率≥1.3',
  '初日(9/10・非クリーン): インソール+41,480／ガラス+28,321／W固定+17,639／フック+3,259／ナノバブル−11,427','待機',''],
 ['9/14(月)','曜日指数の定期更新・unitCost週次再取得','毎週月曜タスク','','待機',''],
 ['9/18','カタログ 14,000の判定（クリーン7日 9/11-17）','7日Meta-ROAS ≥2.0で継続／<2.0なら9,000へ追加減額',
  '変更は9/10（当初9/12予定を前倒し）','待機',''],
 ['10/7','購入後クーポン（LOOTY500）の判定','30日リピート率が 6.38% を超えていれば継続',
  'Shopify Flow設定はユーザー実行待ち','待機',''],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行・提案: **なし**。9/10リバランス後の体制は 250,000円/日・12セット（フックは9/10中に8,000→10,000）',
  '★次の判定: 9/12朝3件（フックPhase1・4-in-1本判定・チェア再点検）→ 9/14朝 増額5本 → 9/18朝 カタログ',
  '★倍率の定義: **MER ÷ 分岐MER**。1.0未満＝赤字',
  '★楽天スーパーセールは本日9/11 01:59に終了。9/12-14で反発を確認する'],
 ['判定日','対象','宣言済み基準','実測','結果','備考'], JUDGE, [10,26,34,34,16,46])

# ---------- 9a 予算見直しチェック（9/1 → 9/2 の差分）----------
PREVB = collections.defaultdict(int)
for r in body:
    if r[0] == '2026-09-10c': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
...
sh('予算見直しチェック', ['前日(9/10cリバランス後)スナップショット → 本日(9/11・Meta直読み確認)',
  '合計 248,000 → **250,000円/日**。9/10c以降の変更: **フック8,000→10,000**（9/10中・2日目の好調を受けた増額とみられる）',
  '★9/10は大リバランス日: 増額5本（インソール32,000/W固定33,000/ナノバブル14,000/ガラス16,000/フック10,000）・停止2本（かっさ/2WAY）・カタログ14,000 → **250,000円/日**',
  '★9/10の消化は 283,011円＝予算の113%。リバランス直後＋木曜（強い曜日）にMetaが寄せた。単日上振れは仕様内（暦週で収束を見る）',
...
'2026-08-28':3149,'2026-08-29':3537,'2026-08-30':3310,'2026-08-31':2761,'2026-09-01':2936,'2026-09-02':3142,'2026-09-03':3340,'2026-09-04':3293,'2026-09-05':3217,'2026-09-06':3752,'2026-09-07':2787,'2026-09-08':2739,'2026-09-09':2801,'2026-09-10':3514}
...
'2026-08-28':85,'2026-08-29':115,'2026-08-30':114,'2026-08-31':84,'2026-09-01':88,'2026-09-02':92,'2026-09-03':103,'2026-09-04':74,'2026-09-05':106,'2026-09-06':122,'2026-09-07':96,'2026-09-08':95,'2026-09-09':98,'2026-09-10':116}
CV = [[d, WDJ[datetime.date.fromisoformat(d).weekday()], SES[d], ORDD[d], round(ORDD[d]/SES[d], 4),
       ('★昨日（CVR 3.30%・セッション3,514は9/6以来の水準）' if d == '2026-09-10' else '（CVR 3.50%・9月最高）' if d == '2026-09-09'
...
sh('CVR推移', ['全店CVR = Shopify注文 ÷ Shopifyセッション。商品別レポートのCVR（注文÷Metaクリック）とは分母が違うので混ぜないこと',
  '★9/10は セッション3,514・注文116件 = **CVR 3.30%**。3日連続更新は止まったが、セッション+25%で注文数は9月最多の116件',
  '★チェックアウト完了率: 9/9 58.5% → 9/10 **57.1%**（108/189）。カゴ投入率 5.55%（195/3,514）は平常圏',
  '★点数/注文 1.216（141点/116注文）。値引20,854円と2日連続でまとめ買いが太い',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/12-9/10)。売上=gross−値引。次回の定期更新は 9/14(月)',
  '★同曜日平均との比較は当面使わない（8月上旬の高予算期が窓に混ざるため）。「前週同曜日比」で読む',
  '★9/10(木)は前週木曜(9/3 518,956円)比 **+11.4%**（木曜は指数110.1の強い曜日）。'
  '訂正: 昨日の「9/9は前週水曜(9/3)比+4.3%」は曜日の取り違え。正しくは前週水曜9/2(438,759円)比 **+23.4%**',
  f'★{_hi[0]}曜が最強({_hi[1]:.1f})・{_lo[0]}曜が最弱({_lo[1]:.1f})。中位の曜日は窓次第で入れ替わるので差を根拠に判断しない'],
 ['曜日','指数(全体=100)'], [[k, round(v, 1)] for k, v in _ix], [10, 16])

wb.save('data/reports/report-2026-09-11.xlsx')
