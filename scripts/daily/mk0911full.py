# -*- coding: utf-8 -*-
"""2026-09-11 定例レポート本体（データ終端=9/09 水／9/10の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0909w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-11'   # ★Meta 直読。伸縮ガラスクリーナー 8,000→16,000／ナノバブル 8,000→14,000 をユーザーが増額
...
'優先配送':5,'携帯電動シェーバー':5,'偏光・調光サングラス':5,'壁掛けディスペンサー':4,'完全遮光・形状記憶':2,
'形状記憶日傘':2,'高見えレザーヘッドレストフック':2,'ジェットウォッシャー':2,'湯上がりガーゼワンピース':1,
'3D足臭リセットブラシ':1,'スマートノーズEMS美顔器':1,'UV歯ブラシ除菌器':1,'5WAY腰掛けファン':1,
'ビジュアル耳かき':1,'ヘアドライタオル':1,'癒しの指圧マット':1,'完全遮光・接触冷感UVハット':1,'卓上冷感クーラー':1}
STORE_ORD7, STORE_SESS7 = 694, 21929
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 707, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1134,'W固定スマホ車載ホルダー':509,'快適マジックインソール':299,'ムダ毛シェーバー':193,
'2WAYシートボックス':130,'形状記憶日傘':119,'バランスケアスリッパ':117,'姿勢サポートチェア':101,
'偏光・調光サングラス':97,'完全遮光・接触冷感UVハット':79,'むくみ取りかっさ':77,'接触冷感UVパーカー':65,
'4WAY':53,'害虫ブロッカー':53,'ナノバブルシャワーヘッド':47,'優先配送':44,'完全遮光・形状記憶':39,
'接触冷感UVアームカバー':32,'伸縮ガラスクリーナー':24,'3D足臭リセットブラシ':22,'携帯電動シェーバー':21,
'温感EMSフェイシャルワンド':16,'5WAY腰掛けファン':15,'4-in-1マルチクリーナー':14,'壁掛けディスペンサー':6,
'ビジュアル耳かき':6,'ジェットウォッシャー':4,'ヘアドライタオル':4,'瞬間冷感ポンチョ':3,'卓上冷感クーラー':3,
'健康サンダル':3,'湯上がりガーゼワンピース':2,'高見えレザーヘッドレストフック':2,'癒しの指圧マット':2,
'ネックマッサージャー':1,'1秒折り畳みチェア':1,'姿勢サポートベルト':1,'UV歯ブラシ除菌器':1,
'3WAYサーキュレーター':1,'スマートノーズEMS美顔器':1}
...
# ★2026-09-11 取得の health_check（窓 = 8/11-9/09 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2388159,1271578,38231),'W固定スマホ車載ホルダー':(1004162,477235,16733),
'快適マジックインソール':(635683,387813,10719),'カタログ全部（テスト）':(580370,145828,5689),
'ムダ毛シェーバー':(545311,439445,6305),'2WAYシートボックス':(355031,116155,3903),
'姿勢サポートチェア':(321699,197526,4644),'バランスケアスリッパ':(267751,185774,4089),
'むくみ取りかっさ':(180671,52778,2083),'ナノバブルシャワーヘッド':(136303,64906,2123),
'温感EMSフェイシャルワンド':(64858,15636,650),'4-in-1マルチクリーナー':(47366,13003,415),
'伸縮ガラスクリーナー':(26259,7474,414),'高見えレザーヘッドレストフック':(3933,777,43)}
...
sh('シンプル判定', ['LOOTY 2026-09-11 定例（データ終端=9/09 水）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/09終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['🔴 判定日','4-in-1マルチクリーナー 20点の中間判定 → **✅ 合格（ぎりぎり）・据え置き**',
  '宣言済み基準「20件到達時に余裕が+0.30未満なら −25%」。9/10で累計20点に到達しました。'
  '7日(9/03-09) 実売97,720 / 14点 / 広告47,366 → 利益 **+8,900円**・MER2.06 vs 分岐1.74・**余裕+0.33**。'
  '★+0.30をわずかに上回ったので **10,000円で据え置き**です。'
  'ただし3日窓は **+0.19** と細く、余裕の水準そのものが薄い。'
  '**次は50点到達時に、余裕が+0.30未満なら −25%**（同じ基準を持ち越します）',
  '—','50点で再判定'],
 ['✅ ユーザー実施','伸縮ガラスクリーナーの日予算が **8,000 → 16,000円** に上がっていました',
  'Meta直読で確認。ナノバブルシャワーヘッドも **8,000 → 14,000円** に上がっています。'
  '★昨日「50点到達時に増額判定」と宣言しましたが、**その前に実行されました**。'
  '結果は良好で、9/09は 広告14,470円（消化率90%）で13点51,740円 → **利益+21,914円・MER3.52**。'
  '**この判断は正しかったと思います**',
  '—','記録'],
 ['🔴 新規宣言','伸縮ガラスクリーナー 増額判定（8,000→16,000）を **9/13** に置きます',
  '基準: **16,000時代（9/09以降）の1日あたり利益 ≥ 8,000時代（9/07-9/08）の +15,294円**。'
  '★9/09の初日は **+21,914円** で基準の143%と好スタート。'
  '★倍増なので、MERが 5.72 → 4.98 → **3.52** と素直に下がっています。'
  '**下がること自体は正常**（規模を取ると効率は落ちる）。見るのは利益額です。'
  '9/13に4日ぶん（9/09-9/12）が揃うので、そこで判定します',
  '—','9/13'],
 ['🚫 除外','快適マジックインソールが「🚀増額候補」に出ていますが、**増額しません**',
  '3窓とも目標超え（余裕 3日+1.13 / 5日+1.22 / 7日+0.97）、週消化率105%で、'
  'スクリーニング上は増額候補の条件を満たしています。'
  '★ですが宣言済みルールに **「直近の増額実験で増分が分岐を割った商品は増額候補から除外する」** があります。'
  '8/30に 27,000→30,000 を試して、増分は「広告+6,514円/日に対し売上−1,825円/日」＝'
  '**追加売上を検出できず**でした。**2週間前に否定された増額を、同じ根拠で蒸し返しません**',
  '—','除外'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（**10日連続で未実行**）',
  'Meta直読で日予算は今も18,000円。9/09の消化は15,701円。30日で **580,370円 ＝ 広告費全体の6.8%**。'
  '★これが唯一残っている未執行の宣言事項です。'
  '商品に紐づかず効率を測れないまま、10日間で約17万円を使い続けています',
  '+8,000円/日','本日'],
 ['📈 好転','3日移動平均利益が3日連続で床の上（155,574 → 135,156 → **135,415円**）',
  '9/07 +135,937 ／ 9/08 +117,482 ／ 9/09 **+152,825（利益率28.2%）**。'
  '9/09は 実売541,333 ÷ 広告240,327 = MER 2.25。'
  '★9月の当月累積は 実売4,416,853 / 利益 **1,047,830円（23.7%）**／1日あたり116,426円。'
  '8月の1日158,620円にはまだ届いていませんが、9/01-04の谷（1日平均76,969円）からは大きく戻しました',
  '—','記録'],
 ['🎉 好調','姿勢サポートチェアが3日で MER3.40・余裕+1.91 まで回復',
  '9/09に11点65,780円、9/10も6点35,880円。3日 +1.91 / 5日 +1.26 / 7日 +0.82。'
  '★9/09に−25%（9,000→6,750）を出して**翌日取り下げた**判断は正解でした。'
  '減額していたら、この回復を自分で潰していたところです。'
  '**単日・2日で決めない**という原則が効いた例として記録します',
  '—','記録'],
 ['🆕 新商品','高見えレザーヘッドレストフックの立ち上がりが良好',
  '9/09 広告3,933円 / 3点11,940円 → 9/10 **6点23,880円**。累計9点。'
  '3窓とも MER2.83 vs 分岐1.55・**余裕+1.28**。日予算8,000円に対し9/09の消化は49%でまだ余裕あり。'
  '★購入50件まで判定しません',
  '—','監視'],
 ['📊 新商品4本の現在地','原価率の低い順に、そのまま強さが並んでいます',
  '**伸縮ガラスクリーナー** 原価率29.1% / 分岐1.41 → **MER4.24・余裕+2.82**（46点）｜'
  '**高見えレザーヘッドレストフック** 35.5% / 1.55 → MER2.83・余裕+1.28（9点）｜'
  '**4-in-1マルチクリーナー** 42.4% / 1.74 → MER2.06・**余裕+0.33**（20点）｜'
  '**温感EMS** 31.2〜34.8% / 1.52 → MER1.57・余裕+0.04（停止済み）。'
  '★**分岐MER 1.5以下（原価率33%以下）を狙う**のが、いまのLOOTYの新商品選定基準として一番機能しています',
  '—','記録'],
 ['📈 速報','9/10(木)は実売 **578,136円**・販売数141点',
  '注文116 / 客単価4,984円 / セッション3,514 / カート追加率5.55%。前週の木曜(9/03 518,956円)比 **+11.4%**。'
  '★**伸縮ガラスクリーナー17点67,660円**（累計46点・50点まであと4点）、'
  '4-in-1が6点41,880円で20点到達、フックが6点23,880円。**新商品3本で29点＝全体の20.6%**。返品なし',
  '—','速報'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
 ...
 ['🟡 推奨','クリーナー系・車用品系をもう1本ずつ',
  '伸縮ガラスクリーナー（原価率29.1%・MER4.24）とフック（車用品・MER2.83）が両方立ち上がっています。'
  'かっさ・2WAY・温感EMSの停止で空いた枠を、**この2系統の低原価率商品**に振り替えるのが最有力です','—','優先'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/09(水)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/07-9/09', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/03-9/09', R['N7'], R['K7'], R['ACCT7']), ('30日 8/11-9/09', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-09', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/11','4-in-1マルチクリーナー 20点の中間判定','20件到達時に余裕が+0.30未満なら −25%',
  '7日(9/03-09) 実売97,720 / 14点 / 広告47,366 → 利益+8,900円・MER2.06 vs 分岐1.74・**余裕+0.33**',
  '✅ 合格（ぎりぎり）→ 10,000で据え置き',
  '★+0.30をわずかに上回った。ただし3日窓は+0.19と細い。**同じ基準を50点到達時へ持ち越す**'],
 ['9/11','伸縮ガラスクリーナー（ユーザーが先に増額）','昨日「50点到達時に増額判定」と宣言していた',
  'Meta直読で日予算 **8,000 → 16,000円**。9/09は広告14,470円（消化率90%）で13点51,740円 → 利益+21,914円',
  '✅ ユーザー実施・結果は良好',
  '★ナノバブルシャワーヘッドも 8,000 → 14,000円 に増額されていた'],
 ['9/13','伸縮ガラスクリーナー 増額判定（8,000→16,000）',
  '**新規宣言: 16,000時代（9/09以降）の1日あたり利益 ≥ 8,000時代（9/07-9/08）の +15,294円**',
  '9/09初日は **+21,914円**（基準の143%）。MERは 5.72 → 4.98 → 3.52 と低下中',
  '⏳ 9/13に4日ぶんで判定',
  '★倍増なのでMERが下がるのは正常（規模を取ると効率は落ちる）。見るのは利益額'],
 ['9/11','快適マジックインソール（増額候補だが除外）','3窓とも目標超え・週消化率105%',
  '余裕 3日+1.13 / 5日+1.22 / 7日+0.97','🚫 増額しない',
  '★宣言済みルール「直近の増額実験で増分が分岐を割った商品は増額候補から除外」に該当。'
  '8/30の 27,000→30,000 は「広告+6,514円/日に対し売上−1,825円/日」で追加売上を検出できなかった'],
 ['9/11','カタログ全部（テスト）','非常ブレーキ由来ではなく単体の効率問題',
  'Meta直読で日予算18,000のまま。9/09の消化15,701円。30日で580,370円＝広告費全体の**6.8%**',
  '🚨 18,000→10,000（**10日連続で未実行**）',
  '★唯一残っている未執行の宣言事項。10日間で約17万円を、効率を測れないまま使い続けている'],
 ['9/11','姿勢サポートチェア（取り下げが正解だった）','9/09に−25%を出し、翌日取り下げた',
  '3日 MER3.40・**余裕+1.91** ／ 5日+1.26 ／ 7日+0.82。9/09は11点65,780円','🎉 回復',
  '★減額していたらこの回復を自分で潰していた。「単日・2日で決めない」が効いた例'],
 ['50点','伸縮ガラスクリーナー／高見えレザーヘッドレストフック','購入50件に達するまで判定しない',
  '伸縮 46点（MER4.24・余裕+2.82）／ フック 9点（MER2.83・余裕+1.28）','⏸ 判定しない',
  '伸縮は9/11〜9/12に50点到達の見込み'],
 ['50点','4-in-1マルチクリーナー','20点判定の基準を持ち越し: 余裕が+0.30未満なら −25%',
  '累計20点。7日 余裕+0.33 / 3日 +0.19','⏸ あと30点','原価率42.4%（分岐1.74）はこの価格帯だと重い'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の判定: **4-in-1マルチクリーナーの20点判定は合格（余裕+0.33・基準+0.30）**→ 10,000で据え置き',
  '★執行は1件だけ: **カタログ 18,000→10,000**（10日連続で未実行・唯一残っている宣言事項）',
  '★伸縮ガラスクリーナーの日予算をユーザーが **8,000→16,000** に増額済み（ナノバブルも 8,000→14,000）。'
  '9/09は消化90%で利益+21,914円と好結果。**9/13に増額判定**を置きます',
  '★快適マジックインソールが増額候補に出ていますが、8/30の増額実験が否定されているので**除外**します',
  '★3日移動平均利益は3日連続で床の上（135,415円）。姿勢サポートチェアは取り下げが正解でした'],
...
for r in body:
    if r[0] == '2026-09-10': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
