# -*- coding: utf-8 -*-
"""[8/27終端に固定した窓ビルダー] 2026-08-29 定例（昨日=8/27 木 実績）。実測のみ・毎回ゼロから積み直す。"""
...
# 2026-08-29: 8/27まで揃った（Σキャンペーン＝アカウントレベルで差0円）。窓は8/27終端。
END='2026-08-29'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-31']
assert D1==['2026-08-29'] and D3[0]=='2026-08-27' and D7[0]=='2026-08-23', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-31', (len(D30),D30[0])
...
assert len(CUM)==29 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-29'
...
# 8/27の縦照合（全店1クエリ実測 gross 670,020円 / 値引 16,328円 / 注文130件 / 点数149）
assert GROSSD['2026-08-29']==599260 and sum(S[n]['2026-08-29'][1] for n in S if '2026-08-29' in S[n])==16121
assert abs(ACCT(D1)-241446)<1, ('8/27 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0827w.pkl','wb'))
