# -*- coding: utf-8 -*-
"""2026-08-02 定例（昨日=8/01 土実績）。実測のみ・毎回ゼロから積み直す。"""
...
'スマートノーズEMS美顔器':2453,'W固定スマホ車載ホルダー':1182}
...
'スマートノーズEMS美顔器':5980,'W固定スマホ車載ホルダー':3980}
...
VAR={'ナノガラス脱毛パッド':{'d1':(41,14),'d3':(103,41),'d7':(212,93),'d30':(774,341)},   # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(1,0),'d3':(5,0),'d7':(7,3),'d30':(76,29)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d1':9,'d3':23,'d7':56,'d30':139},          # 7/27に5,980→6,980
      '湯上がりガーゼワンピース':{'d1':1,'d3':4,'d7':7,'d30':101},      # 窓内に4,980期がある
      '完全遮光・接触冷感UVハット':{'d1':4,'d3':5,'d7':5,'d30':13}}      # 7/31に5,980→4,980（Shopify実測13個・返品0）
HOL={'2026-07-20'}
...
D30=[d for d in ALLD if d>='2026-07-03']
assert D1==['2026-08-01'] and D7[0]=='2026-07-26' and D3[0]=='2026-07-30', (D1,D7[0],D3[0])
assert len(D30)==30 and D30[0]=='2026-07-03', (len(D30),D30[0])
...
print()
SUM={}
for lbl,days,N,K in [('前日(8/01 土)',D1,N1,K1),('3日(7/30-8/01)',D3,N3,K3),
                     ('7日(7/26-8/01)',D7,N7,K7),('30日(7/03-8/01)',D30,N30,K30)]:
...
# 同曜日平均（土曜・祝日除外）
SAT=[d for d in D30C if wd(d)=='土' and d not in HOL]
sat=sum(STORE[d] for d in SAT)/len(SAT)
print(f'\n土曜平均(直近30日 n={len(SAT)}): {sat:,.0f}円 / 8/01実績 {STORE["2026-08-01"]:,}円 = {STORE["2026-08-01"]/sat*100:.1f}%')
BR={};TG={}
...
print('\n'+'='*128)
print('■ 商品別（7日=7/26-8/01 / 3日=7/30-8/01 / 前日=8/01 / 30日=7/03-8/01。すべて実測）')
print('='*128)
...
for n in sorted(N7, key=lambda x:-N7[x]):
    ...
    bud=BUD.get(MAP.get(n,n),{}).get('2026-08-01')
...
pickle.dump(dict(rows=rows,N1=N1,N3=N3,N7=N7,N30=N30,K1=K1,K3=K3,K7=K7,K30=K30,Q1=Q1,Q7=Q7,
  ...
  open('/tmp/claude-0/-home-user-LOOana/42be2ed9-9967-56df-ab9c-75166f45bbc9/scratchpad/rep0802.pkl','wb'))
