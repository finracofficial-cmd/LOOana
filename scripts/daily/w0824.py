# -*- coding: utf-8 -*-
"""[8/24終端に固定した窓ビルダー] 2026-08-21 定例（昨日=8/24 月 実績）。実測のみ・毎回ゼロから積み直す。"""
...
# 2026-08-21: 8/20まで揃った（縦照合とも差0円）。窓は8/24終端。
END='2026-08-24'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-26']
assert D1==['2026-08-24'] and D3[0]=='2026-08-22' and D7[0]=='2026-08-18', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-26', (len(D30),D30[0])
...
assert len(CUM)==24 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-24'
...
assert GROSSD['2026-08-24']==554020 and sum(S[n]['2026-08-24'][1] for n in S if '2026-08-24' in S[n])==24637
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0824w.pkl','wb'))
