# -*- coding: utf-8 -*-
"""[9/21終端に固定した窓ビルダー] 2026-09-23 定例（データ終端=9/21 月・敬老の日）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0921=259080  # 9/21 広告費（health_check 8/23-9/21 − CSV 8/23-9/20 の実測差）
# 窓の検証: 停止済み5本が 0円ちょうど（電動温熱カッサ 20,149 ／ 4-in-1 80,605 ／ ナノバブル 163,744 ／
#   むくみ取りかっさ 180,745 ／ 温感EMS 64,858）。残る3本は微小マイナス（3D足臭 −2 ／ ムダ毛 −10 ／ 2WAY −79）
#   でいずれも再集計ドリフト。8/22始まりは5本が負値、8/24始まりは合計477,839円（日予算238,000の201%）
#   で非現実的 → 8/23-9/21 で確定
...
'スマートノーズEMS美顔器':2453,'W固定スマホ車載ホルダー':1069,'ジェットウォッシャー':1259,'ビジュアル耳かき':1655,'姿勢サポートチェア':1811,
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1120,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609,'温感EMSフェイシャルワンド':1866,'4-in-1マルチクリーナー':2961,'伸縮ガラスクリーナー':1120,'高見えレザーヘッドレストフック':1275,'もちふわ肉球サンダル':1117,'電動温熱カッサ':2533}
...
'スマートノーズEMS美顔器':5980,'W固定スマホ車載ホルダー':3980,'ジェットウォッシャー':4980,'ビジュアル耳かき':4980,'姿勢サポートチェア':5980,
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980,'温感EMSフェイシャルワンド':5980,'4-in-1マルチクリーナー':6980,'伸縮ガラスクリーナー':3980,'高見えレザーヘッドレストフック':3980,'もちふわ肉球サンダル':3980,'電動温熱カッサ':6980}
...
VAR={'ナノガラス脱毛パッド':{'d1':(30,12),'d3':(69,34),'d7':(211,102),'d30':(867,381),'cum':(468,203)},
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(0,2),'d7':(1,2),'d30':(32,10),'cum':(3,2)}}
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981),
      '温感EMSフェイシャルワンド':(1866,2079),  # シルバー, ピンク（2026-09-02 Shopify unitCost 実測）
      '伸縮ガラスクリーナー':(1120,1143,1123,1120),  # ブルー, グレー, グリーン, レッド（2026-09-08 実測）
      '高見えレザーヘッドレストフック':(1275,1344,1344),  # ブラック, ブラウン, グレー（2026-09-10 実測）
      'もちふわ肉球サンダル':(1117,1134,1148,1165)}  # 37-38, 39-40, 41-42, 43-44（2026-09-15 実測・9/14新設）
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
PRICEBYDATE={'優先配送':[('0000-00-00',536),('2026-08-23',790)],                       # 商品ページは590円だが実課金は536円のまま
...
MIXEDDAY={'優先配送':{'2026-08-21':(536,790)},
 '接触冷感UVパーカー':{'2026-08-13':(3980,4980),'2026-08-15':(4980,3980)},         # 値上げ当日の新旧混在（旧価,新価）
...
# ★2026-09-22追加: 9/19(土)〜9/23(水)は **5連休（シルバーウィーク）**。
#   9/21 敬老の日（9月第3月曜）／ 9/22 国民の休日（前後が祝日のため）／ 9/23 秋分の日。
#   祝日は曜日指数の計算から除外する（平日の指数を当てると実態より強く/弱く見えるため）。
HOL={'2026-07-20','2026-08-11','2026-09-21','2026-09-22','2026-09-23'}  # 海の日・山の日・敬老の日・国民の休日・秋分の日
...
V=collections.defaultdict(lambda: collections.defaultdict(int))   # V[商品][日付][原価グループ]
for r in csv.DictReader(open(B+'daily_variant.csv',encoding='utf-8')):
    V[r['name']][r['date']] = V[r['name']].get(r['date'],{}) if isinstance(V[r['name']].get(r['date']),dict) else {}
...
END='2026-09-21'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-23']
assert D1==['2026-09-21'] and D3[0]=='2026-09-19' and D7[0]=='2026-09-15', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-23', (len(D30),D30[0])
...
def qty(n,days,key):
    ...
    if g==0: return 0,0
    if n in VARC:
        # 窓ごとの手打ちをやめ、日次バリアントCSVから毎回積み直す（冪等）
        # ★2026-09-08修正: 旧コードは CG を辞書リテラルで一括構築していたため、
        #   バリアント数の違う商品（伸縮ガラスクリーナー=4色）を足すと
        #   他商品の VARC[n][2] を評価して IndexError になっていた。商品ごとに引く形へ。
        GPALL={'ナノガラス脱毛パッド':('白緑','黒桃'),'壁掛けディスペンサー':('3本','2本'),
               '温感EMSフェイシャルワンド':('シルバー','ピンク'),
               '伸縮ガラスクリーナー':('ブルー','グレー','グリーン','レッド'),
               '高見えレザーヘッドレストフック':('ブラック','ブラウン','グレー'),
               'もちふわ肉球サンダル':('37-38','39-40','41-42','43-44')}
        PRC={'壁掛けディスペンサー':{'3本':6980,'2本':5980}}
        GP={n:{gr:PRC.get(n,{}).get(gr,PRICE[n]) for gr in GPALL[n]}}
        CG={n:{gr:VARC[n][i] for i,gr in enumerate(GPALL[n])}}
        q=0; cost=0; chk=0
        for d in days:
            for grp,gg in VG[n].get(d,{}).items():
                if not gg: continue
                x=gg/GP[n][grp]
                assert abs(x-round(x))<1e-6,(n,d,grp,gg)
                x=round(x); q+=x; cost+=x*CG[n][grp]; chk+=gg
        assert chk==g,(n,key,chk,g)   # バリアント合計 = その窓の gross
        return q, cost
    if n in PRICEBYDATE:
        q=0
        for d in days:
...
CUM=[d for d in ALLD if d>='2026-09-01']
assert len(CUM)==21 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-21'   # 9月に入ったので当月累積は9/01から
...
def CAT(days): return sum(AD[c].get(d,0) for c in ('カタログ全部（テスト）','カタログ全部（テスト） 夏以外') for d in days)
def ACCT(days): return sum(sum(AD[c].get(d,0) for c in AD) for d in days)
# 9/01の縦照合（全店1クエリ実測 gross 444,410円 / 値引 9,008円 / 注文88件 / 点数101）
assert GROSSD['2026-09-21']==782020 and sum(S[n]['2026-09-21'][1] for n in S if '2026-09-21' in S[n])==30049
assert abs(ACCT(D1)-AD0921)<1, ('9/21 広告費 アカウント照合', ACCT(D1))
ALLN=set(N1)|set(N3)|set(N7)|set(N30)|set(NC)
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0921w.pkl','wb'))
