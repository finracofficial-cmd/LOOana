# -*- coding: utf-8 -*-
"""2026-08-31 朝の判定材料。CSV実測のみ（w0830.py 経由）。"""
import runpy
g = runpy.run_path('scripts/daily/w0830.py')
...
def days_back(k, end='2026-08-30'):
...
print('\n■ 判定① ナノガラス2 単独（8/30宣言: 3日連続で倍率<1.00なら60,000→40,000）')
...
print('\n■ 判定② 2WAY・サングラス・ナノバブル（8/30宣言: 2日連続で購入ゼロなら即-50%）')
...
print('\n■ 8/30に配信ゼロだったキャンペーン（7日窓に残消化があるもの）')
D7 = days_back(7)
for c in sorted(AD, key=lambda x: -sum(AD[x].get(d, 0) for d in D7)):
    a7 = sum(AD[c].get(d, 0) for d in D7); a1 = AD[c].get('2026-08-30', 0)
    if a7 > 0 and a1 == 0:
