# -*- coding: utf-8 -*-
"""2026-08-14 定例レポート本体（昨日=8/13 木）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R=pickle.load(open('/tmp/rep0814.pkl','rb')); FEE=0.03452
...
assert any(r[0]=='2026-08-14' for r in body), '8/14スナップショット未記録'
BUD=collections.defaultdict(int)
for r in body:
    if r[0]=='2026-08-14': BUD[r[1]]+=int(r[3])
...
sh('シンプル判定',['LOOTY 2026-08-14 定例（昨日=8/13 木）',
...
# ---------- 2 ネクストアクション ----------
NA=[['🔴 必須','4WAY','3窓すべて余裕<0.30（−0.10/+0.03/+0.08）。限界MER×0.6/0.7/0.8の3端すべてで削ると利益が増える → 11,800→8,850（−25%）','週+5,085円','8/14'],
    ['🔴 必須','5WAY腰掛けファン','宣言済みトリガー発火（3日MER 1.58<分岐1.64 かつ 7日1.93<2.00）→ 14,000→10,500（−25%）','週+3,683円','8/14'],
    ['🔴 必須','バランスケアスリッパ','新素材D 第2関門 不合格（外部CTR 1.61% < 2.00%）→ Dを停止。スリッパはCRでは直らないと確定','—','8/14'],
    ['✅ 判定済','数量割引','本判定 不合格。7日 点数/注文 1.212 < 基準1.25 → 点数/注文の低下はコピーでは直せない。以後この線は追わない','—','完了'],
    ...
    ['👀 様子見','害虫ブロッカー','8/12減額後の判定。3日MER<1.28で停止/−50%、余裕<0.30(MER<1.58)で−25%、以上なら据え置き。現3日1.53＝−25%圏だが判定日まで前倒ししない','週+14,054円','8/15'],
    ['👀 様子見','卓上冷感クーラー','3日MER<1.53で停止。現1.22で不合格圏だが、限界MERの3端で符号が反転する（×0.7で−0.063）ため判定日に頑健性込みで再評価','—','8/15'],
    ['👀 様子見','快適マジックインソール','中間判定（CTR<2.2% or RPM/CPM<0.7で停止）。現 CTR 3.13%・RPM/CPM 2.68で通過見込み。本判定8/19','—','8/15'],
    ['👀 様子見','W固定スマホ車載ホルダー','週次ランプ判定 W-1(8/2-8) vs W0(8/9-15)・増分MER≥2.88','—','8/16'],
    ['👀 様子見','日傘2種','完全遮光のCPMが形状記憶の1.25倍以上のまま かつ余裕<+0.30なら完全遮光を停止','—','8/18'],
    ['👀 様子見','接触冷感UVパーカー','値上げ判定 3,980→4,980。7日販売数≥81個 かつ CVR≥3.22%で合格／58〜80個は4,480へ／<58個は3,980へ戻す。中間チェック8/16','週+86,246円【推計】','8/20'],
    ['👀 様子見','偏光・調光サングラス','LP判定（7日CVR≥3.0%）。実測3.37%で合格圏だが、7日窓にLP改修前の日が残るため8/13に取り直し済みの数値で確定させる','—','再確認済'],
    ['👀 様子見','接触冷感UVアームカバー','監視トリガー 3日MER<1.26（現1.32＝未発火）。ただし限界MER×0.8で符号反転のため削らない','—','監視'],
    ['👀 様子見','ナノガラス脱毛パッド','8/12のCVR2.26%は単日。8/13は2.84%で7日3.51%は健全。3窓は据え置き（3日2.16<目標2.20）','—','監視']]
...
wb.save('data/reports/report-2026-08-14.xlsx')
