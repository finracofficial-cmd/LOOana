# -*- coding: utf-8 -*-
"""[9/18終端に固定した窓ビルダー] 2026-09-20 定例（データ終端=9/18 金）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0918=228472  # 9/18 広告費（health_check 8/20-9/18 − CSV 8/20-9/17 の実測差）
# 窓の検証: 停止済み5本すべてで HC − CSV = 0円ちょうど
...
#   → 8/20-9/18 で確定。★ムダ毛246円・電動温熱カッサ28円＝停止直後の残消化で、停止は9/18から効いている
...
END='2026-09-18'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-20']
assert D1==['2026-09-18'] and D3[0]=='2026-09-16' and D7[0]=='2026-09-12', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-20', (len(D30),D30[0])
...
assert len(CUM)==18 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-18'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-18']==489490 and sum(S[n]['2026-09-18'][1] for n in S if '2026-09-18' in S[n])==14335
assert abs(ACCT(D1)-AD0918)<1, ('9/18 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0918w.pkl','wb'))
