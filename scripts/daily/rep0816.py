# -*- coding: utf-8 -*-
"""2026-08-16 の定例（Supermetrics復旧後の完全版）。前日=2026-08-15。
...
g = runpy.run_path('scripts/daily/w0815.py')
...
print('■ ① 全体（すべて8/15終端・Shopify実測 × Meta実測）')
print(f'  {"窓":<12}{"実売":>12}{"原価":>11}{"広告費":>11}{"総合原価":>11}{"利益":>12}{"利益率":>8}'
      f'{"MER":>7}{"分岐":>7}{"余裕":>7}{"手数料後利益":>13}')
for lbl, N, K, days in [('前日8/15', N1, K1, D1), ('3日', N3, K3, D3), ('7日', N7, K7, D7),
...
print(f'\n  8/15（{wd(D0)}）vs 同曜日平均 {sa:,.0f}円 → {STORE[D0]/sa-1:+.1%}（n={len(same)}）'
...
assert abs(sum(A(D7, n) for n in N7) + CAT(D7) - ACCT(D7)) < 1
