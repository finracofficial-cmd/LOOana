# -*- coding: utf-8 -*-
"""[9/1終端に固定した窓ビルダー] 2026-09-02 定例（昨日=9/1 火 実績）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0901=229722  # data_query 実測（9/2朝取得・8/31分は223,671へ上書き済み）
...
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1120,'温感EMSフェイシャルワンド':2079,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'温感EMSフェイシャルワンド':5980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980}
...
# 2026-09-02: 9/1まで揃った（全店・商品とも実測。8/31広告費は再取得で確定済み）。窓は9/1終端。
END='2026-09-01'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=ALLD[-30:]
assert D1==['2026-09-01'] and D3[0]=='2026-08-30' and D7[0]=='2026-08-26', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-03', (len(D30),D30[0])
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（初日）
assert len(CUM)==1 and CUM[0]=='2026-09-01'
...
# 9/1の縦照合（全店1クエリ実測 gross 444,410円 / 値引 9,008円 / 注文88件 / 点数101）
assert GROSSD['2026-09-01']==444410 and sum(S[n]['2026-09-01'][1] for n in S if '2026-09-01' in S[n])==9008
assert abs(ACCT(D1)-AD0901)<1, ('9/1 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0901w.pkl','wb'))
