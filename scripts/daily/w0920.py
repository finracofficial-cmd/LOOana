# -*- coding: utf-8 -*-
"""[9/20終端に固定した窓ビルダー] 2026-09-22 定例（データ終端=9/20 日）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0920=256038  # 9/20 広告費（health_check 8/22-9/20 − CSV 8/22-9/19 の実測差）
# 窓の検証: 停止済み5本が 0円ちょうど（電動温熱カッサ 20,149 ／ 4-in-1 80,605 ／ ナノバブル 163,744 ／
#   むくみ取りかっさ 180,745 ／ 温感EMS 64,858）。残る3本は微小マイナス（3D足臭 −2 ／ ムダ毛 −10 ／ 2WAY −73）
#   でいずれも再集計ドリフト。8/21始まりは6本が負値、8/23始まりは合計453,545円（日予算238,000の191%）
#   で非現実的 → 8/22-9/20 で確定
...
END='2026-09-20'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-22']
assert D1==['2026-09-20'] and D3[0]=='2026-09-18' and D7[0]=='2026-09-14', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-22', (len(D30),D30[0])
...
assert len(CUM)==20 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-20'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-20']==671340 and sum(S[n]['2026-09-20'][1] for n in S if '2026-09-20' in S[n])==20100
assert abs(ACCT(D1)-AD0920)<1, ('9/20 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0920w.pkl','wb'))
