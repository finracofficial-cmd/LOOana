# -*- coding: utf-8 -*-
"""2026-09-07 定例レポート本体（データ終端=9/05 土／9/06の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0905w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-07'   # ★Meta 直読。カタログは18,000のまま（非常ブレーキ①が6日連続で未実行）
...
'ネックマッサージャー':1,'瞬間冷感ポンチョ':1,'壁掛けディスペンサー':1,'5WAY腰掛けファン':1,'ヘアドライタオル':1,
'癒しの指圧マット':1,'卓上冷感クーラー':1,'1秒折り畳みチェア':1,'完全遮光・形状記憶':1}
STORE_ORD7, STORE_SESS7 = 661, 21999
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 677, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1179,'W固定スマホ車載ホルダー':527,'快適マジックインソール':253,'ムダ毛シェーバー':190,
'形状記憶日傘':168,'2WAYシートボックス':159,'偏光・調光サングラス':130,'接触冷感UVパーカー':125,
'完全遮光・接触冷感UVハット':105,'バランスケアスリッパ':101,'姿勢サポートチェア':101,'害虫ブロッカー':97,
'4WAY':90,'むくみ取りかっさ':65,'完全遮光・形状記憶':59,'優先配送':59,'接触冷感UVアームカバー':47,
'5WAY腰掛けファン':36,'ナノバブルシャワーヘッド':35,'携帯電動シェーバー':27,'3D足臭リセットブラシ':21,
'卓上冷感クーラー':15,'温感EMSフェイシャルワンド':8,'瞬間冷感ポンチョ':7,'ビジュアル耳かき':6,
'ヘアドライタオル':4,'壁掛けディスペンサー':4,'健康サンダル':3,'癒しの指圧マット':2,'ジェットウォッシャー':2,
'4-in-1マルチクリーナー':2,'3WAYサーキュレーター':2,'瞬間冷却ハンディファン':1,'UV歯ブラシ除菌器':1,
'姿勢サポートベルト':1,'1秒折り畳みチェア':1,'湯上がりガーゼワンピース':1,'ネックマッサージャー':1}
...
# ★2026-09-07 取得の health_check（窓 = 8/07-9/05 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2403251,1293489,38632),'W固定スマホ車載ホルダー':(986961,502721,16933),
'カタログ全部（テスト）':(598964,149156,6035),'ムダ毛シェーバー':(560269,452608,6536),
'快適マジックインソール':(526987,321658,9127),'2WAYシートボックス':(377052,129983,4370),
'偏光・調光サングラス':(357601,152479,5118),'姿勢サポートチェア':(323955,202006,4636),
'完全遮光・接触冷感UVハット':(274963,62767,3460),'バランスケアスリッパ':(227108,157959,3301),
'むくみ取りかっさ':(144162,39672,1580),'ナノバブルシャワーヘッド':(99901,47056,1619),
'温感EMSフェイシャルワンド':(33319,7800,312),'4-in-1マルチクリーナー':(3417,1245,20)}
...
sh('シンプル判定', ['LOOTY 2026-09-07 定例（データ終端=9/05 土）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/05終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['📈 回復','**9/05(土)は利益 +178,738円・利益率33.4%。8/29以来のいい日でした**',
  '実売534,465 ÷ 広告 **218,144** = **MER 2.45**（9/04は1.45）。'
  '広告費が前日から **−39,329円** 落ちたのが効いています（サングラス10円・UVハット5,920円＝停止の効果）。'
  '3日移動平均利益も 71,636 → **105,204円** へ戻しました（床130,000の81%）。'
  '★9/06(日)も実売601,723円と続伸しているので、**山は越えつつあります**',
  '—','記録'],
 ['🚨 執行','むくみ取りかっさ **10,000 → 7,500円（−25%）**',
  '3窓とも余裕<0.30 に入りました（3日 **−0.09** ／ 5日 **−0.04** ／ 7日 +0.29）＝宣言済みルールの発動。'
  '3日(9/03-05) 実売42,984 / 11点 / 広告32,822 / 原価12,320 → **損益 −2,158円**。'
  '5日も −1,748円で、**直近は赤字が続いています**（7日はまだ +17,266円）。'
  '★9/05に13,000→10,000へ下げた効果はまだ足りていません。もう一段の−25%です',
  '+2,500円/日','本日'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（非常ブレーキ①・**6日連続で未実行**）',
  'Metaを直読すると日予算は今も18,000円。9/05の消化は16,363円でした。'
  '非常ブレーキは 9/01 から **6日連続**で発動しています（3日移動平均 110,507→81,737→95,960→71,636→105,204）。'
  '★9/05に大きく戻したのは**広告費が−39,329円落ちたから**で、売上が伸びたからではありません。'
  '同じことをカタログでもう一段やるのが、宣言済みで唯一残っている手です',
  '+8,000円/日','本日'],
 ['🎉 好調','**4-in-1マルチクリーナーの立ち上がりが良いです**',
  '9/05(初日) 広告3,417円 / 2点13,960円 → **MER 4.09 vs 分岐1.74・余裕+2.35**、利益 +4,621円。'
  '9/06も3点20,940円と続いています（累計5件）。'
  '★**購入50件まで判定しません**（宣言済みルール）。増額もまだしません。'
  '原価率42.4%と高い商品なので、件数が増えたときにMERが4台を保てるかを見ます。'
  '次の節目は **20件到達時に中間チェック**',
  '—','監視'],
 ['⏸ 監視','温感EMSフェイシャルワンド（累計13件・余裕が薄い）',
  '3日 MER1.62 分岐1.51 余裕+0.11 ／ 5日・7日 1.58 分岐1.50 +0.07。'
  '5日(9/01-05) 実売52,624 / 9点 / 広告33,319 / 原価17,646 → **利益 +1,659円**。'
  '★**黒字ではあるが、5日で+1,659円＝1日あたり332円**。日予算10,000円に対して薄すぎます。'
  '判定表の「⚠️−25%」は誤検知（窓3つが同じ期間しか含まない）なので今日は触りませんが、'
  '**20件到達時点で「余裕が+0.30を超えていなければ−25%」を宣言しておきます**',
  '—','20件で判定'],
 ['✅ 効果確認','UVハット・サングラスの停止が効いています',
  'UVハットは9/05に5,920円消化して停止。3窓判定は **🚨停止/−50%**（3日 −1.12 ／ 5日 −0.27 ／ 7日 −0.19）まで落ちており、'
  '**停止は正しい判断でした**。サングラスも9/05は10円で実質ゼロ。'
  '2本で日あたり約11,000円が止まり、それが9/05のMER改善に直結しています',
  '—','完了'],
 ['⏸ 触らない','姿勢サポートチェアは3窓判定を抜けました',
  '昨日「あと1窓で発動」と書きましたが、9/05に回復して **3日 MER1.82 余裕+0.37** となり、'
  '3窓のうち3日窓が0.30を超えたため **据え置き** です（5日 −0.34 / 7日 +0.15）。'
  '★まだ不安定なので監視は続けます。9/05の減額（12,000→9,000）の効果を待ちます',
  '—','監視'],
 ['⏸ 触らない','W固定スマホ車載ホルダー（余裕の縮小が続いています）',
  '余裕は 7日+0.51 → 5日+0.36 → **3日+0.24** と単調に縮小。3窓とも分岐は割っていないので据え置きですが、'
  '**3日窓が0.30を割りました**。あと2窓で−25%の発動条件に入ります。'
  '★日予算30,000円と大きいので、割れたときの影響も大きい。最優先の監視対象です',
  '—','監視'],
 ['📈 速報','9/06(日)は実売 **601,723円**',
  '注文122 / 販売数137 / 客単価4,932円 / セッション3,752 / カート追加率5.84%。'
  '前週の日曜(8/30 594,407円)比 **+1.2%** で、ようやく前週の同曜日を上回りました。'
  '★スマートノーズEMS美顔器・ジェットウォッシャーが各1点（広告ゼロ＝オーガニック）。返品1件3,980円(W固定)',
  '—','速報'],
 ['🔴 判定日','バランスケアスリッパ 増額判定（12,000→15,000）',
  '基準「1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円」。3窓は余裕+1.16〜1.21で非常に健全','—','明日 9/8'],
 ['🔴 判定日','快適マジックインソール 再テスト',
  '基準「27,000戻しの7日利益 ≥ 27,000時代の7日平均」。3窓は余裕+0.60〜0.88で回復傾向','—','9/10'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
 ...
 ['🟡 推奨','秋冬商材をもう1〜2本',
  '4-in-1マルチクリーナー（MER4.09）と温感EMS（余裕+0.11）で明暗が分かれました。'
  'クリーナー系のほうが手応えがあるので、**同じ生活雑貨カテゴリ**でもう1本試す価値があります','—','優先'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/05(土)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/03-9/05', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/30-9/05', R['N7'], R['K7'], R['ACCT7']), ('30日 8/07-9/05', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-05', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/7','むくみ取りかっさ','3窓すべて 余裕<0.30 → −25%',
  '3日 **−0.09**（実売42,984 / 11点 / 広告32,822 → 損益 **−2,158円**）／ 5日 **−0.04**（−1,748円）／ 7日 +0.29',
  '🚨 −25% → 10,000を7,500へ',
  '9/05に13,000→10,000へ下げたが足りていない。直近3日・5日はどちらも赤字'],
 ['9/7','**非常ブレーキ**','3日移動平均利益 < 130,000円 で発動',
  '9/03 +116,878 / 9/04 +19,995 / 9/05 **+178,738** → **平均 105,204円**（床の81%）',
  '🚨 6日連続・ただし回復中',
  '★9/05の改善は広告費が−39,329円落ちたことによるもので、売上が伸びたからではない。'
  '執行はカタログ 18,000→10,000 の1本（Meta直読で今も18,000のまま）'],
 ['9/7','完全遮光・接触冷感UVハット','3窓すべて 分岐割れ → 停止/−50%',
  '3日 −1.12（9/05は広告5,920円で販売ほぼゼロ）／ 5日 −0.27 ／ 7日 −0.19',
  '✅ ユーザーが停止済み',
  '昨日の時点では「余裕<0.30」だったが、今朝は**分岐割れ**まで落ちた。停止が正しかったことが確定'],
 ['9/7','偏光・調光サングラス','3窓すべて 余裕<0.30 → −25%',
  '3日 −0.07 ／ 5日 +0.17 ／ 7日 +0.15。9/05の消化は **10円**で実質ゼロ',
  '✅ ユーザーが停止済み','—'],
 ['9/8','バランスケアスリッパ 増額判定（12,000→15,000）','1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円',
  '3窓は 3日+1.16 / 5日+1.20 / 7日+1.21 で非常に健全','⏳ 明日が判定日','増額直後のロック中なので重ねない'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均',
  '3窓は 3日+0.88 / 5日+0.78 / 7日+0.60 と回復傾向。9/05も好調','⏳ 進行中','—'],
 ['—','4-in-1マルチクリーナー（新商品）','購入50件に達するまで判定しない',
  '初日9/05: 広告3,417円 / 2点13,960円 → **MER 4.09 vs 分岐1.74・余裕+2.35**・利益+4,621円。9/06も3点。累計5件',
  '⏸ 判定しない（好調）',
  '★立ち上がりは今期の新商品で最良。ただし2日ぶんなので増額もしない。**20件到達時に中間チェック**'],
 ['20件','温感EMSフェイシャルワンド','**新規宣言: 20件到達時に余裕が+0.30を超えていなければ −25%**',
  '累計13件。3日 MER1.62 分岐1.51 余裕+0.11 ／ 5日・7日 1.58 分岐1.50 +0.07。5日で利益 +1,659円',
  '⏸ 判定しない（薄い）',
  '★黒字だが5日で+1,659円＝1日あたり332円。50件を待たずに中間の基準を先に宣言しておく'],
 ['—','W固定スマホ車載ホルダー','3窓すべて 余裕<0.30 で −25%',
  '余裕は 7日+0.51 → 5日+0.36 → **3日+0.24** と単調縮小。3日窓が0.30を割った','⏸ 据え置き（あと2窓）',
  '★日予算30,000円と大きく、割れたときの影響も大きい。最優先の監視対象'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は2件: ①**むくみ取りかっさ 10,000 → 7,500**（3窓とも余裕<0.30）'
  '②**カタログ全部（テスト）18,000 → 10,000**（非常ブレーキ①・6日連続で未実行）。合わせて10,500円/日',
  '★9/05は利益+178,738円・利益率33.4%・MER2.45で大きく戻した。3日移動平均も71,636→105,204円へ回復',
  '★ただし改善の理由は**広告費が−39,329円落ちたこと**で、売上が伸びたからではない。同じことをもう一段やる',
  '★4-in-1マルチクリーナーが MER4.09（分岐1.74）と好調。ただし2日ぶんなので判定も増額もしない'],
...
for r in body:
    if r[0] == '2026-09-06': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
