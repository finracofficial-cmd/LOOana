# -*- coding: utf-8 -*-
"""[8/31終端に固定した窓ビルダー] 2026-09-01 定例（昨日=8/31 月 実績）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0831=223174  # data_query 実測（9/1 00:10取得）。Σキャンペーンと差0円。※確定まで+0.1%程度動きうる→朝に再取得
...
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1120,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980}
...
# 2026-09-01: 8/31まで揃った（全店・商品・広告とも当日終了直後の実測）。窓は8/31終端。
END='2026-08-31'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=ALLD[-30:]
assert D1==['2026-08-31'] and D3[0]=='2026-08-29' and D7[0]=='2026-08-25', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-02', (len(D30),D30[0])
...
CUM=[d for d in ALLD if d>='2026-08-01']   # 当月累積 = 8月全月（前日8/31が月末のため31日で確定）
assert len(CUM)==31 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-31'
...
# 8/31の縦照合（全店1クエリ実測 gross 400,180円 / 値引 5,971円 / 注文84件 / 点数91）
assert GROSSD['2026-08-31']==400180 and sum(S[n]['2026-08-31'][1] for n in S if '2026-08-31' in S[n])==5971
assert abs(ACCT(D1)-AD0831)<1, ('8/31 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0831w.pkl','wb'))
