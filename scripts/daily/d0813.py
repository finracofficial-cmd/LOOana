# -*- coding: utf-8 -*-
"""2026-08-13 定例（昨日=8/12 水 実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(18,11),'d3':(101,44),'d7':(241,112),'d30':(854,374),'cum':(417,180)},
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(0,0),'d7':(1,0),'d30':(39,8),'cum':(3,0)}}
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':196},'湯上がりガーゼワンピース':{'d30':41},
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-14']
assert D1==['2026-08-12'] and D3[0]=='2026-08-10' and D7[0]=='2026-08-06', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-14', (len(D30),D30[0])
...
assert len(CUM)==12 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-12'
...
assert sum(v[0] for n in S for d in CUM for v in [S[n][d]] if d in S[n])==10296944+926316+704468
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0813.pkl','wb'))
