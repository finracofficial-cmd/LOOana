# -*- coding: utf-8 -*-
"""2026-09-08 定例レポート本体（データ終端=9/06 日／9/07の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0906w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-08'   # ★Meta 直読。伸縮ガラスクリーナー 8,000 が新設。カタログは18,000のまま
...
assert sum(ORD.values()) == 680, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1172,'W固定スマホ車載ホルダー':529,'快適マジックインソール':264,'ムダ毛シェーバー':197,
'形状記憶日傘':157,'2WAYシートボックス':152,'偏光・調光サングラス':118,'接触冷感UVパーカー':112,
'バランスケアスリッパ':106,'姿勢サポートチェア':100,'完全遮光・接触冷感UVハット':99,'害虫ブロッカー':90,
'4WAY':78,'むくみ取りかっさ':69,'優先配送':55,'完全遮光・形状記憶':51,'接触冷感UVアームカバー':46,
'ナノバブルシャワーヘッド':36,'5WAY腰掛けファン':34,'携帯電動シェーバー':27,'3D足臭リセットブラシ':21,
'卓上冷感クーラー':12,'温感EMSフェイシャルワンド':12,'ビジュアル耳かき':6,'瞬間冷感ポンチョ':6,
'4-in-1マルチクリーナー':5,'壁掛けディスペンサー':4,'ヘアドライタオル':4,'ジェットウォッシャー':3,
'健康サンダル':3,'癒しの指圧マット':2,'3WAYサーキュレーター':2,'瞬間冷却ハンディファン':1,
'UV歯ブラシ除菌器':1,'スマートノーズEMS美顔器':1,'姿勢サポートベルト':1,'1秒折り畳みチェア':1,
'湯上がりガーゼワンピース':1,'ネックマッサージャー':1}
...
# ★2026-09-08 取得の health_check（窓 = 8/08-9/06 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2417399,1301581,39126),'W固定スマホ車載ホルダー':(1001867,504365,17164),
'カタログ全部（テスト）':(597380,149159,5947),'ムダ毛シェーバー':(562100,454607,6558),
'快適マジックインソール':(559921,343788,9633),'2WAYシートボックス':(375811,129739,4342),
'姿勢サポートチェア':(327368,204651,4741),'バランスケアスリッパ':(241476,168099,3598),
'むくみ取りかっさ':(156790,43651,1747),'ナノバブルシャワーヘッド':(110183,53080,1786),
'温感EMSフェイシャルワンド':(44699,10853,450),'4-in-1マルチクリーナー':(14766,4183,109)}
...
sh('シンプル判定', ['LOOTY 2026-09-08 定例（データ終端=9/06 日）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/06終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['🔴 判定日','バランスケアスリッパ 増額判定（12,000→15,000）→ **✅ 合格・15,000で据え置き**',
  '宣言済み基準「1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円」。'
  '★12,000→15,000の変更日が特定できなかったので、**窓を2通り**で測って両方が同じ結論になることを確認しました。'
  '主窓 9/05-9/06（Meta直読で15,000を確認済み）= 1日 **+11,088円（基準の123%）** ／ '
  '広窓 9/03-9/06（変更が早かった場合）= 1日 **+9,129円（101%）**。**両窓とも合格**。'
  '3窓も余裕+0.90〜0.96で健全（7日利益 +63,507円）',
  '—','据え置き'],
 ['⚠️ 注意','ただしスリッパの9/06は 広告18,952円で利益 +3,104円しかありません',
  '日予算15,000に対して**126%**まで消化して、実売は29,880円（6点）。'
  '★広窓が基準の101%ギリギリなのはこの日が効いています。'
  '**合格は合格なので据え置きますが、明日以降このパターンが続くなら3窓が割れます**。'
  '9/06単日で決めない（宣言済みの原則）ので今日は動かしません',
  '—','監視'],
 ['🚨 執行','むくみ取りかっさ **10,000 → 7,500円（−25%）**（昨日から2日連続）',
  '3窓とも余裕<0.30（3日+0.23 ／ 5日+0.01 ／ **7日 −0.05**）。昨日より7日窓が悪化して**分岐割れ**に入りました。'
  '9/06の消化は12,628円（日予算10,000の126%）。'
  '★昨日提案した減額がまだ入っていません。**今日こそ7,500へ**',
  '+2,500円/日','本日'],
 ['🚨 執行','2WAYシートボックス **8,000 → 6,000円（−25%）**（新規）',
  '3窓とも余裕<0.30 に入りました（3日 **−0.33** ／ 5日+0.26 ／ 7日+0.06）＝宣言済みルールの発動。'
  '3日窓(9/04-06)は MER1.34 vs 分岐1.67 で**分岐割れ**。9/06の消化は9,568円（日予算8,000の120%）。'
  '★7日ではまだ+0.06で黒字圏なので、停止ではなく**−25%**です',
  '+2,000円/日','本日'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（非常ブレーキ①・**7日連続で未実行**）',
  'Meta直読で日予算は今も18,000円。9/06の消化は **20,303円**。'
  '★非常ブレーキは9/01から**6日連続**で発動しています（3日移動平均 116,927円＝床130,000の90%）。'
  '回復基調ではありますが、まだ床を下回っています。'
  '上の3本を合わせると **12,500円/日** の圧縮になります',
  '+8,000円/日','本日'],
 ['📈 回復','3日移動平均利益は3日連続で改善（71,636 → 105,204 → **116,927円**）',
  '9/04 +19,995 → 9/05 +178,738 → 9/06 **+152,048（利益率25.3%）**。床130,000まであと13,073円。'
  '★ただし9/06の広告費は **291,745円**（前日から +73,601円）と急増しており、MERは2.45→**2.06**に落ちています。'
  '売上601,723円は前週日曜(8/30 594,407)比+1.2%なので、**上振れは広告費で買ったもの**です',
  '—','記録'],
 ['🆕 新商品','**伸縮ガラスクリーナー**（9/07にキャンペーン新設・日予算8,000円）',
  '売価3,980円 / 原価 ブルー1,120・グレー1,143・グリーン1,123・レッド1,120 → 実売分の原価率28.3%・'
  '**分岐MER 1.39・目標MER 2.73**。初日9/07に **3点11,940円**（ブルー2・グレー1）。'
  '★これまでの新商品で**いちばん原価率が低い**（4-in-1は42.4%、温感EMSは31.2〜34.8%）ので、'
  '同じMERでも利益率が高く出ます。購入50件まで判定しません',
  '—','監視'],
 ['📊 新商品3本の比較','原価率が低い順に有利です',
  '**伸縮ガラスクリーナー** 原価率28.3% / 分岐1.39 → 3点（9/07開始）｜'
  '**温感EMS** 原価率31.2〜34.8%（色差）/ 分岐1.50 → 累計16件・3窓 余裕+0.15〜+0.20｜'
  '**4-in-1マルチクリーナー** 原価率42.4% / 分岐1.74 → 累計9件・3窓 余裕+0.63（MERは初日4.09→**2.36**へ低下）。'
  '★4-in-1は消化が3,417→11,349円に増えた分MERが落ちました。**スケールに耐えるかはこれから**',
  '—','記録'],
 ['⏸ 監視','姿勢サポートチェアの7日窓が分岐割れに入りました',
  '3日 MER2.05 余裕+0.60 ／ 5日 1.23 **−0.22** ／ 7日 1.09 **−0.35**。'
  '3日窓が+0.60なので3窓ルールでは据え置きですが、**5日・7日が両方マイナス**です。'
  '★明日3日窓が0.30を割ったら−25%を出します。9/05の減額（12,000→9,000）の効果を待ちます',
  '—','監視'],
 ['📈 速報','9/07(月)は実売 **478,435円**',
  '注文96 / 販売数108 / 客単価4,984円 / セッション **2,787**（前日3,752から−26%）/ カート追加率5.60%。'
  '前週の月曜(8/31 394,209円)比 **+21.4%**。返品なし。'
  '★壁掛けディスペンサーが2点（広告ゼロ＝オーガニック）',
  '—','速報'],
 ['🔴 判定日','快適マジックインソール 再テスト',
  '基準「27,000戻しの7日利益 ≥ 27,000時代の7日平均」。3窓は余裕+0.75〜1.00と好調','—','9/10'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/06(日)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/04-9/06', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/31-9/06', R['N7'], R['K7'], R['ACCT7']), ('30日 8/08-9/06', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-06', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/8','バランスケアスリッパ 増額判定（12,000→15,000）','1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円',
  '主窓 9/05-9/06 = 1日 **+11,088円（123%）** ／ 広窓 9/03-9/06 = 1日 **+9,129円（101%）**',
  '✅ 両窓とも合格 → 15,000で据え置き',
  '★変更日が特定できなかったので窓を2通りで測り、両方が同じ結論になることを確認した。'
  'ただし9/06は広告18,952円で利益+3,104円のみ。広窓が101%ギリギリなのはこの日が効いている'],
 ['9/8','むくみ取りかっさ','3窓すべて 余裕<0.30 → −25%',
  '3日+0.23 ／ 5日+0.01 ／ **7日 −0.05**（昨日の+0.29から分岐割れへ悪化）。9/06の消化12,628円',
  '🚨 −25% → 10,000を7,500へ',
  '昨日と同じ執行。まだ入っていない'],
 ['9/8','2WAYシートボックス','3窓すべて 余裕<0.30 → −25%',
  '3日 **−0.33**（MER1.34 vs 分岐1.67）／ 5日+0.26 ／ 7日+0.06','🚨 −25% → 8,000を6,000へ',
  '7日はまだ黒字圏なので停止ではなく−25%'],
 ['9/8','**非常ブレーキ**','3日移動平均利益 < 130,000円 で発動',
  '9/04 +19,995 / 9/05 +178,738 / 9/06 +152,048 → **平均 116,927円**（床の90%）',
  '🚨 6日連続・ただし3日連続で改善',
  '★9/06の広告費291,745円は前日から+73,601円の急増でMERは2.45→2.06に低下。'
  '執行はカタログ 18,000→10,000（7日連続で未実行）'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均',
  '3窓は 3日+1.00 / 5日+0.81 / 7日+0.75 と好調','⏳ 進行中','—'],
 ['20件','温感EMSフェイシャルワンド','宣言済み: 20件到達時に余裕が+0.30を超えていなければ −25%',
  '累計16件。3日 余裕+0.15 ／ 5日・7日 +0.20','⏸ あと4件','—'],
 ['—','4-in-1マルチクリーナー（新商品）','購入50件に達するまで判定しない',
  '累計9件。3窓とも MER2.36 vs 分岐1.74・余裕+0.63。消化が3,417→11,349円に増えてMERは4.09→2.36へ低下',
  '⏸ 判定しない','★スケールに耐えるかはこれから。20件到達時に中間チェック'],
 ['—','伸縮ガラスクリーナー（新商品）','購入50件に達するまで判定しない',
  '9/07新設・日予算8,000円。初日3点11,940円。売価3,980 / 原価1,120〜1,143 → **原価率28.3%・分岐MER1.39**',
  '⏸ 判定しない','★新商品3本で最も原価率が低く、同じMERでも利益率が高く出る'],
 ['—','姿勢サポートチェア','3窓すべて 余裕<0.30 で −25%',
  '3日+0.60 ／ 5日 **−0.22** ／ 7日 **−0.35**','⏸ 据え置き（3日窓が抜けている）',
  '★5日・7日が両方マイナス。明日3日窓が0.30を割ったら−25%を出す'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の判定: **バランスケアスリッパは合格**（主窓+11,088円/日=123%、広窓+9,129円/日=101%）→ 15,000で据え置き',
  '★執行3件: ①**むくみ取りかっさ 10,000→7,500** ②**2WAYシートボックス 8,000→6,000** '
  '③**カタログ 18,000→10,000**（7日連続で未実行）。合わせて12,500円/日',
  '★3日移動平均利益は3日連続で改善（71,636→105,204→116,927円）。床130,000まであと13,073円',
  '★ただし9/06の広告費291,745円は前日から+73,601円の急増で、MERは2.45→2.06に低下している'],
...
for r in body:
    if r[0] == '2026-09-07': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
