# -*- coding: utf-8 -*-
"""[9/6終端に固定した窓ビルダー] 2026-09-07 定例（昨日=9/6 日 実績）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0906=291845  # 9/6 data_query 実測（キャンペーン単位・アカウント照合 差0円）※翌朝に確定値で再取得判断
...
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1120,'温感EMSフェイシャルワンド':2079,'4-in-1マルチクリーナー':2961,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'温感EMSフェイシャルワンド':5980,'4-in-1マルチクリーナー':6980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980}
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（6日）
assert len(CUM)==6 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-06'
...
# 9/6の縦照合（全店1クエリ実測 gross 614,260円 / 値引 12,537円 / 注文122件 / 点数136）
assert GROSSD['2026-09-06']==614260 and sum(S[n]['2026-09-06'][1] for n in S if '2026-09-06' in S[n])==12537
assert abs(ACCT(D1)-AD0906)<1, ('9/6 広告費 アカウント照合', ACCT(D1))
