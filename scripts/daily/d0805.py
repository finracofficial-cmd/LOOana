# -*- coding: utf-8 -*-
"""2026-08-05 定例（昨日=8/04 火実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(28,13),'d3':(105,46),'d7':(241,97),'d30':(771,343),'cum':(146,60)},   # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(0,0),'d7':(5,0),'d30':(58,20),'cum':(1,0)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':174},                # 7/27に5,980→6,980（30日窓のみ新旧混在）
      '湯上がりガーゼワンピース':{'d30':86},            # 30日窓に4,980期がある
      '完全遮光・接触冷感UVハット':{'d30':20},          # 7/31に5,980→4,980
      '3WAYサーキュレーター':{'d3':13,'d7':38,'d30':183,'cum':20},   # 8/2に5,980→6,980
      '姿勢サポートチェア':{'d3':4,'d7':4,'d30':4,'cum':4}}          # 8/4に7,980→5,980（8/2-3に7,980が1個）
HOL={'2026-07-20'}
...
D30=[d for d in ALLD if d>='2026-07-06']
assert D1==['2026-08-04'] and D7[0]=='2026-07-29' and D3[0]=='2026-08-02', (D1,D7[0],D3[0])
assert len(D30)==30 and D30[0]=='2026-07-06', (len(D30),D30[0])
...
print()
SUM={}
for lbl,days,N,K in [('前日(8/04 火)',D1,N1,K1),('3日(8/02-8/04)',D3,N3,K3),
                     ('7日(7/29-8/04)',D7,N7,K7),('30日(7/06-8/04)',D30,N30,K30)]:
...
# 同曜日平均（土曜・祝日除外）
SAT=[d for d in D30C if wd(d)=='火' and d not in HOL]
sat=sum(STORE[d] for d in SAT)/len(SAT)
print(f'\n火曜平均(直近30日 n={len(SAT)}): {sat:,.0f}円 / 8/04実績 {STORE["2026-08-04"]:,}円 = {STORE["2026-08-04"]/sat*100:.1f}%')
BR={};TG={}
...
print('\n'+'='*128)
print('■ 商品別（7日=7/29-8/04 / 3日=8/02-8/04 / 前日=8/04 / 30日=7/06-8/04。すべて実測）')
print('='*128)
...
for n in sorted(N7, key=lambda x:-N7[x]):
    ...
    bud=BUD.get(MAP.get(n,n),{}).get('2026-08-05')
...
pickle.dump(dict(rows=rows,N1=N1,N3=N3,N7=N7,N30=N30,K1=K1,K3=K3,K7=K7,K30=K30,Q1=Q1,Q7=Q7,
  ...
  open('/tmp/claude-0/-home-user-LOOana/42be2ed9-9967-56df-ab9c-75166f45bbc9/scratchpad/rep0805.pkl','wb'))
