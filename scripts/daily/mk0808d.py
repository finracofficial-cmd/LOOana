...
if not any(r[0]=='2026-08-08' for r in body):      # 既に8/07行があれば上書きしない（当日の変更を消さないため）
    for r in [x for x in body if x[0]=='2026-08-07']: body.append(['2026-08-08',r[1],r[2],r[3]])
with open('/home/user/LOOana/data/budget-snapshots.csv','w',newline='') as f:
    w=csv.writer(f); w.writerow(hdr); w.writerows(body)
BUD={r[2]:int(r[3]) for r in body if r[0]=='2026-08-08'}
TOTB=sum(BUD.values())
...
# 1 全体サマリー
S=[]
for k,lbl,N,K,ACC,days in [('d1','前日 8/07(金)',R['N1'],R['K1'],R['ACCT1'],R['D1']),
    ('d3','3日 8/05-07',R['N3'],R['K3'],R['ACCT3'],R['D3']),
    ('d7','7日 8/01-07',R['N7'],R['K7'],R['ACCT7'],R['D7']),
    ('d30','30日 7/09-8/07',R['N30'],R['K30'],R['ACCT30'],R['D30'])]:
...
wb.save('/home/user/LOOana/data/reports/report-2026-08-08.xlsx')
