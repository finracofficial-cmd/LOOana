# -*- coding: utf-8 -*-
"""[9/4終端に固定した窓ビルダー] 2026-09-05 定例（昨日=9/4 金 実績）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0904=257498  # data_query 実測（9/5 13:06 確定値で上書き済み・fix0904ad.py）
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（4日）
assert len(CUM)==4 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-04'
...
# 9/4の縦照合（全店1クエリ実測 gross 381,300円 / 値引 8,560円 / 注文74件 / 点数85）
assert GROSSD['2026-09-04']==381300 and sum(S[n]['2026-09-04'][1] for n in S if '2026-09-04' in S[n])==8560
assert abs(ACCT(D1)-AD0904)<1, ('9/4 広告費 アカウント照合', ACCT(D1))
