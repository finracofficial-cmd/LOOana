# -*- coding: utf-8 -*-
"""[9/04終端に固定した窓ビルダー] 2026-09-06 定例（データ終端=9/04 金）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0904=257473  # 9/04 広告費（health_check 8/06-9/04 − CSV 8/06-9/03 の実測差）
...
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1120,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609,'温感EMSフェイシャルワンド':1866,'4-in-1マルチクリーナー':2961}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980,'温感EMSフェイシャルワンド':5980,'4-in-1マルチクリーナー':6980}
...
END='2026-09-04'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-06']
assert D1==['2026-09-04'] and D3[0]=='2026-09-02' and D7[0]=='2026-08-29', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-06', (len(D30),D30[0])
...
def qty(n,days,key):
    ...
    if n in VARC:
        ...
        CG={'ナノガラス脱毛パッド':{'白緑':VARC[n][0],'黒桃':VARC[n][1]},
            ...
            '温感EMSフェイシャルワンド':{'シルバー':VARC[n][0],'ピンク':VARC[n][1]}}
        q=0; cost=0; chk=0
        for d in days:
...
assert len(CUM)==4 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-04'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-04']==381300 and sum(S[n]['2026-09-04'][1] for n in S if '2026-09-04' in S[n])==8560
assert abs(ACCT(D1)-AD0904)<1, ('9/04 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0904w.pkl','wb'))
