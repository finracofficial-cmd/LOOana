# -*- coding: utf-8 -*-
"""2026-08-22 の定例。前日=2026-08-21。

売上・値引・バリアント = Shopify実測 ／ 広告費 = Meta実測（8/21まで揃った）
窓はすべて 8/21 終端。
"""
import csv, collections, datetime, runpy

g = runpy.run_path('scripts/daily/w0821.py')
...
print('■ ① 全体（すべて8/21終端・Shopify実測 × Meta実測）')
print(f'  {"窓":<12}{"実売":>12}{"原価":>11}{"広告費":>11}{"総合原価":>11}{"利益":>12}{"利益率":>8}'
      f'{"MER":>7}{"分岐":>7}{"余裕":>7}{"手数料後利益":>13}')
for lbl, N, K, days in [('前日8/21', N1, K1, D1), ('3日', N3, K3, D3), ('7日', N7, K7, D7),
...
print(f'\n  8/21（{wd(D0)}）vs 同曜日平均 {sa:,.0f}円 → {STORE[D0]/sa-1:+.1%}（n={len(same)}）'
