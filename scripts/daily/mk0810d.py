...
if not any(r[0]=='2026-08-10' for r in body):      # 既に8/07行があれば上書きしない（当日の変更を消さないため）
    for r in [x for x in body if x[0]=='2026-08-09']: body.append(['2026-08-10',r[1],r[2],r[3]])
with open('/home/user/LOOana/data/budget-snapshots.csv','w',newline='') as f:
    w=csv.writer(f); w.writerow(hdr); w.writerows(body)
BUD={r[2]:int(r[3]) for r in body if r[0]=='2026-08-10'}
TOTB=sum(BUD.values())
...
wb.save('/home/user/LOOana/data/reports/report-2026-08-10.xlsx')
