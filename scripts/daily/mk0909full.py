# -*- coding: utf-8 -*-
"""2026-09-09 定例レポート本体（データ終端=9/07 月／9/08の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0907w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-09'   # ★Meta 直読。温感EMSが PAUSED（−10,000）。カタログは18,000のまま
...
assert sum(BUD.values()) == 240000, sum(BUD.values())
...
assert sum(ORD.values()) == 692, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1169,'W固定スマホ車載ホルダー':521,'快適マジックインソール':274,'ムダ毛シェーバー':195,
'2WAYシートボックス':140,'形状記憶日傘':138,'偏光・調光サングラス':111,'バランスケアスリッパ':109,
'姿勢サポートチェア':98,'完全遮光・接触冷感UVハット':96,'接触冷感UVパーカー':96,'害虫ブロッカー':80,
'むくみ取りかっさ':72,'4WAY':67,'優先配送':52,'完全遮光・形状記憶':45,'接触冷感UVアームカバー':43,
'ナノバブルシャワーヘッド':37,'5WAY腰掛けファン':25,'携帯電動シェーバー':24,'3D足臭リセットブラシ':21,
'温感EMSフェイシャルワンド':15,'卓上冷感クーラー':10,'4-in-1マルチクリーナー':9,'壁掛けディスペンサー':6,
'ビジュアル耳かき':6,'ヘアドライタオル':4,'伸縮ガラスクリーナー':3,'ジェットウォッシャー':3,
'瞬間冷感ポンチョ':3,'健康サンダル':3,'癒しの指圧マット':2,'3WAYサーキュレーター':2,'ネックマッサージャー':1,
'UV歯ブラシ除菌器':1,'スマートノーズEMS美顔器':1,'姿勢サポートベルト':1,'1秒折り畳みチェア':1,
'湯上がりガーゼワンピース':1}
...
# ★2026-09-09 取得の health_check（窓 = 8/09-9/07 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2396588,1292078,38932),'W固定スマホ車載ホルダー':(1007551,501242,17228),
'カタログ全部（テスト）':(591556,148723,5873),'快適マジックインソール':(581847,357833,9972),
'ムダ毛シェーバー':(552175,446929,6439),'2WAYシートボックス':(371668,127163,4272),
'姿勢サポートチェア':(325725,203733,4669),'バランスケアスリッパ':(249528,174396,3758),
'むくみ取りかっさ':(166150,47192,1885),'ナノバブルシャワーヘッド':(117011,56746,1882),
'温感EMSフェイシャルワンド':(55084,13312,563),'4-in-1マルチクリーナー':(27168,7599,217),
'伸縮ガラスクリーナー':(2086,561,35)}
...
sh('シンプル判定', ['LOOTY 2026-09-09 定例（データ終端=9/07 月）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/07終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['🎉 脱出','**非常ブレーキを抜けました。3日移動平均利益 155,574円（床130,000）**',
  '9/01から6日続いた発動が止まりました。9/05 +178,738 ／ 9/06 +152,048 ／ 9/07 **+135,937（利益率28.4%）**。'
  '★9/07は 実売478,435 ÷ 広告 **217,505** = MER 2.20。広告費が前日から **−74,240円** 落ちています。'
  '★抜けた理由は明確で、**止めた4本（UVハット・サングラス・耳かき・3D足臭）の消化が消えたこと**です。'
  '売上ではなく支出側で戻しました',
  '—','記録'],
 ['🚨 執行','むくみ取りかっさ **10,000 → 7,500円（−25%）**（**3日連続で未実行**）',
  '3窓とも余裕<0.30（3日+0.18 ／ 5日 **−0.11** ／ 7日 **−0.06**）。'
  '7日(9/01-07) 実売106,664 / 27点 / 広告80,140 / 原価30,240 → **損益 −3,716円**。'
  '5日も −4,126円。**7日窓が赤字に転落しました**。今日こそ7,500へ',
  '+2,500円/日','本日'],
 ['🚨 執行','姿勢サポートチェア **9,000 → 6,750円（−25%）**（新規）',
  '3窓とも余裕<0.30 に入りました（3日+0.25 ／ 5日+0.10 ／ 7日 **−0.32**）＝宣言済みルールの発動。'
  '7日(9/01-07) 実売82,524 / 14点 / 広告73,613 / 原価25,354 → **損益 −16,443円**。'
  '★7日で1.6万円の赤字は今の全商品で最大です。9/05に12,000→9,000へ下げた分では止まっていません',
  '+2,250円/日','本日'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（**8日連続で未実行**）',
  'Meta直読で日予算は今も18,000円。9/07の消化は17,190円。'
  '★非常ブレーキは抜けましたが、**この減額は「ブレーキが理由」ではなく「カタログ自体が商品に紐づかず'
  '効率を測れないまま日1.7〜2万円使い続けている」ことが理由**です。'
  '3本合わせて **12,750円/日** の圧縮になります',
  '+8,000円/日','本日'],
 ['🔁 撤回','2WAYシートボックスの −25%（8,000→6,000）は **今日は発動条件を外れました**',
  '昨日は3窓とも余裕<0.30でしたが、今朝は **7日が+0.34** へ改善して条件から外れています'
  '（3日 −0.18 ／ 5日+0.21 ／ 7日 **+0.34**）。7日(9/01-07)の損益も **+11,423円**の黒字。'
  '★宣言済みルールに機械的に当てはめると「据え置き」です。**昨日の提案は取り下げます**。'
  'まだ実行していなければ、そのままで構いません。3日窓は−0.18なので監視は続けます',
  '—','取り下げ'],
 ['🚀 好調','**伸縮ガラスクリーナーが3窓とも増額候補ライン（MER 5.72・余裕+4.33）**',
  '初日9/07: 広告 **2,086円** で 3点11,940円 → MER5.72 vs 分岐1.40。9/08は **13点51,740円** と急伸。'
  '★**ただし増額はしません。** 理由は2つ: ①購入50件まで判定しない（宣言済み・現在16件）'
  '②**そもそも日予算8,000円のうち26%しか使えていない**（2,086円）。'
  '予算が天井になっていないので、増やしても配信は増えません。**Metaが自力で消化を伸ばすのを待つ段階**です',
  '—','監視'],
 ['✅ 実行確認','温感EMSフェイシャルワンドは **停止**されていました',
  '9/07は10,385円で稼働し、Metaでは PAUSED を確認。累計17点（注文15件）・3窓は 3日+0.37 / 5日+0.23 / 7日+0.20。'
  '★私は「20件到達時に余裕+0.30未満なら−25%」と宣言していましたが、'
  '**その前にユーザーが停止**しました。1日あたり利益が332円と薄かったので、判断としては妥当です。'
  '日予算10,000円が空きます',
  '+10,000円/日','完了'],
 ['📊 新商品3本の現在地','原価率が低いほど強い、が実際に出ています',
  '**伸縮ガラスクリーナー** 原価率28.3% / 分岐1.39 → **MER5.72・余裕+4.33**（16点）｜'
  '**4-in-1マルチクリーナー** 原価率42.4% / 分岐1.74 → MER2.31・余裕+0.58（10点）｜'
  '**温感EMS** 原価率31.2〜34.8% / 分岐1.50 → MER1.72・余裕+0.20（17点）→ **停止済み**。'
  '★クリーナー系2本が残りました。伸縮のほうが原価率も立ち上がりも良いので、'
  '**次に仕込むならこの系統**です',
  '—','記録'],
 ['📈 速報','9/08(火)は実売 **495,040円**',
  '注文95 / 販売数111 / 客単価 **5,211円**（9月に入ってからで最高）/ セッション2,739 / カート追加率6.02%。'
  '前週の火曜(9/01 435,402円)比 **+13.7%**。'
  '★**伸縮ガラスクリーナーが13点51,740円**で全体の10.4%を占めました。'
  '停止済みキャンペーンの商品（3D足臭3点・日傘2点・UV歯ブラシ1点ほか）も計45,820円オーガニックで売れています。'
  '返品1件3,980円(W固定)',
  '—','速報'],
 ['🔴 判定日','快適マジックインソール 再テスト',
  '基準「27,000戻しの7日利益 ≥ 27,000時代の7日平均」。3窓は余裕+0.83〜1.29と好調（3日 MER2.52）','—','明日 9/10'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
 ...
 ['🟡 推奨','クリーナー系をもう1本',
  '伸縮ガラスクリーナー（原価率28.3%・MER5.72）が今期いちばんの立ち上がり。'
  '温感EMS停止で日予算10,000円が空いたので、**同じ生活雑貨・低原価率の枠**に振り替える価値があります','—','優先'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/07(月)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/05-9/07', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/01-9/07', R['N7'], R['K7'], R['ACCT7']), ('30日 8/09-9/07', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-07', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/9','**非常ブレーキ 解除**','3日移動平均利益 ≥ 130,000円',
  '9/05 +178,738 / 9/06 +152,048 / 9/07 +135,937 → **平均 155,574円**','🎉 9/01からの発動が終了',
  '★抜けた理由は止めた4本（UVハット・サングラス・耳かき・3D足臭）の消化が消えたこと。売上ではなく支出側'],
 ['9/9','むくみ取りかっさ','3窓すべて 余裕<0.30 → −25%',
  '3日+0.18 ／ 5日 **−0.11** ／ 7日 **−0.06**。7日 実売106,664 / 27点 / 広告80,140 → **損益 −3,716円**',
  '🚨 −25% → 10,000を7,500へ','**3日連続で未実行**。7日窓が赤字に転落した'],
 ['9/9','姿勢サポートチェア','3窓すべて 余裕<0.30 → −25%',
  '3日+0.25 ／ 5日+0.10 ／ 7日 **−0.32**。7日 実売82,524 / 14点 / 広告73,613 → **損益 −16,443円**',
  '🚨 −25% → 9,000を6,750へ','★7日で1.6万円の赤字は現在の全商品で最大。9/05の減額では止まっていない'],
 ['9/9','2WAYシートボックス（昨日の提案を取り下げ）','3窓すべて 余裕<0.30 → −25%',
  '3日 −0.18 ／ 5日+0.21 ／ **7日+0.34**（昨日は+0.06）。7日の損益は **+11,423円の黒字**',
  '🔁 発動条件を外れた → 据え置き',
  '★昨日の−25%提案は取り下げる。まだ実行していなければそのままで構わない。3日窓は−0.18なので監視は続ける'],
 ['9/9','温感EMSフェイシャルワンド','宣言: 20件到達時に余裕+0.30未満なら −25%',
  '累計17点（注文15件）。3窓は 3日+0.37 / 5日+0.23 / 7日+0.20','✅ ユーザーが**停止**',
  '★20件を待たずに停止された。1日あたり利益332円と薄かったので妥当。日予算10,000円が空く'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均',
  '3窓は 3日+1.29 / 5日+0.92 / 7日+0.83 と好調','⏳ 明日が判定日','—'],
 ['—','伸縮ガラスクリーナー（新商品）','購入50件に達するまで判定しない',
  '9/07: 広告2,086円 / 3点11,940円 → **MER5.72 vs 分岐1.40・余裕+4.33**。9/08は13点51,740円。累計16件',
  '🚀 増額候補だが**増額しない**',
  '★理由2つ: ①50件まで判定しない（宣言済み）②**日予算8,000のうち26%しか消化していない**。'
  '予算が天井になっていないので増やしても配信は増えない'],
 ['—','4-in-1マルチクリーナー（新商品）','購入50件に達するまで判定しない',
  '累計10件。3窓とも MER2.31 vs 分岐1.74・余裕+0.58','⏸ 判定しない','20件到達時に中間チェック'],
 ['—','カタログ全部（テスト）','非常ブレーキ由来ではなく、単体の効率問題として',
  'Meta直読で日予算18,000のまま。9/07の消化17,190円。**8日連続で未実行**','🚨 18,000→10,000',
  '★ブレーキは解除されたが、**商品に紐づかず効率を測れないまま日1.7〜2万円使い続けている**のが理由'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★**非常ブレーキを抜けました**。3日移動平均利益 155,574円（床130,000）。9/01からの6日連続発動が終了',
  '★執行3件: ①**むくみ取りかっさ 10,000→7,500**（3日連続で未実行）②**姿勢サポートチェア 9,000→6,750**（新規）'
  '③**カタログ 18,000→10,000**（8日連続で未実行）。合わせて12,750円/日',
  '★取り下げ1件: **2WAYシートボックスの−25%は発動条件を外れました**（7日が+0.06→+0.34へ改善）',
  '★伸縮ガラスクリーナーが MER5.72（分岐1.40）と絶好調。ただし日予算8,000の26%しか使えていないので増額しない'],
