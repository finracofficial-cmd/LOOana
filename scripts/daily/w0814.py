...
# 2026-08-16: Metaが8/15を返さないため、意思決定の窓は8/14終端に固定する
END='2026-08-14'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-16']
assert D1==['2026-08-14'] and D3[0]=='2026-08-12' and D7[0]=='2026-08-08', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-16', (len(D30),D30[0])
...
assert len(CUM)==14 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-14'
...
assert sum(v[0] for n in S for d in CUM for v in [S[n][d]] if d in S[n])==10296944+926316+704468+676628+605384
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0814w.pkl','wb'))
