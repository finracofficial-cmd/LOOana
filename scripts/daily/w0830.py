...
AD0830=264342  # 8/30 広告費（health_check 30日 − CSV 29日 の実測差）
...
assert len(D30)==30 and D30[0]=='2026-08-01', (len(D30),D30[0])
...
assert len(CUM)==30 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-30'
...
# 8/30の縦照合（全店1クエリ実測 gross 607,150円 / 値引 12,743円 / 注文114件 / 点数133）
assert GROSSD['2026-08-30']==607150 and sum(S[n]['2026-08-30'][1] for n in S if '2026-08-30' in S[n])==12743
assert abs(ACCT(D1)-AD0830)<1, ('8/30 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0830w.pkl','wb'))
