# -*- coding: utf-8 -*-
"""2026-08-26 定例レポート本体（昨日=8/25 火）＋ 当日8/25 14:47時点の診断シート。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0825w.pkl', 'rb')); FEE = 0.03452
...
ORD = {'ナノガラス脱毛パッド':269,'W固定スマホ車載ホルダー':117,'快適マジックインソール':68,'形状記憶日傘':43,
'2WAYシートボックス':37,'ムダ毛シェーバー':37,'偏光・調光サングラス':31,'完全遮光・接触冷感UVハット':28,
'姿勢サポートチェア':25,'バランスケアスリッパ':16,'3D足臭リセットブラシ':15,'4WAY':14,'優先配送':13,
'むくみ取りかっさ':12,'接触冷感UVアームカバー':11,'完全遮光・形状記憶':9,'接触冷感UVパーカー':6,
'携帯電動シェーバー':4,'健康サンダル':1,'癒しの指圧マット':1,'湯上がりガーゼワンピース':1}
STORE_ORD7, STORE_SESS7 = 732, 22796
assert sum(ORD.values()) == 758, sum(ORD.values())
# --- 7日(8/19-25) Metaファネル（キャンペーン別実測: cost, impressions, link_clicks）---
MF_RAW = {'ナノガラス脱毛パッド':(559095,275278,6679),'W固定スマホ車載ホルダー':(242672,114130,3266),
'形状記憶日傘':(174845,41167,1708),'ムダ毛シェーバー':(130110,98590,1255),'快適マジックインソール':(128151,80456,1900),
'2WAYシートボックス':(96267,27087,847),'偏光・調光サングラス':(93058,37462,1065),'姿勢サポートチェア':(81836,47843,969),
'完全遮光・接触冷感UVハット':(67685,13780,662),'3D足臭リセットブラシ':(55043,12663,235),
'バランスケアスリッパ':(40887,25236,507),'4WAY 取り付けOK 小型瞬間冷却ハンディファン':(39110,9835,276),
'接触冷感UVアームカバー':(32287,7054,256),'むくみ取りかっさ':(24693,6214,223),
'接触冷感UVパーカー':(9133,2328,69),'1秒折り畳みチェア':(5704,1773,50),'ナノバブルシャワーヘッド':(2043,932,36)}
...
sh('シンプル判定', ['LOOTY 2026-08-26 定例（昨日=8/25 火）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて8/25終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 8/25(火)', R['N1'], R['K1'], R['ACCT1']), ('3日 8/23-25', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/19-25', R['N7'], R['K7'], R['ACCT7']), ('30日 7/27-8/25', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 8/01-25', R['NC'], R['KC'], R['ACCTC'])]:
...
ws.cell(r, 1, '■ 昨日(8/25 火) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','8/25(火)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
sh('商品別', ['商品別（7日 8/19-25 = 判定単位）。カタログ行の広告費は「7日合計」（前日ではない）',
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['8/26','むくみ取りかっさ 中間判定','RPM/CPM < 0.7 で停止（クリック100未満なら延期）',
  'クリック223件（100超）／ RPM/CPM **1.56** ／ 倍率1.56（一致）','✅継続。停止しない',
  '実CPA 2,058円 vs 分岐CPA 3,204円。CTR 3.59%×CVR 5.38%=19.31‱ vs 必要12.40‱。'
  '増額は見送り（MER 2.19 < 目標 2.78・3窓も未成立）'],
 ['8/26','3D足臭 CR「B」停止の確認','—',
  '8/25 A 8,272 / B 3,437 → 8/26 A 1,743 / **Bなし**','✅停止を確認',
  '宣言どおり 8/26-30 のA単独クリーン窓が開始。8/30に RPM/CPM ≥1.3 増額 ／ <1.0 停止 ／ 1.0〜1.3 据え置き'],
 ['8/27','快適マジックインソール 増額判定','1日あたり利益 ≥ 23,589円',
  '7日利益 155,869円 = 1日 22,267円','判定日に執行',
  'MER 2.70 > 目標 2.13 で🚀増額候補。ただし基準にわずかに届いていない'],
 ['8/27','ムダ毛シェーバー','8/24の 20,000→17,000 の効果判定',
  '7日利益 +24,824円 / MER 1.99 / 余裕 +0.32','判定日に執行',
  '倍率は前回1.10（全負け圏）。3窓は⏸のまま'],
 ['8/28','ナノガラス コピー修正判定','点数/注文 1.06 → 1.15',
  '7日実測 295個/269注文 = **1.10**','判定日に執行','8/25時点で1.07 → 1.10。動き始めている'],
 ['8/28','ナノバブルシャワーヘッド 中間ゲート','3日累計 Shopify実注文 ≥ 5件。下回れば停止',
  '8/25: 消化2,043円 / 36クリック / 注文0件（19時台開始の部分日）','進行中',
  '帰属は注文UTM（utm_campaign = 6934285209608）で測る。Metaの計上は使わない'],
 ['8/29','カタログ 減額判定','ラストクリック注文シェア < 消化シェアの0.8倍 → 10,000→5,000',
  '基準線: 減額前 6.0% vs 6.7% = 0.90倍','進行中',
  '⚠️ カタログは18,000のまま（14,000＋夏以外4,000）。宣言した10,000への減額は未実行'],
 ['8/30','4WAY・接触冷感UVアームカバー 停止判定','全店週次利益(8/24-30) ≥ 696,901円','進行中','—','—'],
 ['9/1','ナノバブルシャワーヘッド 本判定','倍率 ≥1.3 継続 ／ 1.0〜1.3 は−50% ／ <1.0 停止','進行中','—',
  '★増額は日10,000円を上限とする（規模を追うと壊れる構造）'],
 ['9/4','優先配送 790円','14日で件数 ≥ 28件','7日実測 13件','進行中','8/22に536→790へ値上げ済み'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★8/26のむくみ取りかっさは RPM/CPM 1.56 で継続。停止基準0.7を大きく上回った',
  '★3D足臭のCR「B」は停止済みを確認。8/26からA単独のクリーン窓が始まっている'],
...
'2026-08-22':3220,'2026-08-23':3438,'2026-08-24':3192,'2026-08-25':3280}
...
'2026-08-22':98,'2026-08-23':106,'2026-08-24':100,'2026-08-25':99}
CV = [[d, WDJ[datetime.date.fromisoformat(d).weekday()], SES[d], ORDD[d], round(ORDD[d]/SES[d], 4),
       '★昨日（確定値）' if d == '2026-08-25' else ''] for d in sorted(SES)]
o1 = sum(ORDD[d] for d in sorted(SES) if d <= '2026-08-16'); s1_ = sum(SES[d] for d in sorted(SES) if d <= '2026-08-16')
o2 = sum(ORDD[d] for d in sorted(SES) if '2026-08-17' <= d <= '2026-08-25'); s2 = sum(SES[d] for d in sorted(SES) if '2026-08-17' <= d <= '2026-08-25')
sh('CVR推移', ['全店CVR = Shopify注文 ÷ Shopifyセッション。商品別レポートのCVR（注文÷Metaクリック）とは分母が違うので混ぜないこと',
  ...
  '★8/25は日中に−27.6%まで落ちたが、確定値は 99注文/3,280セッション = 3.02%（8/24は3.13%）。単日の揺れだった',
  '★セッションは 5,000〜6,000/日 → 3,200〜3,500/日 へ減った。これは日予算 590,000→258,000 の直接の結果'],
 ['日付','曜','セッション','注文','全店CVR','備考'], CV, [12,5,12,10,10,20],
 {3:'#,##0',4:'#,##0',5:'0.00%'})

# ---------- 11 曜日指数 ----------
sh('曜日指数', ['直近30日(7/27-8/25)・祝日 8/11(山の日) を除外。売上=gross−値引',
...
wb.save('data/reports/report-2026-08-26.xlsx')
