# -*- coding: utf-8 -*-
"""2026-09-15 朝の判定材料。CSV実測のみ（w0913.py 経由）。"""
import runpy
g = runpy.run_path('scripts/daily/w0913.py')
...
GP = {'ナノガラス脱毛パッド': {'白緑': 3980, '黒桃': 3980},
      ...
      '伸縮ガラスクリーナー': {'ブルー': 3980, 'グレー': 3980, 'グリーン': 3980, 'レッド': 3980},
      '高見えレザーヘッドレストフック': {'ブラック': 3980, 'ブラウン': 3980, 'グレー': 3980},
      'もちふわ肉球サンダル': {'37-38': 3980, '39-40': 3980, '41-42': 3980, '43-44': 3980}}
...
def days_back(k, end='2026-09-13'):
...
print('\n■ 9/13に配信ゼロだったキャンペーン（7日窓に残消化があるもの）')
D7 = days_back(7)
for c in sorted(AD, key=lambda x: -sum(AD[x].get(d, 0) for d in D7)):
    a7 = sum(AD[c].get(d, 0) for d in D7); a1 = AD[c].get('2026-09-13', 0)
    if a7 > 0 and a1 == 0:
