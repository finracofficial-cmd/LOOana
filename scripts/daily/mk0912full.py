# -*- coding: utf-8 -*-
"""2026-09-12 定例レポート本体（データ終端=9/10 木／9/11の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0910w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-12'   # ★Meta 直読。前日から変更なし（合計244,000円）
...
'壁掛けディスペンサー':3,'偏光・調光サングラス':3,'完全遮光・形状記憶':2,'形状記憶日傘':2,
'ジェットウォッシャー':2,'湯上がりガーゼワンピース':1,'3D足臭リセットブラシ':1,'UV歯ブラシ除菌器':1,
'5WAY腰掛けファン':1,'スマートノーズEMS美顔器':1,'ビジュアル耳かき':1,'癒しの指圧マット':1,
'完全遮光・接触冷感UVハット':1,'卓上冷感クーラー':1}
STORE_ORD7, STORE_SESS7 = 707, 22103
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 718, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1126,'W固定スマホ車載ホルダー':507,'快適マジックインソール':315,'ムダ毛シェーバー':185,
'2WAYシートボックス':125,'バランスケアスリッパ':121,'形状記憶日傘':105,'姿勢サポートチェア':104,
'偏光・調光サングラス':86,'むくみ取りかっさ':77,'完全遮光・接触冷感UVハット':74,'ナノバブルシャワーヘッド':48,
'接触冷感UVパーカー':47,'4WAY':45,'優先配送':44,'害虫ブロッカー':43,'伸縮ガラスクリーナー':38,
'完全遮光・形状記憶':32,'接触冷感UVアームカバー':30,'3D足臭リセットブラシ':22,'携帯電動シェーバー':19,
'4-in-1マルチクリーナー':18,'温感EMSフェイシャルワンド':16,'5WAY腰掛けファン':10,
'高見えレザーヘッドレストフック':6,'壁掛けディスペンサー':6,'ビジュアル耳かき':6,'ジェットウォッシャー':4,
'ヘアドライタオル':4,'卓上冷感クーラー':3,'健康サンダル':3,'湯上がりガーゼワンピース':2,'瞬間冷感ポンチョ':2,
'癒しの指圧マット':2,'ネックマッサージャー':1,'1秒折り畳みチェア':1,'姿勢サポートベルト':1,
'UV歯ブラシ除菌器':1,'3WAYサーキュレーター':1,'スマートノーズEMS美顔器':1}
...
# ★2026-09-12 取得の health_check（窓 = 8/12-9/10 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2396026,1265701,37939),'W固定スマホ車載ホルダー':(1008451,466512,16504),
'快適マジックインソール':(669557,408362,11260),'カタログ全部（テスト）':(569400,143218,5548),
'ムダ毛シェーバー':(542892,436109,6286),'2WAYシートボックス':(342064,109922,3699),
'姿勢サポートチェア':(319507,192191,4587),'バランスケアスリッパ':(279752,192648,4335),
'むくみ取りかっさ':(180745,52807,2083),'ナノバブルシャワーヘッド':(152461,72557,2343),
'温感EMSフェイシャルワンド':(64858,15636,650),'4-in-1マルチクリーナー':(56495,15985,503),
'伸縮ガラスクリーナー':(43817,13782,733),'高見えレザーヘッドレストフック':(14907,3446,176)}
...
sh('シンプル判定', ['LOOTY 2026-09-12 定例（データ終端=9/10 木）',
...
NA = [
 ['🔴 判定日','伸縮ガラスクリーナーが **累計67点で50点を突破**。ただし増額は **明日9/13に1本化**します',
  '9/11に21点83,580円で、9/07からの累計が **67点**。宣言していた50点ラインを超えました。'
  '★**基準は両方とも満たしています**: ①7日MER **4.03** ≥ 目標MER 2.79 ②直近2日の消化率 **100%**'
  '（9/09 90% → 9/10 110%）。'
  '★それでも今日は動かしません。理由は **8,000→16,000 の増額判定を9/13に置いており、'
  '今日さらに上げると その窓が汚れる**からです（増額直後は重ねない＝宣言済みの原則）。'
  '**明日9/13に2つの判定をまとめて出し、合格なら 16,000 → 20,000（+25%）**とします',
  '—','明日 9/13'],
 ['📊 参考','その9/13判定の中間値は **基準の165%** です',
  '基準「16,000時代の1日あたり利益 ≥ 8,000時代（9/07-9/08）の +15,294円」。'
  '16,000時代の2日（9/09-9/10）は 実売116,216 / 30点 / 広告32,028 → 利益+50,588 ＝ **1日あたり +25,294円**。'
  '★MERは 5.72 → 4.98 → 3.52 → **3.72** と、倍増したわりに下げ止まっています。'
  '明日3日ぶん（9/09-9/11）が揃うので、そこで確定させます',
  '—','9/13'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（**11日連続で未実行**）',
  'Meta直読で日予算は今も18,000円。9/10の消化は15,640円。30日で **569,400円**。'
  '★これが唯一残っている未執行の宣言事項です。11日間で約18万円を、'
  '商品に紐づかず効率を測れないまま使っています。'
  '★伸縮ガラスクリーナーを20,000へ上げるなら、その原資はここから出せます',
  '+8,000円/日','本日'],
 ['📈 好転','3日移動平均利益が4日連続で床の上（155,574 → 135,156 → 135,415 → **140,071円**）',
  '9/08 +117,482 ／ 9/09 +152,825 ／ 9/10 **+149,905（利益率25.9%）**。'
  '9/10は 実売578,136 ÷ 広告282,752 = MER 2.04。'
  '★9月の当月累積（10日）は 実売4,994,989 / 利益 **1,197,920円（24.0%）**／1日あたり119,792円。'
  '8月の1日158,620円との差は、ほぼ**夏商品の消滅ぶん**です',
  '—','記録'],
 ['🎉 好調','姿勢サポートチェアが3日で MER4.25・余裕+2.77。9/09の取り下げが完全に正解でした',
  '3日 +2.77 ／ 5日 +1.40 ／ 7日 +1.38。9/09に11点、9/10に6点。'
  '★9/09に−25%（9,000→6,750）を出し、翌日データが変わったので取り下げた判断です。'
  '減額していたら、この回復を自分で潰していました',
  '—','記録'],
 ['✅ 改善','4-in-1マルチクリーナーが20点判定のあと持ち直しています',
  '昨日は 7日余裕 **+0.33**（基準+0.30ぎりぎり）でしたが、今朝は 3日 **+0.72** ／ 5日+0.54 ／ 7日 **+0.65**。'
  '9/10に6点41,880円、9/11も2点。'
  '★**据え置きの判断は妥当でした**。50点到達時に同じ基準（余裕<+0.30なら−25%）で再判定します',
  '—','50点で再判定'],
 ['🆕 好調','高見えレザーヘッドレストフックが9/11に **14点55,720円** と急伸（累計23点）',
  '9/09 3点 → 9/10 6点 → 9/11 **14点**。3窓とも MER2.24 vs 分岐1.55・余裕+0.70。'
  '★9/10の消化は10,974円で**日予算8,000円の137%**。すでに予算が天井になっています。'
  '**購入50件まで判定しません**が、伸縮ガラスクリーナーと同じ「車用品・低原価率」の筋で'
  '立ち上がりが良い。50点は9/13〜9/14に到達する見込みです',
  '—','50点で判定'],
 ['✅ 確認','停止済み3本は今朝も⚠️/🚨のままで、判断は正しかった',
  '**温感EMS** 3窓とも分岐割れ（−0.92/−0.02/−0.05）→ 🚨停止水準｜'
  '**2WAYシートボックス** 3窓とも分岐割れ（−0.12/−0.28/−0.14）→ 🚨停止水準｜'
  '**むくみ取りかっさ** 3窓とも余裕<0.30（−0.03/−0.09/+0.10）→ ⚠️。'
  '★9/10の残消化は かっさ74円・2WAY 67円・温感EMS 0円で、実害なく止まっています',
  '—','完了'],
 ['🚫 除外','快適マジックインソールが3日連続で「🚀増額候補」に出ていますが、**引き続き増額しません**',
  '3窓とも目標超え（余裕 3日+1.22 / 5日+1.13 / 7日+1.12）。数字だけ見れば魅力的です。'
  '★ですが8/30に 27,000→30,000 を実際に試して、増分は「広告+6,514円/日に対し売上−1,825円/日」＝'
  '**追加売上を検出できず**でした。宣言済みルール「直近の増額実験で増分が分岐を割った商品は除外」に該当します。'
  '★**再挑戦するなら、9/22（前回実験から3週間）以降**が妥当です。それまでは触りません',
  '—','9/22まで除外'],
 ['📈 速報','9/11(金)は実売 **505,174円**・客単価 **5,262円**',
  '注文96 / 販売数128 / セッション3,014 / カート追加率5.57%。前週の金曜(9/04 372,740円)比 **+35.5%**。'
  '★**新商品3本で37点**（伸縮21点・フック14点・4-in-1 2点）＝全体の28.9%。'
  '9/04は今回の谷の底だったので、伸び率が大きく見える点は割り引いて読んでください。返品なし',
  '—','速報'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
 ...
 ['🟡 推奨','車用品系をもう1本',
  'W固定スマホ車載ホルダー（分岐1.38）と 高見えレザーヘッドレストフック（分岐1.55）が両方立っています。'
  '**車用品は原価率が低く客層も安定**しているので、この筋でもう1本テストする価値があります','—','優先'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/10(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/08-9/10', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/04-9/10', R['N7'], R['K7'], R['ACCT7']), ('30日 8/12-9/10', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-10', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/13','伸縮ガラスクリーナー（2つの判定を1本化）',
  '①50点到達時: 7日MER ≥ 目標MER かつ 消化率 ≥95% ／ ②16,000時代の1日あたり利益 ≥ 8,000時代の +15,294円',
  '①は**両方クリア済み**（7日MER 4.03 ≥ 目標2.79／直近2日の消化率100%）。'
  '②の中間値は 9/09-9/10 で **1日あたり +25,294円＝基準の165%**',
  '⏳ 明日まとめて判定 → 合格なら 16,000→20,000',
  '★今日動かさないのは、8,000→16,000の判定窓を汚さないため（増額直後は重ねない＝宣言済み）'],
 ['9/12','カタログ全部（テスト）','非常ブレーキ由来ではなく単体の効率問題',
  'Meta直読で日予算18,000のまま。9/10の消化15,640円。30日で569,400円',
  '🚨 18,000→10,000（**11日連続で未実行**）',
  '★唯一残っている未執行の宣言事項。伸縮を20,000へ上げるなら原資はここから出せる'],
 ['9/12','快適マジックインソール（3日連続で増額候補だが除外）','3窓とも目標超え（+1.12〜+1.22）',
  '8/30の 27,000→30,000 は「広告+6,514円/日に対し売上−1,825円/日」で追加売上を検出できなかった',
  '🚫 9/22まで増額しない',
  '★宣言済みルール「直近の増額実験で増分が分岐を割った商品は除外」。'
  '**再挑戦は前回実験から3週間後の9/22以降**と、ここで期日を切っておく'],
 ['9/12','4-in-1マルチクリーナー（20点判定のフォロー）','20点時点では 7日余裕+0.33 で合格（ぎりぎり）',
  '今朝は 3日 **+0.72** ／ 5日+0.54 ／ 7日 **+0.65** へ改善','✅ 据え置きが妥当だった',
  '50点到達時に同じ基準（余裕<+0.30なら−25%）で再判定'],
 ['9/12','温感EMS／2WAYシートボックス／むくみ取りかっさ','停止済み3本の事後確認',
  '温感EMS 3窓とも分岐割れ（🚨）／2WAY 3窓とも分岐割れ（🚨）／かっさ 3窓とも余裕<0.30（⚠️）',
  '✅ 停止は正しかった','9/10の残消化は かっさ74円・2WAY 67円・温感EMS 0円で実害なし'],
 ['9/12','姿勢サポートチェア（9/09の取り下げの結果）','9/09に−25%を出し、翌日取り下げた',
  '3日 MER4.25・**余裕+2.77** ／ 5日+1.40 ／ 7日+1.38','🎉 完全に回復',
  '★減額していたらこの回復を自分で潰していた'],
 ['50点','高見えレザーヘッドレストフック','購入50件に達するまで判定しない',
  '累計23点（9/09 3点 → 9/10 6点 → 9/11 **14点**）。3窓とも MER2.24 vs 分岐1.55・余裕+0.70。'
  '9/10の消化10,974円＝**日予算8,000の137%**で既に天井','⏸ 9/13〜9/14に50点到達見込み',
  '伸縮ガラスクリーナーと同じ「車用品・低原価率」の筋'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★伸縮ガラスクリーナーが **累計67点**で50点を突破。基準は両方クリア（7日MER4.03 ≥ 目標2.79／消化率100%）',
  '★それでも今日は動かしません。**8,000→16,000の判定窓を汚さない**ため、'
  '**明日9/13に2つの判定を1本化**して出します（合格なら 16,000→20,000）',
  '★執行は1件だけ: **カタログ 18,000→10,000**（11日連続で未実行・唯一残っている宣言事項）',
  '★快適マジックインソールが3日連続で増額候補ですが、**9/22まで除外**（8/30の実験が否定されているため）',
  '★3日移動平均利益は4日連続で床の上（140,071円）。姿勢サポートチェアは3日 MER4.25 まで回復しました'],
...
for r in body:
    if r[0] == '2026-09-11': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
