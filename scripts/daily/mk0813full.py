...
MF={}   # Supermetrics MCPが8/13朝に停止中のため7日ファネルは取得不可（Shopify実測のみで構成）
...
'5WAY腰掛けファン':42,'完全遮光・接触冷感UVハット':40,'優先配送':28,'接触冷感UVアームカバー':23,
'姿勢サポートチェア':22,'携帯電動シェーバー':17,'卓上冷感クーラー':14,'バランスケアスリッパ':11,'瞬間冷感ポンチョ':9,
'3WAYサーキュレーター':4,'壁掛けディスペンサー':2,'ナノバブルシャワーヘッド':2,'瞬間冷却ハンディファン':1,
'UV歯ブラシ除菌器':1,'健康サンダル':1,'リカバリーサンダル':1,'快適マジックインソール':1,'湯上がりガーゼワンピース':1}
...
sh('シンプル判定',['LOOTY 2026-08-13 定例（昨日=8/12 水）',
...
# ---------- 2 ネクストアクション ----------
NA=[['🔴 必須','CJ原価の確認2件','①車載シートヒーター(目標1,400円以下) ②電動スカルプブラシ(目標900円以下)。新商品パイプラインが空','9月末の崖 週505,088円','未着手'],
    ...
    ['✅ 判定済','2WAYシートボックス','本判定 合格。CPA(注文)1,615円 < 分岐3,725円・RPM/CPM 2.22（基準1.3）→ 増額は8/13にユーザー実施済(15,000→18,000)','7日利益+99,635円','完了'],
    ['✅ 判定済','害虫ブロッカー','秋CR判定 不合格。新CR3本すべて既存に負け、秋CRは本体消化の1.2%のみ→ 素材では直らないと確定','—','完了'],
    ['⏸ 延期','偏光・調光サングラス','LP判定（7日CVR≥3.0%）はSupermetrics停止でクリック数が取れず延期。CPA(注文)2,643円 < 分岐3,528円で経済性は健全のため据え置き','—','復旧後すぐ'],
    ['🟡 推奨','接触冷感UVパーカー','売価 3,980→4,480円。CTR3.30%・CVR4.48%は健全で、粗利2,546円が薄いだけ','週+45,224円【推計】','8/13'],
    ['👀 様子見','ナノガラス脱毛パッド','3窓すべて目標超・消化96/101/101%で+25%の条件を満たす。8/12のCVR2.26%が単日か2日連続かを見てから','週+47,682円','8/14'],
    ['👀 様子見','害虫ブロッカー','8/12の減額(37,000)後の3窓を見る。3日MER<1.28で停止/−50%、余裕<0.30で−25%、それ以上は据え置き','—','8/15'],
    ['👀 様子見','数量割引','7日 点数/注文≥1.25（現1.208）→ 不合格見通し','—','8/14'],
    ['👀 様子見','バランスケアスリッパ','新素材D 第2関門（外部CTR≥2.00%かつ売上/クリック≥163円）','—','8/14'],
    ['👀 様子見','快適マジックインソール','中間判定（CTR<2.2% or RPM/CPM<0.7で停止）。本判定8/19','—','8/15'],
    ['👀 様子見','卓上冷感クーラー','3日MER<1.53で停止','—','8/15'],
    ['👀 様子見','W固定スマホ車載ホルダー','週次ランプ判定 W-1(8/2-8) vs W0(8/9-15)・増分MER≥2.88','—','8/16'],
    ['👀 様子見','日傘2種','完全遮光のCPMが形状記憶の1.25倍以上のまま かつ余裕<+0.30なら完全遮光を停止','—','8/18'],
    ['👀 様子見','5WAY腰掛けファン','監視トリガー 3日MER<1.64 or 7日MER<2.00（現 1.81/2.23＝未発火）','—','監視'],
    ['👀 様子見','接触冷感UVアームカバー','監視トリガー 3日MER<1.26（現1.64＝未発火）','—','監視']]
sh('ネクストアクション',['今日やること・判定カレンダー・持ち越しタスク（実施が確認できるまで毎日残す）'],
   ['区分','商品/対象','内容','効果','期限'],NA,[12,26,58,20,10])
# ---------- 3 全体サマリー ----------
SUMR=[]
for lbl,N,K,ACC in [('前日 8/11(火・祝)',R['N1'],R['K1'],R['ACCT1']),('3日 8/09-11',R['N3'],R['K3'],R['ACCT3']),
        ('7日 8/05-11',R['N7'],R['K7'],R['ACCT7']),('30日 7/13-8/11',R['N30'],R['K30'],R['ACCT30']),
...
sh('曜日指数',['直近30日(7/12-8/10)・祝日7/20除外。売上=gross−値引',
  ...
  '★2026-08-12(水) 売上674,797円は非祝日水曜平均1,199,611円の58.7%・前週水曜(8/05)比81.0%。単日ではなく3週トレンド（7日MER 2.38→2.16→1.99）',
  '★Supermetrics MCP が本日朝より停止（Anthropic Proxy: Invalid content from server）。Meta側の配信指標（CTR/CPM/クリック）は取得不可。広告費は8/12取得済みのCSVで確定',
  '★曜日指数は 7/20(海の日)・8/11(山の日) を除外して算出'],
 ['曜日','指数(全体=100)'],[[k,round(v,1)] for k,v in sorted(R['IDX'].items(),key=lambda x:-x[1])],[10,16])
wb.save('data/reports/report-2026-08-13.xlsx')
