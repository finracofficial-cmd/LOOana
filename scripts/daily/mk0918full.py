# -*- coding: utf-8 -*-
"""2026-09-18 定例レポート本体（昨日=9/17 木）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0917w.pkl', 'rb'))
...
ORD = {'ナノガラス脱毛パッド':257,'伸縮ガラスクリーナー':126,'W固定スマホ車載ホルダー':118,
...
ORD30 = {'ナノガラス脱毛パッド':1134,'W固定スマホ車載ホルダー':491,'快適マジックインソール':376,
...
MF_RAW = {'ナノガラス脱毛パッド':(2396232,1228660,31093),'W固定スマホ車載ホルダー':(988229,396347,12134),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-18（昨日 = 9/17 木）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-17・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-18 定例（昨日=9/17 木）',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/17(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/15-17', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/11-17', R['N7'], R['K7'], R['ACCT7']), ('30日 8/19-9/17', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-17', R['NC'], R['KC'], R['ACCTC'])]:
...
ws.cell(r, 1, '■ 昨日(9/17 木) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','9/17(木)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/17(木)は実売487,748円・連休前軟調3日目だが昼までの−21%を夜が取り返した（日内回復型）。本日の判定4本でガラス27,000とフック15,000が合格確定、カタログは9,000へ減額提案、電動かっさは停止提案＝負け枠の整理が完了する。3日移動平均121,839円で非常ブレーキが初発動したが、対処はこの判定4本そのもの。9/19からは連休モード（週末扱い）で運転し、9/24-25のCVR回復が答え合わせ').font = Font(name='Arial', size=10, color='CC0000')
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/19-9/17)」。他シートの7日窓と混ぜて読まないこと',
...
for r in body:
    if r[0] == '2026-09-17b': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
...
sh('予算見直しチェック', ['前日(9/17b=リバランス後)スナップショット → 本日(9/18)',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/19-9/17)。売上=gross−値引。次回定期更新は 9/21(月)',
