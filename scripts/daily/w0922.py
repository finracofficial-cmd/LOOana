# -*- coding: utf-8 -*-
"""[9/22終端の窓ビルダー] 2026-09-23 定例（昨日=9/22 火・国民の休日 実績）。実測のみ・毎回ゼロから積み直す。原価は9/20反映のCJ最新表（再調達原価ベース）。"""
import csv, collections, datetime, json, pickle
AD0922=240094  # キャンペーン合算（アカウント240,009と差85円=取得時刻ドリフト）※翌朝再確認。予算256,000の94%＝週内収束
...
'携帯電動シェーバー':1764,'バランスケアスリッパ':1304,'湯上がりガーゼワンピース':1968,'UV歯ブラシ除菌器':3240,
'優先配送':0,'瞬間冷却ハンディファン':2053,'完全遮光・接触冷感UVハット':1411,'ネックマッサージャー':3034,
'リカバリーサンダル':1309,'ナノバブルシャワーヘッド':2255,'姿勢サポートベルト':1429,'癒しの指圧マット':1648,
'スマートノーズEMS美顔器':2453,'W固定スマホ車載ホルダー':1069,'ジェットウォッシャー':1259,'ビジュアル耳かき':1625,'姿勢サポートチェア':1811,
'2WAYシートボックス':1547,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'むくみ取りかっさ':1149,'温感EMSフェイシャルワンド':2079,'4-in-1マルチクリーナー':2961,'ヘアドライタオル':859,'1秒折り畳みチェア':1455,'3D足臭リセットブラシ':598,'伸縮ガラスクリーナー':988,'高見えレザーヘッドレストフック':1344,'もちふわ肉球サンダル':1165,'電動温熱カッサ':2483}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'むくみ取りかっさ':3980,'温感EMSフェイシャルワンド':5980,'4-in-1マルチクリーナー':6980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980,'伸縮ガラスクリーナー':3980,'高見えレザーヘッドレストフック':3980,'もちふわ肉球サンダル':3980,'電動温熱カッサ':6980}
...
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2604,2114)}
...
END='2026-09-22'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=ALLD[-30:]
assert D1==['2026-09-22'] and D3[0]=='2026-09-20' and D7[0]=='2026-09-16', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-24', (len(D30),D30[0])
...
def qty(n,days,key):
    ...
    if n in VARC:
        ...
        CG={'ナノガラス脱毛パッド':{'白緑':VARC[n][0],'黒桃':VARC[n][1]},
            '壁掛けディスペンサー':{'3本':VARC[n][0],'2本':VARC[n][1]}}
        q=0; cost=0; chk=0
        for d in days:
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（22日）
assert len(CUM)==22 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-22'
...
# 9/22の縦照合（全店1クエリ実測 gross 638,980円 / 値引 29,215円 / 注文117件 / 点数151）
assert GROSSD['2026-09-22']==638980 and sum(S[n]['2026-09-22'][1] for n in S if '2026-09-22' in S[n])==29215
assert abs(ACCT(D1)-AD0922)<1, ('9/22 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0922w.pkl','wb'))
