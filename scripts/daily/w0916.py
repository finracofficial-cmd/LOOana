# -*- coding: utf-8 -*-
"""[9/16終端に固定した窓ビルダー] 2026-09-18 定例（データ終端=9/16 水）。実測のみ・毎回ゼロから積み直す。"""
import csv, collections, datetime, json, pickle
AD0916=246536  # 9/16 広告費（health_check 8/18-9/16 − CSV 8/18-9/15 の実測差）
...
END='2026-09-16'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-08-18']
assert D1==['2026-09-16'] and D3[0]=='2026-09-14' and D7[0]=='2026-09-10', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-18', (len(D30),D30[0])
...
assert len(CUM)==16 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-16'   # 9月に入ったので当月累積は9/01から
...
assert GROSSD['2026-09-16']==465530 and sum(S[n]['2026-09-16'][1] for n in S if '2026-09-16' in S[n])==10547
assert abs(ACCT(D1)-AD0916)<1, ('9/16 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0916w.pkl','wb'))
