# -*- coding: utf-8 -*-
"""2026-08-03 定例（昨日=8/02 日実績）。実測のみ・毎回ゼロから積み直す。"""
...
'スマートノーズEMS美顔器':5980,'W固定スマホ車載ホルダー':3980,'ジェットウォッシャー':4980,'姿勢サポートチェア':7980}
...
VAR={'ナノガラス脱毛パッド':{'d1':(40,21),'d3':(110,50),'d7':(220,94),'d30':(790,351)},   # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(4,0),'d7':(6,1),'d30':(70,24)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d1':16,'d3':32,'d7':62,'d30':155},          # 7/27に5,980→6,980
      '湯上がりガーゼワンピース':{'d1':0,'d3':1,'d7':4,'d30':97},        # 窓内に4,980期がある
      '完全遮光・接触冷感UVハット':{'d1':1,'d3':6,'d7':6,'d30':14},      # 7/31に5,980→4,980
      '3WAYサーキュレーター':{'d1':7,'d3':16,'d7':42,'d30':185}}         # 8/2に5,980→6,980（当日は新旧混在）
HOL={'2026-07-20'}
...
D30=[d for d in ALLD if d>='2026-07-04']
assert D1==['2026-08-02'] and D7[0]=='2026-07-27' and D3[0]=='2026-07-31', (D1,D7[0],D3[0])
assert len(D30)==30 and D30[0]=='2026-07-04', (len(D30),D30[0])
...
print()
SUM={}
for lbl,days,N,K in [('前日(8/02 日)',D1,N1,K1),('3日(7/31-8/02)',D3,N3,K3),
                     ('7日(7/27-8/02)',D7,N7,K7),('30日(7/04-8/02)',D30,N30,K30)]:
...
# 同曜日平均（土曜・祝日除外）
SAT=[d for d in D30C if wd(d)=='日' and d not in HOL]
sat=sum(STORE[d] for d in SAT)/len(SAT)
print(f'\n日曜平均(直近30日 n={len(SAT)}): {sat:,.0f}円 / 8/02実績 {STORE["2026-08-02"]:,}円 = {STORE["2026-08-02"]/sat*100:.1f}%')
BR={};TG={}
...
print('\n'+'='*128)
print('■ 商品別（7日=7/27-8/02 / 3日=7/31-8/02 / 前日=8/02 / 30日=7/04-8/02。すべて実測）')
print('='*128)
...
for n in sorted(N7, key=lambda x:-N7[x]):
    ...
    bud=BUD.get(MAP.get(n,n),{}).get('2026-08-03')
...
pickle.dump(dict(rows=rows,N1=N1,N3=N3,N7=N7,N30=N30,K1=K1,K3=K3,K7=K7,K30=K30,Q1=Q1,Q7=Q7,
  ...
  open('/tmp/claude-0/-home-user-LOOana/42be2ed9-9967-56df-ab9c-75166f45bbc9/scratchpad/rep0803.pkl','wb'))
