...
# --- 7日(8/21-27) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD = {'ナノガラス脱毛パッド':269,'W固定スマホ車載ホルダー':126,'快適マジックインソール':84,'ムダ毛シェーバー':48,
'2WAYシートボックス':43,'偏光・調光サングラス':25,'形状記憶日傘':24,'バランスケアスリッパ':23,
'完全遮光・接触冷感UVハット':22,'むくみ取りかっさ':21,'姿勢サポートチェア':20,'3D足臭リセットブラシ':18,
'4WAY':8,'優先配送':7,'ナノバブルシャワーヘッド':7,'完全遮光・形状記憶':7,'接触冷感UVアームカバー':5,
'携帯電動シェーバー':3,'健康サンダル':1,'ヘアドライタオル':1,'湯上がりガーゼワンピース':1}
STORE_ORD7, STORE_SESS7 = 748, 22128
assert sum(ORD.values()) == 763, sum(ORD.values())
# --- 7日(8/21-27) Metaファネル（キャンペーン別実測: cost, impressions, link_clicks）---
MF_RAW = {'ナノガラス脱毛パッド':(563766,257701,6754),'W固定スマホ車載ホルダー':(246922,108482,3374),
'快適マジックインソール':(144430,91604,2166),'ムダ毛シェーバー':(126067,94984,1222),
'カタログ全部（テスト）':(119884,28756,1173),'形状記憶日傘':(108765,26510,1102),
'2WAYシートボックス':(93328,26342,874),'姿勢サポートチェア':(82270,45015,972),
'偏光・調光サングラス':(73866,29028,893),'3D足臭リセットブラシ':(68914,18335,306),
'完全遮光・接触冷感UVハット':(57837,11669,592),'バランスケアスリッパ':(51058,29933,638),
'むくみ取りかっさ':(44742,11090,427),'4WAY 取り付けOK 小型瞬間冷却ハンディファン':(21475,5459,160),
'ナノバブルシャワーヘッド':(19539,8230,300),'接触冷感UVアームカバー':(15299,3240,101),
'カタログ全部（テスト） 夏以外':(6080,1903,62)}
_mfc = sum(v[0] for v in MF_RAW.values())
# CSV(日次書き込み時の確定値)は1,843,499円。Metaは翌日以降もわずかに確定ドリフトするので±1%で照合する
assert abs(_mfc/1843499 - 1) < 0.01, (_mfc, _mfc/1843499 - 1)
...
sh('シンプル判定', ['LOOTY 2026-08-28 定例（昨日=8/27 木）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて8/27終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['🎉 変化点','非常ブレーキの上を2日連続で維持',
  f'3日移動平均利益 {p3ma:,.0f}円 ≧ 発動ライン130,000円。8/18-25の9日連続割れ → 8/26に136,211で上抜け → 8/27は156,354で2日連続ライン上。'
  '前日利益 +194,334円・利益率29.7% は今月の最高値。実売653,692円は7日平均比 +22.5%',
  '—','毎日'],
 ['🔴 判定日','ナノガラス コピー修正判定 → 🚨不合格',
  '宣言済み基準「点数/注文 ≥ 1.15」に対し 7日実測 301点 ÷ 269注文 = **1.119**。'
  '推移は 8/26朝1.100 → 8/27朝1.113 → 本日1.119 で上向きだが、基準には届かなかった。'
  '基準は宣言時のまま動かさない（後付けで1.10に下げない）',
  '—','本日'],
 ['🔴 判定日','ナノバブルシャワーヘッド 中間ゲート → ✅合格',
  '宣言済み基準「3日累計 Shopify実注文 ≥ 5件」に対し 8/25 0件 / 8/26 2件 / 8/27 5件 = **7件**。'
  '3日 実売48,860・広告19,530・利益+13,545・MER 2.50（分岐1.48）／ 実CPA 2,790円 vs 分岐CPA 4,725円 = 倍率1.69。'
  '日8,000円のまま9/1の本判定へ進む',
  '—','本日'],
 ['⏸ 前倒ししない','3D足臭リセットブラシ（3窓すべて分岐割れ）',
  '3窓判定は 3日0.81 / 5日1.01 / 7日1.09 vs 分岐1.18 で機械的には「停止/−50%」。'
  'ただし **8/30のA単独判定（8/26-30クリーン窓）を宣言済み**なので、宣言済み基準を優先して前倒し停止はしない。8/30に執行する',
  '—','8/30に執行'],
 ['⏸ 前倒ししない','快適マジックインソール（3窓すべて目標超え）',
  '3窓とも MER 2.93〜3.05 で目標2.13を超えている。ただし 8/27に22,000→27,000へ増額済みで、'
  '**8/31の増額判定（P2=8/28-30の1日あたり利益 ≥ 28,310円）を宣言済み**。重ねての増額はしない',
  '—','8/31に執行'],
 ['⚠️ 当日調査','8/28朝の不調はどこで起きているか',
  '11:45時点の実測。Meta配信は無罪（CPM 2,297円=7日2,310円 / link CTR 2.82%=7日2.64%より上 / CPC 88円=7日92円より安い）。'
  'Shopifyも取り込み遅れを避けたJST 0:00-8:59でセッション840（前日823）・カート追加率7.02%（前日7.05%）で正常。'
  '落ちているのは カート→購入 のみ（35.6% vs 46.6%）だが z=1.21 で**有意差なし**。'
  '11:30時点の注文32件も過去7日同時刻平均41.1±5.4に対し z=−1.68 で通常のばらつき',
  '—','監視のみ'],
 ['🔴 判定日','カタログ 減額判定（★2基準が逆の答えを出す）',
  '宣言済み基準A「注文シェア ÷ 消化シェア < 0.8 → 減額」。8/27単日は 6/130 ÷ 20,248/283,342 = **0.65倍**で減額側。'
  '一方 基準B「CPA(注文) vs 全店の1注文あたり粗利」ではカタログは黒字。'
  '★どちらを優先するかをユーザーが8/29までに決める必要がある（私の推奨はCPA基準を優先、share基準は監視指標へ格下げ）。'
  '⚠️ 宣言済みの非常ブレーキ「カタログ 18,000 → 10,000」は依然として未実行','—','8/29'],
 ['🔴 判定日','3D足臭 CR A単独判定','8/26-30のクリーン窓。RPM/CPM ≥1.3 増額 ／ <1.0 停止 ／ 1.0〜1.3 据え置き','—','8/30'],
 ['🔴 判定日','4WAY・接触冷感UVアームカバー 停止判定',
  '宣言済み基準「全店週次利益(8/24-30) ≥ 696,901円」。8/24-27の4日で **602,968円**（7日換算 1,055,194円）で大きく上回るペース。'
  '両方とも8/23が最終消化で停止済み','—','8/30'],
 ['🔴 判定日','快適マジックインソール 増額判定',
  'P1=8/24-26（予算22,000）の1日あたり利益 **31,704円** が確定。P2=8/28-30（予算27,000）で **28,310円以上**なら合格。'
  '8/27は変更当日なので両窓から除外','—','8/31'],
 ['🔴 判定日','ナノバブルシャワーヘッド 本判定',
  '倍率 ≥1.3 継続 ／ 1.0〜1.3 は−50% ／ <1.0 停止。★増額は日10,000円が上限（3〜5月にリーチを2倍に広げて壊した実測がある）','—','9/1'],
 ['🔴 判定日','優先配送 790円',
  '宣言済み基準「14日で件数 ≥ 28件」。7日実測 **7件**（アタッチ率 7/748 = 0.94%）。前週13件・1.78%から半減し、8/27は0件。'
  'このペースだと14日で14件前後にしかならず不合格の見込み','—','9/4'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 8/27(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 8/25-27', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/21-27', R['N7'], R['K7'], R['ACCT7']), ('30日 7/29-8/27', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 8/01-27', R['NC'], R['KC'], R['ACCTC'])]:
...
sh('全体サマリー', ['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '縦照合3本すべて通過: Σ商品gross=全店670,020 ／ Σ値引=16,328 ／ Σキャンペーン広告費=アカウント実消化283,342円（差0円）',
...
ws.cell(r, 1, '■ 昨日(8/27 木) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for c, v in enumerate(['指標','8/27(木)','3日平均/日','7日平均/日','30日平均/日','vs7日平均'], 1): ws.cell(r, c, v)
for c in range(1, 7):
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '⚠️ 「同曜日平均比 −20.8%」は読まないこと。直近30日の木曜（7/30・8/6・8/13・8/20）が夏物崩壊前で汚染されている（n=4）。'
  f'実態は7日平均比 **+22.5%** でこの窓の最高。3日移動平均利益 {p3ma:,.0f}円 は非常ブレーキ130,000円を'
  '2日連続で上回っている').font = Font(name='Arial', size=10, color='CC0000')
...
sh('商品別', ['商品別（7日 8/21-27 = 判定単位）。カタログ行の広告費は「7日合計」（前日ではない）',
  '分岐CPA(注文) = 1個あたり粗利 × 点数/注文。まとめ買い商品（点数/注文>1.2）は必ず注文ベースで比較する',
  'Σ商品広告費 + カタログ = Metaアカウント7日消化 1,843,499円（差0円・w0827.pyでassert済み）',
  '★ナノガラスの点数/注文は 301/269 = 1.119。本日のコピー修正判定（基準1.15）は不合格',
  '★3D足臭の点数/注文は 19/18 = 1.06。まとめ買いはほぼ起きていない'],
...
end = datetime.date(2026, 8, 26); WK = []
for i in range(6):
...
sh('週次診断', ['全店の週次分解。原価は日次のバリアント構成が取れないナノガラス/ディスペンサーのみ30日実測ミックスの加重平均単価【近似】',
  ...
  '★最新週(8/20-26)は日予算250,000〜256,000で運転。日予算590,000だった7月とは水準が違うので、利益額の絶対比較はしない'],
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['8/28','ナノガラス コピー修正判定','点数/注文 ≥ 1.15',
  '7日(8/21-27) 301点 ÷ 269注文 = **1.119**','🚨不合格',
  '8/26朝1.100 → 8/27朝1.113 → 本日1.119。方向は正しいが基準に未達。基準は宣言時のまま動かさない'],
 ['8/28','ナノバブルシャワーヘッド 中間ゲート','3日累計 Shopify実注文 ≥ 5件。下回れば停止',
  '8/25 0件 / 8/26 2件 / 8/27 5件 = **7件**','✅合格 → 日8,000円で継続',
  '3日 MER 2.50（分岐1.48）／ 実CPA 2,790円 vs 分岐CPA 4,725円 = 倍率1.69。9/1の本判定へ進む'],
 ['8/29','カタログ 減額判定','注文シェア ÷ 消化シェア ≥ 0.8（3日累計）',
  '8/27単日 6/130 ÷ 20,248/283,342 = **0.65倍**','進行中（★基準が衝突）',
  '★share基準は減額、CPA基準は黒字と逆の答えを出す。どちらを優先するかを8/29までにユーザーが決める。'
  '私の推奨はCPA基準を優先しshare基準は監視指標へ格下げ。⚠️非常ブレーキ由来の18,000→10,000も未実行'],
 ['8/30','3D足臭 CR A単独判定','RPM/CPM ≥1.3 増額 ／ <1.0 停止 ／ 1.0〜1.3 据え置き',
  '8/26-30がクリーン窓。3窓判定は既に3日0.81 / 5日1.01 / 7日1.09 vs 分岐1.18 で全割れ','進行中',
  '★機械的には今日「停止/−50%」だが、8/30判定を宣言済みなので前倒ししない'],
 ['8/30','4WAY・接触冷感UVアームカバー 停止判定','全店週次利益(8/24-30) ≥ 696,901円',
  '8/24-27 の4日で **602,968円**（7日換算 1,055,194円）','✅ペース上は大きく上回る','両方とも8/23が最終消化'],
 ['8/31','快適マジックインソール 増額判定','P2(8/28-30)の1日あたり利益 ≥ 28,310円',
  'P1(8/24-26・予算22,000) 実売193,826 / 52個 / 広告63,511 / 利益+95,111 = **1日31,704円**','進行中',
  '8/27は22,000→27,000の変更当日なので両窓から除外。3窓ともMER 2.93〜3.05で目標2.13超だが重ねての増額はしない'],
 ['9/1','ナノバブルシャワーヘッド 本判定','倍率 ≥1.3 継続 ／ 1.0〜1.3 は−50% ／ <1.0 停止',
  '8/25-27の倍率 **1.69**','進行中',
  '★増額は日10,000円が上限（3〜5月にリーチを71,205→145,211人へ広げて壊した実測がある）'],
 ['9/4','優先配送 790円','14日で件数 ≥ 28件','7日実測 **7件**（アタッチ率 7/748 = 0.94%）','🚨不合格の見込み',
  '前週13件・1.78%から半減。8/27は0件。8/22に536→790へ値上げ済み'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は2件: ナノガラス コピー修正=🚨不合格（1.119 < 1.15）／ ナノバブル 中間ゲート=✅合格（7件 ≥ 5件）',
  '★3D足臭とインソールは3窓判定が動くシグナルを出しているが、いずれも宣言済みの判定日（8/30・8/31）を優先し前倒ししない'],
...
CV = [[d, WDJ[datetime.date.fromisoformat(d).weekday()], SES[d], ORDD[d], round(ORDD[d]/SES[d], 4),
       '★昨日（確定値）' if d == '2026-08-27' else ''] for d in sorted(SES)]
o1 = sum(ORDD[d] for d in sorted(SES) if d <= '2026-08-16'); s1_ = sum(SES[d] for d in sorted(SES) if d <= '2026-08-16')
o2 = sum(ORDD[d] for d in sorted(SES) if '2026-08-17' <= d <= '2026-08-27'); s2 = sum(SES[d] for d in sorted(SES) if '2026-08-17' <= d <= '2026-08-27')
sh('CVR推移', ['全店CVR = Shopify注文 ÷ Shopifyセッション。商品別レポートのCVR（注文÷Metaクリック）とは分母が違うので混ぜないこと',
  ...
  f'★8/17-27（広告費を絞った後）: {o2}注文 / {s2:,}セッション = {o2/s2:.2%} → 広告費を削ったらCVRは **{o2/s2/(o1/s1_)-1:+.1%}** 改善している',
  '★8/27は セッション3,145に対し注文130件 = **CVR 4.13%** でこの窓の最高（前日の8/26 3.84%を更新）',
  '★セッションは 5,000〜6,000/日 → 2,700〜3,400/日 へ減った。これは日予算 590,000→250,000 の直接の結果'],
 ['日付','曜','セッション','注文','全店CVR','備考'], CV, [12,5,12,10,10,20],
 {3:'#,##0',4:'#,##0',5:'0.00%'})

# ---------- 11 曜日指数 ----------
sh('曜日指数', ['直近30日(7/29-8/27)・祝日 8/11(山の日) を除外。売上=gross−値引',
  '★同曜日平均との比較は当面使わない。直近30日の木曜（7/30・8/6・8/13・8/20）が夏物崩壊前で汚染されているため',
  '★代わりに「7日平均比」で読む。8/27は **+22.5%**（同曜日平均比では−20.8%だが、これは分母が汚染されている）'],
 ['曜日','指数(全体=100)'], [[k, round(v, 1)] for k, v in sorted(R['IDX'].items(), key=lambda x: -x[1])], [10, 16])

wb.save('data/reports/report-2026-08-28.xlsx')
