# -*- coding: utf-8 -*-
"""[8/16終端に固定した窓ビルダー] 2026-08-17 定例（昨日=8/16 日 実績）。実測のみ・毎回ゼロから積み直す。"""
...
'2WAYシートボックス':1943,'快適マジックインソール':677,'ヘアドライタオル':985}
...
'2WAYシートボックス':4980,'快適マジックインソール':3980,'ヘアドライタオル':3980}
...
# 2026-08-17: 8/16まで揃った（縦照合・横照合とも差0円）。窓は8/16終端。
END='2026-08-16'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-18']
assert D1==['2026-08-16'] and D3[0]=='2026-08-14' and D7[0]=='2026-08-10', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-18', (len(D30),D30[0])
...
assert len(CUM)==16 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-16'
...
# 8/16の縦照合（全店1クエリ実測 728,388円 / 値引 13,985円）
assert GROSSD['2026-08-16']==728388 and sum(S[n]['2026-08-16'][1] for n in S if '2026-08-16' in S[n])==13985
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0816w.pkl','wb'))
