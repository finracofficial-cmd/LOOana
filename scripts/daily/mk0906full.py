# -*- coding: utf-8 -*-
"""2026-09-06 定例レポート本体（データ終端=9/04 金／9/05の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0904w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-06'   # ★Meta 直読（UVハット・サングラスは PAUSED、4-in-1マルチクリーナー 10,000 が新設）
...
assert sum(ORD.values()) == 686, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1181,'W固定スマホ車載ホルダー':536,'快適マジックインソール':238,'ムダ毛シェーバー':187,
'形状記憶日傘':180,'2WAYシートボックス':157,'接触冷感UVパーカー':136,'偏光・調光サングラス':135,
'完全遮光・接触冷感UVハット':111,'害虫ブロッカー':108,'姿勢サポートチェア':101,'4WAY':99,
'バランスケアスリッパ':95,'完全遮光・形状記憶':70,'優先配送':62,'むくみ取りかっさ':60,
...
# ★2026-09-06 取得の health_check（窓 = 8/06-9/04 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2408868,1297829,38423),'W固定スマホ車載ホルダー':(978303,500923,16751),
'カタログ全部（テスト）':(603787,149977,6092),'ムダ毛シェーバー':(563578,457203,6555),
'快適マジックインソール':(502451,305514,8704),'偏光・調光サングラス':(381807,164323,5402),
'2WAYシートボックス':(376003,129233,4360),'姿勢サポートチェア':(325324,203717,4636),
'完全遮光・接触冷感UVハット':(275769,62863,3477),'バランスケアスリッパ':(219844,153722,3156),
'むくみ取りかっさ':(133992,36794,1447),'ナノバブルシャワーヘッド':(92532,43411,1504),
'温感EMSフェイシャルワンド':(26542,6088,236)}
...
sh('シンプル判定', ['LOOTY 2026-09-06 定例（データ終端=9/04 金）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/04終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['🚨 最優先','**カタログ全部（テスト）18,000 → 10,000円。5日連続で未実行です**',
  '9/04の利益は **+19,995円（利益率5.4%）**。同じ日のカタログ消化は **18,492円**。'
  '★つまり **カタログ1本で、その日の利益とほぼ同額を使っています**。'
  'カタログは商品に紐づかないので売上への寄与を分解できません（だから配賦せず全体から引いています）。'
  'したがって「10,000にすれば利益がいくら増える」とは言えませんが、**支出側で確実に −8,000円/日**です。'
  'Metaを読むと日予算は今も18,000円のまま。**非常ブレーキ中に打てる唯一の宣言済みの手です**',
  '+8,000円/日','本日'],
 ['🚨 非常ブレーキ','**5日連続で発動。過去最低を更新しました（95,960 → 71,636円）**',
  '3日移動平均利益 **71,636円** ＝ 床130,000円の**55%**。'
  '9/02 +78,035 ／ 9/03 +116,878 ／ 9/04 **+19,995（利益率5.4%）**。'
  '9/04は 実売372,740 ÷ 広告257,473 = **MER 1.45** で、この窓で最低。'
  '★広告費は 9/1 230,763 → 9/2 253,111 → 9/3 264,898 → 9/4 257,473 と高止まりしたまま、'
  '売上だけが落ちています',
  '—','本日'],
 ['📊 原因分解','9/04(金)を商品別に割ると、赤字は4本だけ。合計 −11,854円',
  '**偏光・調光サングラス −5,013円**（広告5,013円・販売**0点**）／**温感EMS −2,746円**（新商品・想定内）／'
  '**UVハット −2,693円**（広告6,262・1点）／**ムダ毛シェーバー −1,402円**（MER1.54 vs 分岐1.67）。'
  '★残りは全部黒字で、ナノガラス +16,892 ／ スリッパ +8,198 ／ インソール +6,830 ／ 姿勢チェア +6,253。'
  '**つまり商品側が壊れているのではなく、カタログ18,492円が丸ごと利益を消しています**',
  '—','記録'],
 ['✅ 実行確認','完全遮光・接触冷感UVハット と 偏光・調光サングラスは **停止**されていました',
  '昨日UVハットに「−25%（6,000→4,500）」を出しましたが、Metaでは **キャンペーンごと PAUSED**。'
  '**データはユーザーの判断（停止）を支持しています**: UVハット 3窓 −0.37/+0.05/+0.01、'
  'サングラス 3窓 −0.11/+0.15/−0.07 で、どちらも今朝の判定は「⚠️−25%」に落ちています。'
  '9/04の実績も UVハット −2,693円・サングラス −5,013円（0点）。**減額より停止のほうが正しかった**',
  '+約11,000円/日','完了'],
 ['⚠️ 訂正','サングラスについて、私は9/2→9/4で判断を2転させ、**9/4の訂正のほうが誤りでした**',
  '9/2「季節終了なので停止を」→ 9/4「早計でした、停止しないで」と書きましたが、'
  '9/04は **広告5,013円に対して販売0点**、3窓判定も⚠️に落ちました。'
  '★**結論としては最初の「停止」が正しく、私の訂正が間違いでした。** 9/2の根拠（Metaが入札をやめた）は'
  '確かに2日の消化だけで断定していて雑でしたが、結論自体は当たっていました。'
  '**停止のままで問題ありません。以後この件で再提案はしません**',
  '—','訂正'],
 ['🆕 新商品','**4-in-1マルチクリーナー**（9/05にキャンペーン新設・日予算10,000円）',
  '売価6,980円 / 原価2,961円（両バリアント同額）→ 原価率42.4%・**分岐MER 1.74**・目標MER 4.42。'
  '初日9/05に **2点13,960円**。原価率が高いので、**分岐1.74を超えられるかが勝負**です。'
  '★購入50件まで判定しません。3窓判定に⚠️が出ても誤検知です',
  '—','監視'],
 ['🆕 新商品','温感EMSフェイシャルワンド（累計11件）',
  '9/02 1点 → 9/03 4点 → 9/04 2点 → 9/05 2点。広告は 4,551 → 11,230 → 10,761円。'
  '3窓とも MER1.53 vs 分岐1.52 ＝ **余裕+0.01のトントン**。'
  '★判定表の「⚠️−25%」は**誤検知**（窓3つが同じ3日しか含まない）。50件まで判定しません。'
  'ただし **日予算10,000円で余裕+0.01は薄い**ので、20件を超えたあたりで一度見ます',
  '—','監視'],
 ['⏸ 触らない','姿勢サポートチェア と むくみ取りかっさ（3日・5日が割れている）',
  '姿勢チェア 3日 MER0.86 余裕−0.60 ／ 5日 0.83 余裕−0.62 ／ **7日 1.89 余裕+0.44**。'
  'かっさ 3日 1.21 余裕−0.18 ／ 5日 1.20 余裕−0.19 ／ **7日 1.82 余裕+0.41**。'
  '★**3窓ルールでは据え置き**（7日が黒字）ですが、**2商品とも「あと1窓で発動」の位置**です。'
  '9/05に予算を下げた効果（12,000→9,000 / 13,000→10,000）が出るのを待ちます。'
  '明日7日窓が割れたら、宣言どおり−25%を出します',
  '—','監視'],
 ['📈 速報','9/05(土)は実売 **534,465円** と戻しています',
  '注文106 / 販売数119 / 客単価5,042円 / セッション3,217 / カート追加率 **6.15%**（9/04の5.04%から回復）。'
  '前週の土曜(8/29 583,139円)比 −8.3%。返品2件7,960円。'
  '★停止済みキャンペーンの商品（形状記憶日傘・卓上冷感クーラー・5WAY・ビジュアル耳かき）が'
  'オーガニックで計24,900円売れています。広告費0なのでまるごと利益です',
  '—','速報'],
 ['🔴 判定日','バランスケアスリッパ 増額判定（12,000→15,000）','—',
  '基準「1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円」。3窓は余裕+0.93〜1.07で健全','—','9/8'],
 ['🔴 判定日','快適マジックインソール 再テスト','—',
  '基準「27,000戻しの7日利益 ≥ 27,000時代の7日平均」。3窓は余裕+0.52〜0.61','—','9/10'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/04(金)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/02-9/04', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/29-9/04', R['N7'], R['K7'], R['ACCT7']), ('30日 8/06-9/04', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-04', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/6','**非常ブレーキ**','3日移動平均利益 < 130,000円 で発動',
  '9/02 +78,035 / 9/03 +116,878 / 9/04 +19,995 → **平均 71,636円**（床の55%・過去最低）',
  '🚨 5日連続・最悪を更新',
  '★執行はカタログ 18,000→10,000 の1本のみ。宣言済みの他2本（4WAY・形状記憶日傘）は対象が停止済み。'
  '9/04の利益19,995円に対しカタログ消化18,492円＝**1本で1日の利益と同額を使っている**'],
 ['9/6','完全遮光・接触冷感UVハット','3窓すべて 余裕<0.30 → −25%',
  '3日 −0.37 ／ 5日 +0.05 ／ 7日 +0.01。9/04は広告6,262円で1点・損益 −2,693円',
  '✅ ユーザーが**停止**（減額より上）',
  '昨日は6,000→4,500の減額を出したが、Metaで PAUSED を確認。3日窓が分岐割れに落ちており停止が正しい'],
 ['9/6','偏光・調光サングラス','3窓すべて 余裕<0.30 → −25%',
  '3日 −0.11 ／ 5日 +0.15 ／ 7日 −0.07。9/04は**広告5,013円で販売0点**・損益 −5,013円',
  '✅ ユーザーが**停止**（私の9/4の訂正が誤り）',
  '★9/2「停止を」→9/4「早計でした」と2転させたが、**結論としては最初の停止が正しかった**。'
  '9/2の根拠（2日の消化で断定）は雑だったが結論は当たっていた。以後この件で再提案はしない'],
 ['9/8','バランスケアスリッパ 増額判定（12,000→15,000）','1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円',
  '3窓は 3日+0.97 / 5日+0.93 / 7日+1.07 で健全。9/04も +8,198円','⏳ 進行中','増額直後のロック中なので重ねない'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均',
  '3窓は 3日+0.52 / 5日+0.54 / 7日+0.61。9/04も +6,830円','⏳ 進行中',
  'P2の日別が単調改善していたため、27,000へ戻したうえで7日窓で測り直す'],
 ['—','4-in-1マルチクリーナー（新商品）','購入50件に達するまで判定しない',
  '9/05にキャンペーン新設（日予算10,000）。初日2点13,960円。売価6,980 / 原価2,961 → **分岐MER1.74**','⏸ 判定しない',
  '原価率42.4%と高い。分岐1.74を超えられるかが勝負。3窓判定に⚠️が出ても誤検知'],
 ['—','温感EMSフェイシャルワンド（3窓判定の誤検知）','購入50件に達するまで判定しない',
  '累計11件。3窓とも MER1.53 vs 分岐1.52 ＝ **余裕+0.01**','⏸ 判定しない',
  '★窓3つが同じ3日しか含まないための誤検知。ただし日予算10,000で余裕+0.01は薄いので20件超で一度見る'],
 ['—','姿勢サポートチェア／むくみ取りかっさ','3窓すべてが割れたら −25%',
  '姿勢チェア −0.60/−0.62/**+0.44** ／ かっさ −0.18/−0.19/**+0.41**','⏸ 据え置き（7日が黒字）',
  '★どちらも「あと1窓で発動」。9/05の減額（12,000→9,000 / 13,000→10,000）の効果を待つ。'
  '明日7日窓が割れたら宣言どおり−25%を出す'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は1件だけ: **カタログ全部（テスト）18,000 → 10,000円**（非常ブレーキ①・5日連続で未実行）',
  '★3日移動平均利益 71,636円 ＝ 床130,000円の55%で**過去最低**。9/04は利益+19,995円・利益率5.4%',
  '★同じ9/04のカタログ消化は18,492円。**カタログ1本でその日の利益と同額を使っている**',
  '★UVハットとサングラスはユーザーが停止済み。3窓判定も⚠️に落ちており、**減額より停止が正しかった**'],
...
for r in body:
    if r[0] == '2026-09-05': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
