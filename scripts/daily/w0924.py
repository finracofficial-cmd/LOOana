# -*- coding: utf-8 -*-
"""[9/24終端の窓ビルダー] 2026-09-25 定例（昨日=9/24 木・連休明け平日初日 実績）。実測のみ・毎回ゼロから積み直す。原価は9/20反映のCJ最新表（再調達原価ベース）。"""
import csv, collections, datetime, json, pickle
AD0924=216715  # キャンペーン合算（アカウント照合 差0円）※翌朝再確認。予算256,000の84.7%
...
END='2026-09-24'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=ALLD[-30:]
assert D1==['2026-09-24'] and D3[0]=='2026-09-22' and D7[0]=='2026-09-18', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-08-26', (len(D30),D30[0])
...
CUM=[d for d in ALLD if d>='2026-09-01']   # 当月累積 = 9月（24日）
assert len(CUM)==24 and CUM[0]=='2026-09-01' and CUM[-1]=='2026-09-24'
...
# 9/24の縦照合（全店1クエリ実測 gross 507,120円 / 値引 12,338円 / 注文107件 / 点数125=net124+卓上返品1）
assert GROSSD['2026-09-24']==507120 and sum(S[n]['2026-09-24'][1] for n in S if '2026-09-24' in S[n])==12338
assert abs(ACCT(D1)-AD0924)<1, ('9/24 広告費 アカウント照合', ACCT(D1))
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0924w.pkl','wb'))
