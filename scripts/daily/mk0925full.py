# -*- coding: utf-8 -*-
"""2026-09-25 定例レポート本体（昨日=9/24 木・連休明け平日初日）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0924w.pkl', 'rb'))
...
ORD = {'ナノガラス脱毛パッド':260,'伸縮ガラスクリーナー':179,'W固定スマホ車載ホルダー':129,
...
ORD30 = {'ナノガラス脱毛パッド':1125,'W固定スマホ車載ホルダー':503,'快適マジックインソール':410,
...
MF_RAW = {'ナノガラス脱毛パッド':(2394797,1237875,32853),'W固定スマホ車載ホルダー':(975363,369998,12623),
...
sh('収益', [f'LOOTY 収益レポート 2026-09-25（昨日 = 9/24 木・連休明け平日初日＝答え合わせ初日）',
  f'★昨日の利益 {round(_pl[-1][3]):,}円（利益率 {_pl[-1][3]/_pl[-1][0]:.1%}・MER {_pl[-1][0]/_pl[-1][2]:.2f}）／手数料控除後 {round(_pl[-1][3]-_pl[-1][0]*FEE):,}円',
  f'★9月累積（9/01-24・{len(CUM)}日）: 実売 {cs:,.0f} / 利益 {cp:,.0f}円（{cp/cs:.1%}）／1日あたり {cp/len(CUM):,.0f}円（8月確定は {aup:,.0f}円・{aup/31:,.0f}円/日）',
...
sh('シンプル判定', ['LOOTY 2026-09-25 定例（昨日=9/24 木・連休明け平日初日）',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('前日 9/24(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/22-24', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/18-24', R['N7'], R['K7'], R['ACCT7']), ('30日 8/26-9/24', R['N30'], R['K30'], R['ACCT30']),
        ('9月累積 9/01-24', R['NC'], R['KC'], R['ACCTC'])]:
...
ws.cell(r, 1, '■ 昨日(9/24 木) × 期間比較').font = Font(name='Arial', bold=True, size=11); r += 1
...
for lbl, d1, m3, m7, m30 in per:
    ...
    r += 1
ws.cell(r+1, 1, '9/24(木・平日初日)は実売494,782円・利益+160,317円（32.4%）・CVR3.16%で答え合わせ初日をクリア。前週木曜比+1.4%と平日水準に正常着地し、外部要因説（連休前買い控え＋モール大型セール）はほぼ確定。本朝の判定5本は合格2（インソール・ガラス）・未達3（フック15,000へ減額・サンダル8,000へ戻し・カタログ停止提案）＝ユーザー実行待ち。歯ブラシは9/24に1個と再失速し9/26の7日判定は不利。vs7日平均のマイナスは連休5日が窓に入っているためで、平日としては正常').font = Font(name='Arial', size=10, color='CC0000')
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/26-9/24)」。他シートの7日窓と混ぜて読まないこと',
...
for r in body:
    if r[0] == '2026-09-24': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
...
sh('予算見直しチェック', ['前日(9/24)スナップショット → 本日(9/25)',
...
_hi, _lo = _ix[0], _ix[-1]
sh('曜日指数', ['直近30日(8/26-9/24)。売上=gross−値引。次回定期更新9/28(月)',
...
wb.save('data/reports/report-2026-09-25.xlsx')
