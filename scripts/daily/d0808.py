# -*- coding: utf-8 -*-
"""2026-08-08 定例（昨日=8/07 金実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(45,21),'d3':(111,41),'d7':(257,101),'d30':(822,352)},  # (白/緑757, 黒/桃740)
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(0,0),'d7':(2,0),'d30':(53,0)}}              # (3本2528, 2本1981)
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':190},'湯上がりガーゼワンピース':{'d30':72},
      '完全遮光・接触冷感UVハット':{'d30':37},'3WAYサーキュレーター':{'d7':25,'d30':176},
      '姿勢サポートチェア':{'d7':17,'d30':17}}
# 壁掛けディスペンサーの30日は gross の9.1%がバリアント未分解 → 原価は【概算】
DISP_D30_COST_APPROX=154068   # 430,720×35.77%（バリアント未分解分あり=概算）
HOL={'2026-07-20'}
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-09']
assert D1==['2026-08-07'] and D3[0]=='2026-08-05' and D7[0]=='2026-08-01', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-09', (len(D30),D30[0])
...
def qty(n,days,key):
    ...
    if n in VAR:
        ...
        if n=='壁掛けディスペンサー' and key=='d30': return a+b, DISP_D30_COST_APPROX
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0808.pkl','wb'))
