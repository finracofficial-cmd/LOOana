# -*- coding: utf-8 -*-
"""2026-09-13 朝の判定材料。CSV実測のみ（w0911.py 経由）。"""
import runpy
...
def days_back(k, end='2026-09-11'):
...
print('\n■ 9/11に配信ゼロだったキャンペーン（7日窓に残消化があるもの）')
D7 = days_back(7)
for c in sorted(AD, key=lambda x: -sum(AD[x].get(d, 0) for d in D7)):
    a7 = sum(AD[c].get(d, 0) for d in D7); a1 = AD[c].get('2026-09-11', 0)
    if a7 > 0 and a1 == 0:
