# -*- coding: utf-8 -*-
"""[8/22終端に固定した窓ビルダー] 2026-08-21 定例（昨日=8/22 土 実績）。実測のみ・毎回ゼロから積み直す。"""
...
'2WAYシートボックス':1943,'オートソープディスペンサー':3191,'スタイルアップインナー':1202,'快適マジックインソール':677,'ヘアドライタオル':985,'1秒折り畳みチェア':1584,'3D足臭リセットブラシ':609}
...
'2WAYシートボックス':4980,'オートソープディスペンサー':7980,'スタイルアップインナー':3980,'快適マジックインソール':3980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980,'3D足臭リセットブラシ':3980}
...
# 2026-08-21: 8/20まで揃った（縦照合とも差0円）。窓は8/22終端。
END='2026-08-22'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-24']
assert D1==['2026-08-22'] and D3[0]=='2026-08-20' and D7[0]=='2026-08-16', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-24', (len(D30),D30[0])
...
assert len(CUM)==22 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-22'
...
assert GROSSD['2026-08-22']==475820 and sum(S[n]['2026-08-22'][1] for n in S if '2026-08-22' in S[n])==6572
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0822w.pkl','wb'))
