# -*- coding: utf-8 -*-
"""2026-08-12 定例（昨日=8/11 火・山の日 実績）。実測のみ・毎回ゼロから積み直す。"""
...
VAR={'ナノガラス脱毛パッド':{'d1':(48,23),'d3':(108,55),'d7':(253,109),'d30':(855,373),'cum':(399,169)},
     '壁掛けディスペンサー':{'d1':(0,0),'d3':(1,0),'d7':(2,0),'d30':(41,13),'cum':(3,0)}}
VARC={'ナノガラス脱毛パッド':(757,740),'壁掛けディスペンサー':(2528,1981)}
# 窓内で売価が動いた商品はvariant別net_items_sold（返品0を実測確認済）を販売数として使用
MIXQ={'ムダ毛シェーバー':{'d30':202},'湯上がりガーゼワンピース':{'d30':42},
      '3WAYサーキュレーター':{'d30':155,'cum':26},'姿勢サポートチェア':{'d30':28,'cum':28},
      '完全遮光・接触冷感UVハット':{'d30':61}}
...
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-13']
assert D1==['2026-08-11'] and D3[0]=='2026-08-09' and D7[0]=='2026-08-05', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-13', (len(D30),D30[0])
...
assert len(CUM)==11 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-11'
...
assert sum(v[0] for n in S for d in CUM for v in [S[n][d]] if d in S[n])==10296944+926316
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0812.pkl','wb'))
