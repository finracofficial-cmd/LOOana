# -*- coding: utf-8 -*-
"""2026-09-19 定例レポート本体（データ終端=9/17 木／9/18の広告費は未確定）。

★**非常ブレーキが発動した**（3日移動平均利益 122,606円 ＜ 130,000円）。9/07以来。
  ただし処方（カタログ減額 → 赤字商品−25%）は **すでに実行済み**:
  カタログは9,000（宣言10,000より深い）、赤字だったムダ毛シェーバーと電動温熱カッサは9/18から停止。
  ブレーキの構成日（9/15-17）はその2本が毎日15,190〜20,532円を消化していた期間で、
  「打った手が効く前の数字」でブレーキが鳴っている。よって本日は追加の一律−25%を打たない。
★伸縮ガラスクリーナー 27,000→20,000（昨日の提案）は **未執行**。9/17も26,282円消化している。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0917w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-19'   # ★Meta 直読。9/18から変更なし（9本・228,000円）
...
# 9/17終端の窓でも 9/15 は7日窓(9/11-9/17)の中に入るので、この補完は引き続き必要。
...
'快適マジックインソール':106,'高見えレザーヘッドレストフック':45,'バランスケアスリッパ':25,
'姿勢サポートチェア':24,'ムダ毛シェーバー':20,'もちふわ肉球サンダル':18,'優先配送':8,
'電動温熱カッサ':3,'ナノバブルシャワーヘッド':3,'4-in-1マルチクリーナー':3,'リカバリーサンダル':2,
'携帯電動シェーバー':2,'完全遮光・形状記憶':1,'3D足臭リセットブラシ':1,'スマートノーズEMS美顔器':1,
'接触冷感UVパーカー':1,'接触冷感UVアームカバー':1,'偏光・調光サングラス':1,'壁掛けディスペンサー':1,
'むくみ取りかっさ':1,'癒しの指圧マット':1,'完全遮光・接触冷感UVハット':1}
STORE_ORD7, STORE_SESS7 = 757, 24108
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 770, sum(ORD.values())

# --- 30日(8/19-9/17) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1134,'W固定スマホ車載ホルダー':491,'快適マジックインソール':376,'伸縮ガラスクリーナー':164,
'ムダ毛シェーバー':162,'バランスケアスリッパ':130,'姿勢サポートチェア':102,'2WAYシートボックス':87,
'むくみ取りかっさ':78,'高見えレザーヘッドレストフック':51,'ナノバブルシャワーヘッド':50,'偏光・調光サングラス':49,
'形状記憶日傘':47,'完全遮光・接触冷感UVハット':46,'優先配送':34,'3D足臭リセットブラシ':23,
'4-in-1マルチクリーナー':21,'もちふわ肉球サンダル':18,'4WAY':18,'温感EMSフェイシャルワンド':16,
'携帯電動シェーバー':15,'完全遮光・形状記憶':13,'接触冷感UVアームカバー':12,'接触冷感UVパーカー':8,
'ビジュアル耳かき':6,'壁掛けディスペンサー':6,'癒しの指圧マット':3,'電動温熱カッサ':3,'ジェットウォッシャー':3,
'ヘアドライタオル':2,'スマートノーズEMS美顔器':2,'リカバリーサンダル':2,'湯上がりガーゼワンピース':2,
'ネックマッサージャー':1,'5WAY腰掛けファン':1,'UV歯ブラシ除菌器':1,'瞬間冷感ポンチョ':1,
'卓上冷感クーラー':1,'健康サンダル':1,'1秒折り畳みチェア':1}

# --- 30日(8/19-9/17) Metaファネル ---
# ★★ data_query がハーネス側でブロックされているため、通る campaign_and_resource_get の
#    health_check（last_30_days = 8/19-9/17 の実測・UTC基準の窓）から取った。したがって:
#      ・窓は「7日」ではなく「30日」。過去レポートの CVR順位シートと数値を並べて比較しないこと
#      ・クリックは link_clicks ではなく **全クリック**（CTR/CVR/CPC は全クリック基準で低め/高めに出る）
# ★2026-09-19 取得の health_check（窓 = 8/19-9/17 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2396456,1228757,37421),'W固定スマホ車載ホルダー':(988329,396394,15615),
'快適マジックインソール':(806257,508653,13520),'カタログ全部（テスト）':(506962,125474,4905),
'ムダ毛シェーバー':(486985,398872,5927),'バランスケアスリッパ':(324326,213950,5043),
'姿勢サポートチェア':(315158,171733,4507),'2WAYシートボックス':(241575,73283,2416),
'伸縮ガラスクリーナー':(212257,86549,4730),'むくみ取りかっさ':(180745,52807,2083),
'ナノバブルシャワーヘッド':(163744,79679,2464),'高見えレザーヘッドレストフック':(105248,24032,1157),
'3D足臭リセットブラシ':(85838,24927,422),'4-in-1マルチクリーナー':(80605,23105,727),
'温感EMSフェイシャルワンド':(64858,15636,650),'もちふわ肉球サンダル':(34235,10909,482),
'電動温熱カッサ':(20121,5279,224)}
...
sh('収益', [f'LOOTY 収益レポート（データ終端 = {D1[0][5:].replace("-", "/")} {_WD}）',
  ...
  '🚨 **非常ブレーキが発動しました**（3日移動平均利益 122,606円 ＜ 130,000円・9/07以来）。'
  'ただし処方（カタログ減額→赤字商品−25%）は**すでに実行済み**です。詳細はネクストアクション1行目',
...
sh('シンプル判定', ['LOOTY 2026-09-19 定例（データ終端=9/17 木）',
...
NA = [
 ['🚨 非常ブレーキ','**発動しました**（3日移動平均利益 122,606円 ＜ 130,000円）。ただし**追加の減額は打ちません**',
  '9/07以来の発動です。9/15 +152,863 → 9/16 **+97,232** → 9/17 **+117,722** の3日平均。'
  '★理由を数字で: 処方（カタログ減額 → 赤字商品−25%）は **すでに実行済み**です。'
  'カタログは **9,000**（私の宣言10,000より深い）、赤字だったムダ毛シェーバーと電動温熱カッサは **9/18から停止**。'
  '★ブレーキの構成日（9/15-17）は、その2本が毎日 15,190 / 20,532 / **20,127円** を消化していた期間です。'
  '**打った手が効く前の数字でブレーキが鳴っています。**'
  '★9/18からこの2本が消え、カタログも14,000→9,000。支出側は1日あたり約24,000円軽くなります。'
  '★明日（データ終端9/18）に3日移動平均を再確認します。そこで戻らなければ追加の手を打ちます',
  '—','明日再確認'],
 ['🚨 未執行','伸縮ガラスクリーナー **27,000 → 20,000** がまだ実行されていません',
  '9/17も **26,282円**（日予算27,000の97%）消化しています。'
  '★昨日の判定は不合格でした（1日あたり利益 +33,876円 ＜ 基準 +39,620円・**増分MER 0.65**）。'
  '★9/17の3窓は 3日 MER2.56 / 5日 2.84 / 7日 3.48。'
  '**3日MERが2.56まで下がり、9/13の判定窓開始時（4.95）の半分近く**です。'
  '★いま20,000へ戻せば支出は1日あたり約6,000円軽くなり、非常ブレーキの戻りも早まります',
  '+約6,000円/日','本日'],
 ['📈 途中経過','9/20判定の2件 — **どちらも合格ペース**です（新予算3日目・9/15-9/17）',
  '★高見えレザーヘッドレストフック: 実売126,962 / 34点 / 広告44,945 → 利益 **+12,567円/日**。'
  '基準 +7,675円 の **1.64倍**。'
  '★快適マジックインソール: 実売230,243 / 61点 / 広告95,435 → 利益 **+31,170円/日**。'
  '基準 +30,269円 を **+901円/日** 上回っています（ぎりぎり）。'
  '★4日目（9/18）を足して明日確定します',
  '—','9/20'],
 ['⚠️ 監視','バランスケアスリッパの3日が分岐割れ（MER 1.22 ＜ 分岐1.35・余裕 **−0.14**）',
  '5日 +0.27 ／ 7日 +0.34 なので3窓とも割れてはおらず、ルール上は据え置きです。'
  '★ただし7日でも目標2.61には遠く（MER1.70）、余裕は3日連続で縮んでいます。'
  '★9/17に日予算を12,000→9,000へ落としたばかりなので、**その効果を7日見てから**判断します。'
  '判定日は **9/24**（減額実施+7日）。基準: 9/18-9/24の7日で MER ≥ 分岐1.37',
  '—','9/24'],
 ['✅ 確認','ムダ毛シェーバー と 電動温熱カッサ の停止は、9/17の数字でも裏づけられました',
  '★ムダ毛シェーバー: 3窓とも余裕<0.30（3日 **−0.30** / 5日 +0.19 / 7日 +0.01）＝ルール上も「−25%」。'
  '9/17も12,676円消化していたので、停止が効いたのは9/17の途中〜9/18です。'
  '★電動温熱カッサ: 3窓すべて分岐割れ（MER **1.04** ＜ 分岐1.57）＝ルール上も「停止/−50%」',
  '—','完了'],
 ['⏸ 据え置き','快適マジックインソールは3窓とも 🚀 ですが、**明日が判定日なので動かしません**',
  '3日 +1.19 ／ 5日 +1.09 ／ 7日 +1.18（分岐1.22・目標2.13に対し MER2.31〜2.41）。'
  '★9/20に32,000の判定を置いています。判定中の商品を追加で動かすと窓が汚れて何も測れなくなります',
  '—','9/20'],
 ['⏸ 経過','もちふわ肉球サンダル（日予算14,000）: 累計26点。**天井に張り付いています**',
  '9/17は広告12,329円で、当時の日予算11,000の **112%**。'
  '7日 MER2.65 vs 分岐1.42・**目標2.82**・余裕+1.23。目標まであと0.17です。'
  '★50点まで判定しない宣言は継続します（あと24点）',
  '—','50点で判定'],
 ['📌 記録','9/18(金)に **ムダ毛シェーバーの返品が1件（6,980円）**',
  'A案どおり売上からも利益からも控除していません。9月の返品はこれが1件目です。'
  '★9/18は UV歯ブラシ除菌器が3点26,940円・携帯電動シェーバーが3点14,940円。'
  'どちらも広告ゼロのオーガニックで、久々に動きました',
  '—','記録'],
 ['🟡 推奨','アップセル手動ペア5組の設定',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/17(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/15-9/17', R['N3'], R['K3'], R['ACCT3']),
        ('7日 9/11-9/17', R['N7'], R['K7'], R['ACCT7']), ('30日 8/19-9/17', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-17', R['NC'], R['KC'], R['ACCTC'])]:
...
sh('全体サマリー', ['全体サマリー（売上=Shopify gross−値引・広告費=Meta実測。返品は売上からも利益からも控除しない=A案）',
 ...
 '縦照合3本すべて通過（9/17）: Σ商品gross=全店 506,160 ／ Σ値引=18,412 ／ Σキャンペーン広告費=246,001円',
 '★9/17の広告費は data_query がブロックされたため health_check(8/19-9/17実測) − CSV(8/19-9/16実測) で導出。'
 '窓の検証: 停止済み **5本すべて**で HC − CSV = **0円ちょうど**（3D足臭85,838／かっさ180,745／'
 '温感EMS64,858／ナノバブル163,744／4-in-1 80,605）。8/18始まりは5本が負値（2WAY −15,034等）、'
 '8/20始まりは合計444,592円＝日予算228,000の195%で非現実的 → 8/19-9/17で確定',
...
ws.cell(r+1, 1, '🚨 3日移動平均利益 '
  f'{p3ma:,.0f}円 が非常ブレーキ130,000円を **{130000-p3ma:,.0f}円 下回った**（9/07以来の発動）。'
  '9/17(木)の実売487,748円は前週木曜(9/10 578,136円)比 −15.6%、木曜指数108.5に対しても弱い。'
  'ただし広告側は 9/18から ムダ毛+電動温熱カッサ（9/17実績で20,127円/日）が消え、'
  'カタログも14,000→9,000。支出は1日あたり約24,000円軽くなる').font = Font(name='Arial', size=10, color='CC0000')
...
sh('商品別', ['商品別（7日 9/11-9/17 = 判定単位）。カタログ行の広告費は「7日合計」（前日ではない）',
  ...
  '★停止済みで7日窓に残消化が入っているもの: 4-in-1マルチクリーナー(9/14最終・7日24,110円)・'
  'ナノバブルシャワーヘッド(9/13最終・21,982円)。判定の対象外',
  '★ムダ毛シェーバー と 電動温熱カッサ は **9/18から PAUSED**。7日窓には9/17までの消化が入っている'
  '（ムダ毛7日 95,600円 → 12,676円/日・カッサ7日 20,121円）',
  '★もちふわ肉球サンダル(9/14新設)は稼働4日、電動温熱カッサ(9/15新設)は3日。'
  '他商品と7日利益で単純比較しないこと'],
...
sh('週次診断', ['全店の週次分解。原価は日次のバリアント構成が取れないナノガラス/ディスペンサーのみ30日実測ミックスの加重平均単価【近似】',
  '★週は日〜土で切っている。9/13(日)〜9/17(木)は週の途中なのでこの表には入らない（日次推移シートで見ること）',
...
# 17キャンペーン中16本は差0円。2WAYシートボックス（9/10で最終消化）だけ HC 241,575 vs CSV 241,624 の
# **−49円**（Metaの過去日再集計ドリフト＝0.020%）。許容幅を60円に置いて、差の実額をここに明記しておく。
...
sh('ファネル30日', ['⚠️ このシートだけ窓が「30日(8/19-9/17)」。他シートの7日窓と混ぜて読まないこと',
  ...
  '✅ 検算: 17キャンペーン中16本で health_check の消化 = CSV(8/19-9/17) が差0円で一致。'
  '2WAYシートボックスのみ **−49円**（0.020%・Metaの過去日再集計ドリフト）。コード内assertの許容幅は60円',
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/19','🚨 非常ブレーキ','3日移動平均利益 < 130,000円 → カタログ減額 → 赤字商品一律−25%',
  '3日移動平均 **122,606円**（9/15 +152,863 / 9/16 +97,232 / 9/17 +117,722）。9/07以来の発動',
  '⏸ **処方はすでに実行済み → 追加なし**',
  '★カタログは9,000（宣言10,000より深い）、赤字の2本（ムダ毛・電動温熱カッサ）は9/18から停止。'
  'ブレーキの構成日はその2本が毎日15,190〜20,127円を消化していた期間で、打った手が効く前の数字。明日再確認'],
 ['9/19','伸縮ガラスクリーナー 27,000 → 20,000（9/18の判定結果）','昨日の不合格判定にもとづく戻し',
  'Meta直読で **27,000のまま**。9/17も26,282円（97%）消化。'
  '3日MERは **2.56** まで低下（判定窓開始時の9/13は4.95）',
  '🚨 **未執行 → 本日**',
  '★20,000へ戻せば支出は1日あたり約6,000円軽くなり、非常ブレーキの戻りも早まる'],
 ...
 ['9/20','高見えレザーヘッドレストフック 15,000（9/16宣言ぶん・継続）',
  ...
  '新予算3日（9/15-17）: 実売126,962 / 34点 / 広告44,945 → 利益 **+12,567円/日**（基準の1.64倍）',
  '⏳ **合格ペース**。9/18を足して明日確定',
  '★7日 MER2.86 vs 分岐1.54・余裕+1.32 で黒字も安定'],
 ['9/20','快適マジックインソール 32,000（9/16宣言ぶん・継続）',
  '**32,000時代の1日あたり利益 ≥ 27,000時代（9/08-14）の +30,269円**',
  '新予算3日（9/15-17）: 実売230,243 / 61点 / 広告95,435 → 利益 **+31,170円/日**（基準を +901円/日 上回る）',
  '⏳ **合格ペース（ぎりぎり）**。9/18を足して明日確定',
  '★3窓とも🚀だが判定中なので動かさない'],
 ['9/24','バランスケアスリッパ（減額の効果判定・新規宣言）',
  '**9/18-9/24の7日で MER ≥ 分岐1.37**',
  '3日 MER1.22 ＜ 分岐1.35（余裕 **−0.14**）／ 5日 +0.27 ／ 7日 +0.34。3窓とも割れてはいないので据え置き',
  '⏳ 減額実施(9/17)+7日',
  '★7日でも目標2.61に遠く（MER1.70）、余裕は3日連続で縮んでいる。12,000→9,000の効果を7日見る'],
 ['9/19','ムダ毛シェーバー（停止の事後確認）','3窓すべて余裕<0.30 → −25%',
  '3日 **−0.30** / 5日 +0.19 / 7日 +0.01。7日 MER1.68 vs 分岐1.67 で分岐すれすれ',
  '✅ 停止は妥当',
  '★ルール上も「−25%」に該当していた。9/17も12,676円消化していたので停止は9/17の途中〜9/18'],
 ['9/19','電動温熱カッサ（停止の事後確認）','3窓すべて分岐割れ → 停止/−50%',
  '3窓とも MER **1.04** ＜ 分岐1.57（余裕 −0.53）。累計3点・広告20,121円',
  '✅ 停止は妥当','★ルール上も「停止/−50%」に該当していた'],
 ['50点','もちふわ肉球サンダル（日予算14,000）','購入50点到達時に判定',
  '累計26点。7日 MER2.65 vs 分岐1.42・**目標2.82**・余裕+1.23。9/17は広告12,329円＝当時の日予算11,000の112%',
  '⏸ 50点まで動かさない',
  '★目標2.82まであと0.17。天井に張り付いているので、50点判定で増額候補になる可能性が高い'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '🚨 **非常ブレーキが発動しました**（3日移動平均 122,606円 ＜ 130,000円・9/07以来）。'
  'ただし処方（カタログ減額→赤字商品−25%）は**すでに実行済み**なので、今日は追加の減額を打ちません。'
  'カタログは9,000、赤字の2本は9/18から停止。**打った手が効く前の数字でブレーキが鳴っています**',
  '★本日の執行は **伸縮ガラスクリーナー 27,000 → 20,000**（昨日の不合格判定ぶん・まだ未執行）の1件です',
  '★9/20の判定2件は **どちらも合格ペース**です'
  '（フック +12,567円/日 vs 基準+7,675 ／ インソール +31,170円/日 vs 基準+30,269）',
  '★バランスケアスリッパの3日が分岐割れ（−0.14）。3窓とも割れてはいないので据え置き、'
  '**9/24に減額の効果判定**を置きます'],
 ['判定日','対象','宣言済み基準','実測','結果','備考'], JUDGE, [10,26,34,34,16,46])

# ---------- 9a 予算見直しチェック ----------
# 変更前 = data/budget-snapshots.csv の '2026-09-18' ／ 変更後 = SNAP '2026-09-19'（Meta直読）
PREVB = collections.defaultdict(int)
for r in body:
    if r[0] == '2026-09-18': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
...
sh('予算見直しチェック', ['変更後の値はMetaを直読して確認済み'
  ...
  '合計 **228,000円で据え置き**（9/18から変更なし）。稼働キャンペーンは9本',
  '★昨日の提案（伸縮ガラスクリーナー 27,000 → 20,000）は **未執行**です。9/17も26,282円消化しています',
  '★実消化は日予算を上回っています: 9/17は 246,001円 ÷ 日予算228,000 = **108%**。'
  'ただしこの差の大半は 9/18から停止した2本（ムダ毛12,676 + 電動温熱カッサ7,451 = 20,127円）です',
  '★9/18以降の実効水準: 228,000 − 伸縮を20,000にすれば **221,000円**',
  '⚠️ 週消化率の 2026-09-15 は補完値（スナップショット欠測・9/16の値を当てた）'],
...
# ---------- 10 CVR推移 ----------
SES = {'2026-08-19':3190,'2026-08-20':3356,'2026-08-21':3120,'2026-08-22':3220,'2026-08-23':3438,
'2026-08-24':3192,'2026-08-25':3280,'2026-08-26':2733,'2026-08-27':3145,'2026-08-28':3149,
'2026-08-29':3537,'2026-08-30':3310,'2026-08-31':2761,'2026-09-01':2936,'2026-09-02':3142,
'2026-09-03':3340,'2026-09-04':3293,'2026-09-05':3217,'2026-09-06':3752,'2026-09-07':2787,
'2026-09-08':2739,'2026-09-09':2801,'2026-09-10':3514,'2026-09-11':3014,'2026-09-12':3132,
'2026-09-13':4246,'2026-09-14':3592,'2026-09-15':3358,'2026-09-16':3348,'2026-09-17':3418}
ORDD = {'2026-08-19':106,'2026-08-20':113,'2026-08-21':110,'2026-08-22':98,'2026-08-23':106,
'2026-08-24':100,'2026-08-25':99,'2026-08-26':105,'2026-08-27':130,'2026-08-28':85,
'2026-08-29':115,'2026-08-30':114,'2026-08-31':84,'2026-09-01':88,'2026-09-02':92,
'2026-09-03':103,'2026-09-04':74,'2026-09-05':106,'2026-09-06':122,'2026-09-07':96,
'2026-09-08':95,'2026-09-09':98,'2026-09-10':116,'2026-09-11':96,'2026-09-12':117,
'2026-09-13':140,'2026-09-14':102,'2026-09-15':103,'2026-09-16':99,'2026-09-17':100}
...
sh('CVR推移', ['全店CVR = Shopify注文 ÷ Shopifyセッション。商品別レポートのCVR（注文÷Metaクリック）とは分母が違うので混ぜないこと',
  f'★8/19-8/31（13日）: {o1}注文 / {s1_:,}セッション = {o1/s1_:.2%}',
  f'★9/01-9/17（17日）: {o2}注文 / {s2:,}セッション = {o2/s2:.2%} → **{o2/s2/(o1/s1_)-1:+.1%}**',
  '★直近5日は セッション 3,132 → 4,246 → 3,592 → 3,358 → 3,348 → 3,418 と高止まりなのに、'
  '注文は 117 → 140 → 102 → 103 → 99 → 100 で頭打ちです。**CVRが 3.74% → 2.93% まで落ちています**',
  '★9/17(木)は セッション3,418・注文100件 = CVR 2.93%・カート追加率5.03%・チェックアウト到達162。'
  'カート追加率も直近では低いほうで、上流ではなく **カートに入る手前**が弱いです',
  '★9/18(金)は セッション3,583・注文96件 = CVR 2.68%・カート追加率5.33%（窓の外なので表には入れていません）'],
 ['日付','曜','セッション','注文','全店CVR','備考'], CV, [12,5,12,10,10,20],
 {3:'#,##0',4:'#,##0',5:'0.00%'})

# ---------- 11 曜日指数 ----------
sh('曜日指数', ['直近30日(8/19-9/17)・祝日なし。売上=gross−値引',
  '★30日窓が夏物崩壊前の日（日予算590,000時代）を含まないので、**この指数は汚染されていません**',
  '★日曜が最強(114.2)・木曜が2番手(108.5)・金曜が最弱(88.7)。日曜が突出する形は7月から変わっていません',
  '★曜日ごとの手動予算調整は不要です（Metaの週内ペーシングが自動で強い日に寄せます）'],
 ['曜日','指数(全体=100)'], [[k, round(v, 1)] for k, v in sorted(R['IDX'].items(), key=lambda x: -x[1])], [10, 16])

wb.save('data/reports/report-2026-09-19.xlsx')
