# -*- coding: utf-8 -*-
"""2026-08-04 定例（昨日=8/03 月実績）。実測のみ・毎回ゼロから積み直す。"""
...
D30=[d for d in ALLD if d>='2026-07-05']
assert D1==['2026-08-03'] and D7[0]=='2026-07-28' and D3[0]=='2026-08-01', (D1,D7[0],D3[0])
assert len(D30)==30 and D30[0]=='2026-07-05', (len(D30),D30[0])
...
print()
SUM={}
for lbl,days,N,K in [('前日(8/03 月)',D1,N1,K1),('3日(8/01-8/03)',D3,N3,K3),
                     ('7日(7/28-8/03)',D7,N7,K7),('30日(7/05-8/03)',D30,N30,K30)]:
...
# 同曜日平均（土曜・祝日除外）
SAT=[d for d in D30C if wd(d)=='月' and d not in HOL]
sat=sum(STORE[d] for d in SAT)/len(SAT)
print(f'\n月曜平均(直近30日 n={len(SAT)}): {sat:,.0f}円 / 8/03実績 {STORE["2026-08-03"]:,}円 = {STORE["2026-08-03"]/sat*100:.1f}%')
BR={};TG={}
...
print('\n'+'='*128)
print('■ 商品別（7日=7/28-8/03 / 3日=8/01-8/03 / 前日=8/03 / 30日=7/05-8/03。すべて実測）')
print('='*128)
...
for n in sorted(N7, key=lambda x:-N7[x]):
    ...
    bud=BUD.get(MAP.get(n,n),{}).get('2026-08-04')
...
pickle.dump(dict(rows=rows,N1=N1,N3=N3,N7=N7,N30=N30,K1=K1,K3=K3,K7=K7,K30=K30,Q1=Q1,Q7=Q7,
  ...
  open('/tmp/claude-0/-home-user-LOOana/42be2ed9-9967-56df-ab9c-75166f45bbc9/scratchpad/rep0804.pkl','wb'))
