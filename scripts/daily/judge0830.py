# -*- coding: utf-8 -*-
"""2026-08-29 朝の判定材料。CSV実測のみ（w0829.py 経由）。"""
import runpy
...
def days_back(k, end='2026-08-29'):
...
print('\n■ 判定① ナノガラス2 単独（8/29宣言: 3日連続で倍率<1.00なら60,000→40,000）')
...
print('\n■ 判定② 2WAY・サングラス・ナノバブル（8/29宣言: 2日連続で購入ゼロなら即-50%）')
...
print('\n■ 8/29に配信ゼロだったキャンペーン（7日窓に残消化があるもの）')
D7 = days_back(7)
for c in sorted(AD, key=lambda x: -sum(AD[x].get(d, 0) for d in D7)):
    a7 = sum(AD[c].get(d, 0) for d in D7); a1 = AD[c].get('2026-08-29', 0)
    if a7 > 0 and a1 == 0:
