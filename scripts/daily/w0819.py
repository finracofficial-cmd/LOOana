# -*- coding: utf-8 -*-
"""[8/17終端に固定した窓ビルダー] 2026-08-18 定例（昨日=8/17 月 実績）。実測のみ・毎回ゼロから積み直す。"""
...
'2WAYシートボックス':1943,'快適マジックインソール':677,'ヘアドライタオル':985,'1秒折り畳みチェア':1584}
...
'2WAYシートボックス':4980,'快適マジックインソール':3980,'ヘアドライタオル':3980,'1秒折り畳みチェア':4980}
...
# 2026-08-20: 8/19まで揃った（縦照合とも差0円）。窓は8/19終端。
END='2026-08-19'; ALLD=[d for d in ALLD if d<=END]
D1=ALLD[-1:]; D3=ALLD[-3:]; D7=ALLD[-7:]; D30=[d for d in ALLD if d>='2026-07-21']
assert D1==['2026-08-19'] and D3[0]=='2026-08-17' and D7[0]=='2026-08-13', (D1,D3[0],D7[0])
assert len(D30)==30 and D30[0]=='2026-07-21', (len(D30),D30[0])
...
assert len(CUM)==19 and CUM[0]=='2026-08-01' and CUM[-1]=='2026-08-19'
...
assert GROSSD['2026-08-19']==550096 and sum(S[n]['2026-08-19'][1] for n in S if '2026-08-19' in S[n])==10355
...
pickle.dump(dict(D1=D1,D3=D3,D7=D7,D30=D30,CUM=CUM,NC=NC,DCC=DCC,QC=QC,KC=KC,ACCTC=ACCT(CUM),CATC=CAT(CUM),AC={n:A(CUM,n) for n in ALLN},STORE=STORE,GROSSD=GROSSD,IDX=IDX,
  ...
  ACCT1=ACCT(D1),ACCT3=ACCT(D3),ACCT7=ACCT(D7),ACCT30=ACCT(D30)),open('/tmp/rep0819w.pkl','wb'))
