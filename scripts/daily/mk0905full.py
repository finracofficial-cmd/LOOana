# -*- coding: utf-8 -*-
"""2026-09-05 定例レポート本体（データ終端=9/03 木／9/04の広告費は未確定）。"""
import pickle, csv, datetime, collections, statistics
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
R = pickle.load(open('/tmp/rep0903w.pkl', 'rb')); FEE = 0.03423   # 2026-09-01 月次再計測（SP79.7% + スマホ17.1% + Paidy3.2%）
D1, D3, D7, D30, CUM = R['D1'], R['D3'], R['D7'], R['D30'], R['CUM']
SNAP = '2026-09-05'   # ★6日ぶりに Meta 直読で取り直した（8/30b から8セットが変わっていた）
...
ORD = {'ナノガラス脱毛パッド':265,'W固定スマホ車載ホルダー':109,'快適マジックインソール':82,'ムダ毛シェーバー':46,
'バランスケアスリッパ':38,'むくみ取りかっさ':36,'姿勢サポートチェア':27,'2WAYシートボックス':20,
'ナノバブルシャワーヘッド':19,'完全遮光・接触冷感UVハット':12,'優先配送':8,'偏光・調光サングラス':8,
'携帯電動シェーバー':5,'ビジュアル耳かき':5,'温感EMSフェイシャルワンド':4,'壁掛けディスペンサー':2,
'3D足臭リセットブラシ':2,'4WAY':2,'ネックマッサージャー':1,'瞬間冷感ポンチョ':1,'接触冷感UVパーカー':1,
'ジェットウォッシャー':1,'ヘアドライタオル':1,'1秒折り畳みチェア':1,'完全遮光・形状記憶':1}
STORE_ORD7, STORE_SESS7 = 681, 22175
# 商品別注文数の合計 > 全店注文数 なのは、複数商品を含む注文が各商品で1件ずつ数えられるため
assert sum(ORD.values()) == 697, sum(ORD.values())

# --- 30日(8/03-9/01) Shopify注文数（ShopifyQL GROUP BY product_title 実測）---
ORD30 = {'ナノガラス脱毛パッド':1185,'W固定スマホ車載ホルダー':537,'快適マジックインソール':229,'形状記憶日傘':193,
'ムダ毛シェーバー':192,'2WAYシートボックス':155,'接触冷感UVパーカー':149,'偏光・調光サングラス':143,
'害虫ブロッカー':119,'4WAY':115,'完全遮光・接触冷感UVハット':113,'姿勢サポートチェア':103,
'バランスケアスリッパ':91,'完全遮光・形状記憶':74,'優先配送':68,'むくみ取りかっさ':57,
'接触冷感UVアームカバー':53,'5WAY腰掛けファン':51,'携帯電動シェーバー':32,'ナノバブルシャワーヘッド':29,
'3D足臭リセットブラシ':21,'卓上冷感クーラー':16,'瞬間冷感ポンチョ':11,'ビジュアル耳かき':5,
'壁掛けディスペンサー':5,'3WAYサーキュレーター':5,'ヘアドライタオル':4,'健康サンダル':4,
'温感EMSフェイシャルワンド':4,'ジェットウォッシャー':2,'湯上がりガーゼワンピース':2,
'瞬間冷却ハンディファン':1,'UV歯ブラシ除菌器':1,'姿勢サポートベルト':1,'癒しの指圧マット':1,
'1秒折り畳みチェア':1,'リカバリーサンダル':1,'ネックマッサージャー':1}
...
# ★2026-09-05 取得の health_check（窓 = 8/05-9/03 = D30 と一致することを下でassert）
MF_RAW = {'ナノガラス脱毛パッド':(2390593,1288725,38060),'W固定スマホ車載ホルダー':(961463,496013,16475),
'カタログ全部（テスト）':(607549,151927,6205),'ムダ毛シェーバー':(565723,459750,6645),
'快適マジックインソール':(474540,286463,8174),'偏光・調光サングラス':(397003,172423,5607),
'2WAYシートボックス':(367705,125988,4284),'姿勢サポートチェア':(329702,206887,4682),
'完全遮光・接触冷感UVハット':(278649,63502,3513),'バランスケアスリッパ':(210420,146757,2972),
'むくみ取りかっさ':(125489,34376,1344),'ナノバブルシャワーヘッド':(84146,39094,1389),
'温感EMSフェイシャルワンド':(15781,3205,121)}
...
sh('シンプル判定', ['LOOTY 2026-09-05 定例（データ終端=9/03 木）',
 '状態: 🚨縮小・停止=7日MER<分岐 or 7日利益マイナス ／ 🔧改善=3日MER<分岐 or 余裕<0.30 ／ 🚀伸ばす候補=7日MER≥目標かつ週消化率≥95% ／ ✅維持',
 '3窓判定（3日/5日/7日・すべて9/03終端）が本番の意思決定ルール。窓が1つでも割れたら据え置き＝動かさない',
...
NA = [
 ['✅ 解消','**予算スナップショットを6日ぶりに取り直しました**（Meta直読・9/05時点）',
  '毎日お願いしていた件は、こちらでMetaを直接読んで解決しました。以後もこの方法で自分で取ります。'
  '8/30以降に**8セットが変わっていました**: W固定 35,000→30,000 ／ インソール 30,000→**27,000** ／ '
  'かっさ 13,000→10,000 ／ 姿勢チェア 12,000→9,000 ／ 2WAY 11,000→8,000 ／ **スリッパ 12,000→15,000** ／ '
  '耳かき 8,000→停止 ／ サングラス 5,000→**キャンペーンごと停止** ／ 温感EMS **+10,000（新設）**。'
  '**日予算合計 255,000 → 238,000円（−17,000）**',
  '—','完了'],
 ['✅ 済み','快適マジックインソールは **すでに27,000円に戻っています**',
  '昨日の判定（30,000→27,000）の結論と同じ水準です。**追加の操作は不要**。'
  '9/03の消化は37,117円で27,000の137%＝Metaの単日ペーシング（+75%まで）の範囲内。'
  '★予定どおり **9/10に「7日利益 ≥ 27,000時代の7日平均」で再テスト**します',
  '—','9/10に再判定'],
 ['🚨 執行','完全遮光・接触冷感UVハット **6,000 → 4,500円（−25%）**',
  '3窓判定が **3窓とも余裕<0.30** に入りました（3日+0.25 ／ 5日+0.14 ／ 7日+0.07）。宣言済みルールの−25%です。'
  '7日(8/28-9/03)は 実売67,728 / 14点 / 広告45,648 / 原価19,754 → **利益 +2,326円**。MER1.48 vs 分岐1.41。'
  '夏物なので需要そのものが引いています。'
  '★これは減額であって停止ではありません。4,500で1週間見て、さらに3窓割れなら停止を判断します',
  '+1,500円/日','本日'],
 ['🚨 執行','カタログ全部（テスト）**18,000 → 10,000円**（非常ブレーキ①・4日連続で未実行）',
  '★昨日「14,000→10,000」と書いたのは**私の誤り**でした。「夏以外」4,000円は別キャンペーンで、'
  'スナップショット上のカタログは最初から **18,000円**。宣言どおり **18,000→10,000（−8,000円/日）** が正しい。'
  '9/03の消化は19,560円で、減額は入っていません。UVハットと合わせて **約9,500円/日**の圧縮になります',
  '+8,000円/日','本日'],
 ['🚨 非常ブレーキ','**4日連続で発動中。ただし底は打ちました（81,737 → 95,960円）**',
  '3日移動平均利益 **95,960円** ＝ 床130,000円の74%。9/01 +92,968 ／ 9/02 +78,035 ／ 9/03 **+116,878**。'
  '9/03(木)は実売518,956・利益率22.5%まで戻した。'
  '★ただし9/04(金)は実売 **372,740円** と落ちているので、まだ抜けたとは言えません（下の速報）',
  '—','監視'],
 ['⚠️ 注意','広告費が3日連続で増えています（230,763 → 253,111 → **264,898**）',
  '日予算の合計は238,000円なので、9/03の消化264,898円は **111%**。Metaの週内ペーシングの範囲内ですが、'
  '**売上が伸びない中で消化だけ増えている**のが利益率低下の直接原因です。'
  '上の執行2本（カタログ・UVハット）で約9,500円/日が下がります',
  '—','記録'],
 ['⚠️ 判定の前提が変わった','バランスケアスリッパが **12,000 → 15,000** にもう一段上がっていました',
  '昨日「12,000で据え置き」と判定しましたが、Metaを読むと既に15,000です（v2 広告セット）。'
  '**12,000での合格判定は 15,000 の是非を保証しません**。3窓は余裕+1.06〜1.13で健全なので今は触りません。'
  '★**9/8に「12,000→15,000の1日あたり利益 ≥ 12,000時代（8/31-9/2）の +8,997円」で判定**します',
  '—','9/8に判定'],
 ['⏸ 判断待ち','偏光・調光サングラスは **キャンペーンごと停止**されていました',
  '9/2に私が「季節終了なので停止を」と書き、9/4に「早計でした、停止しないで」と訂正しました。'
  '**訂正が間に合わず、停止のほうが実行されたようです。混乱させて申し訳ありません。**'
  '事実だけ並べると: 消化は 8/31 151 → 9/01 124 → 9/02 3,760 → 9/03 **4,768円** と戻り、'
  '3窓は 3日+0.55 / 5日+0.42 / 7日**+0.14** で分岐割れなし。ただし7日の余裕は薄い。'
  '★9/02作成の「- コピー」広告セット（5,000円）はENABLEDのままなので、**キャンペーンを再開すればすぐ動きます**。'
  '非常ブレーキ中なので「止めたまま」も妥当です。**どちらでも構いません。私からは再提案しません**',
  '—','ユーザー判断'],
 ['🆕 新商品','温感EMSフェイシャルワンド（日予算10,000円）',
  '9/02 広告4,551円・1点 → 9/03 **11,230円・4点** → 9/04 2点。累計7件。'
  '売価5,980円 / 原価 シルバー1,866・ピンク2,079 → 分岐MER 約1.53。現在 MER1.82・余裕+0.29。'
  '★3窓判定に「⚠️−25%」と出ますが **誤検知**です（窓3つが同じ2日しか含んでいない）。'
  '**新商品は購入50件まで判定しない**のが宣言済みルール。**触らないでください**',
  '—','監視'],
 ['⏸ 触らない','姿勢サポートチェア と むくみ取りかっさ（3日窓が割れている）',
  '姿勢チェア 3日 MER0.33 余裕−1.10 ／ かっさ 3日 MER1.11 余裕−0.28。どちらも5日・7日は黒字なので'
  '**3窓ルールでは据え置き**。9/03に姿勢チェアが0点だったのが効いている。'
  '★どちらも9/05に予算が下がっている（12,000→9,000 / 13,000→10,000）ので、その効果を待つ',
  '—','監視'],
 ['📉 速報','9/04(金)は実売 **372,740円** と落ちています',
  '注文74 / 販売数85 / 客単価5,037円 / セッション3,293 / カート追加率 **5.04%**（前日5.90%から低下）。'
  '前週の金曜(8/28 423,531円)比 **−12.0%**。'
  '★サングラス0点・優先配送0件。広告費は09:00 JSTに窓が回ってから確定させます',
  '—','速報'],
 ['🔴 判定日','優先配送 790円 → **❌ 不合格**',
  '宣言済み基準「14日で件数 ≥ 28件」。**14日(8/21-9/03)で15件**。7日は8件、9/04は0件。'
  '★値上げ(536→790)後もアタッチ率は 8件/681注文 = **1.17%** で、15%目標には遠い。'
  '原価0の純利益商品なので停止はしませんが、**アップセル導線の改善が先**（下の推奨）',
  '—','判定完了'],
 ['🟡 推奨','アップセル手動ペア5組の設定（優先配送の不合格を受けて優先度を上げます）',
  'data/lp/upsell-pairs-2026-08-26.txt の5組をアプリに手入力する。異商品ミックス率が実質ゼロなので'
  '自動推薦は学習データがなく全店売れ筋にフォールバックする。判定は実施+14日で異商品ミックス率3.0%以上','週+約4万円','優先'],
 ['🟡 推奨','秋冬商材の仕込み',
  '温感EMSの立ち上がり（3日で7件）は良い兆候。同じ枠でもう1〜2本走らせたい。'
  'CJで あったかインソール 1,000円以下 ／ 外反母趾サポーター 800円以下 を確認','—','優先'],
 ['🟡 推奨','カテゴリタグ40件の付与',
...
# ---------- 3 全体サマリー ----------
SUMR = []
for lbl, N, K, ACC in [('9/03(木)', R['N1'], R['K1'], R['ACCT1']), ('3日 9/01-9/03', R['N3'], R['K3'], R['ACCT3']),
        ('7日 8/28-9/03', R['N7'], R['K7'], R['ACCT7']), ('30日 8/05-9/03', R['N30'], R['K30'], R['ACCT30']),
        ('★当月累積 9/01-03', R['NC'], R['KC'], R['ACCTC'])]:
...
# ---------- 9 本日の判定 ----------
JUDGE = [
 ['9/5','完全遮光・接触冷感UVハット','3窓すべて 余裕<0.30 → −25%',
  '3日+0.25 ／ 5日+0.14 ／ 7日+0.07。7日(8/28-9/03) 実売67,728 / 14点 / 広告45,648 → 利益+2,326円・MER1.48 vs 分岐1.41',
  '🚨 −25% → 6,000を4,500へ',
  '夏物で需要が引いている。減額であって停止ではない。4,500で1週間見て、さらに3窓割れなら停止を判断'],
 ['9/5','**非常ブレーキ**','3日移動平均利益 < 130,000円 で発動',
  '9/01 +92,968 / 9/02 +78,035 / 9/03 +116,878 → **平均 95,960円**（床の74%）',
  '🚨 4日連続・ただし底は打った',
  '①カタログ 18,000→10,000（4日連続で未実行）②UVハット −25%。合わせて約9,500円/日。'
  '★昨日「カタログ14,000」と書いたのは誤り。「夏以外」4,000は別キャンペーンで、カタログ本体は最初から18,000'],
 ['9/4','優先配送 790円','14日で件数 ≥ 28件','14日(8/21-9/03)で **15件**。7日8件、9/04は0件',
  '❌ 不合格',
  '値上げ後もアタッチ率 8/681 = 1.17%。原価0の純利益商品なので停止はしないが、アップセル導線の改善が先'],
 ['9/5','快適マジックインソール（昨日の判定の結果）','不合格 → 30,000を27,000へ戻す',
  'Meta直読で **すでに27,000円**。9/03の消化37,117円は27,000の137%＝ペーシングの範囲内',
  '✅ 済み → 操作不要',
  '予定どおり9/10に「7日利益 ≥ 27,000時代の7日平均」で再テストする'],
 ['9/8','バランスケアスリッパ 増額判定（12,000→15,000）','1日あたり利益 ≥ 12,000時代(8/31-9/2)の +8,997円',
  '昨日「12,000で据え置き」と判定した時点で、実際は既に15,000へ上がっていた','⏳ 新規に設定',
  '★12,000での合格判定は15,000の是非を保証しない。3窓は余裕+1.06〜1.13で健全なので今は触らない'],
 ['9/10','快適マジックインソール 再テスト','27,000戻しの7日利益 ≥ 27,000時代の7日平均','—','予約',
  'P2の日別が単調改善（8/31 +9,383 → 9/01 +17,017 → 9/02 +19,442）だったため、戻したうえで7日窓で測り直す'],
 ['—','温感EMSフェイシャルワンド（3窓判定の誤検知）','新商品は購入50件に達するまで判定しない',
  '9/02新設。累計7件（9/02 1点 → 9/03 4点 → 9/04 2点）。MER1.82・分岐1.53・余裕+0.29','⏸ 判定しない',
  '★判定表に「⚠️−25%」と出るが、窓3つが同じ2日しか含んでいないための誤検知。触らないこと'],
 ['—','偏光・調光サングラス（私の判断が2転しました）','—',
  '消化 8/31 151 → 9/01 124 → 9/02 3,760 → 9/03 4,768円。3窓は分岐割れなし（7日余裕+0.14と薄い）',
  '⏸ ユーザー判断',
  '★9/2に「停止を」→9/4に「早計でした」と訂正したが間に合わず、キャンペーンごと停止された。'
  '「- コピー」広告セット(5,000円)はENABLEDのままなので再開はすぐ可能。'
  '非常ブレーキ中なので止めたままも妥当。私からは再提案しない'],
]
sh('本日の判定', ['宣言済みの基準に当てはめるだけ。基準は宣言時のまま動かさない',
  '★本日の執行は2件: ①**カタログ 18,000→10,000**（非常ブレーキ①・4日連続で未実行）'
  '②**完全遮光・接触冷感UVハット 6,000→4,500**（3窓とも余裕<0.30）。合わせて約9,500円/日',
  '★3日移動平均利益 95,960円 ＝ 床130,000円の74%。4日連続で発動中だが、底は打った（81,737→95,960）',
  '★予算スナップショットを6日ぶりにMeta直読で取り直した（8セット変更・合計255,000→238,000円）。'
  'インソールは既に27,000へ戻っており操作不要。スリッパは12,000ではなく15,000だった'],
...
for r in body:
    if r[0] == '2026-08-30b': PREVB[r[1]] += int(r[3])
BR, inc, dec = [], 0, 0
