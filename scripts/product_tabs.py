...
ws['A1'] = 'LOOTY 商品別 日次パフォーマンス（2026-06-29〜2026-07-28・全てShopify/Metaの実測）'
ws['A1'].font = Font(name='Arial', bold=True, size=13)
ws['A2'] = '各商品タブに日次の 売上／値引／販売数／原価／広告費／利益／利益率／MER／曜日補正MER／日予算／消化率 を載せています。'
ws['A3'] = '曜日指数（祝日7/20を除外した30日実測）: ' + '・'.join(f'{k}{v:.1f}' for k, v in sorted(IDX.items(), key=lambda x: -x[1]))
r = 5
...
for n in ADV:
    ...
    for lbl, dd in [('前日(7/28)', 1), ('3日(7/26-28)', 3), ('7日(7/22-28)', 7), ('30日(6/29-7/28)', 30)]:
